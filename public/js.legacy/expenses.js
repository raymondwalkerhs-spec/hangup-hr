...
window.ExpensesModule = (function () {
  ...
  function paidByDisplay(paymentMethod) {
  ...
  function paidByOptionsHtml(selected = "petty_cash") {
    const opts = [
  ...
  async function renderCostsPage(root, api, state, helpers) {
    ...
    const expenseQs = new URLSearchParams();
    if (showArchived) expenseQs.set("archived", "true");
    if (state.companyContext === "hs2") expenseQs.set("company", "hs2");
    const expenseQsStr = expenseQs.toString() ? `?${expenseQs.toString()}` : "";
    const reqs = [api(`/expenses${expenseQsStr}`)];
    if (canManage()) {
      const billsQs = state.companyContext === "hs2" ? "?company=hs2" : "";
      const fundsQs = state.companyContext === "hs2" ? "?company=hs2" : "";
      reqs.push(api(`/expenses/bills${billsQs}`), api(`/expenses/petty-cash/funds${fundsQs}`));
      if (state.pettyFundId) {
        const ledgerQs = new URLSearchParams();
        ledgerQs.set("fundId", state.pettyFundId);
        if (state.companyContext === "hs2") ledgerQs.set("company", "hs2");
        reqs.push(api(`/expenses/petty-cash/ledger?${ledgerQs.toString()}`).catch(() => ({ ledger: [] })));
      }
    }
    const results = await Promise.all(reqs);
    const expenseData = results[0];
    const expenseCategories = expenseData.categories || [
      "office", "travel", "meals", "utilities", "equipment", "marketing", "rent", "supplies", "other",
    ];
    ...
    root.innerHTML = `
      ...
      <div class="table-wrap card"><table>
        <thead><tr><th>Vendor</th><th>Category</th><th>Amount</th><th>Status</th><th>Paid by</th><th>Receipt</th><th>Priority</th><th>Due</th><th>Submitted by</th>${canManage() ? "<th></th>" : ""}</tr></thead>
        <tbody>${expenses.length ? expenses.map((e) => `<tr class="${isOverdue(e) ? "row-warn" : ""}">
          ...
          <td>${escapeHtml(e.paidByLabel || paidByDisplay(e.paymentMethod || e.paidBy))}</td>
        ...
        </tr>`).join("") : '<tr><td colspan="10" class="muted">No expenses</td></tr>'}
        </tbody>
      </table></div>`;

    root.querySelector("#add-expense-btn")?.addEventListener("click", () =>
      openExpenseModal(api, helpers, funds, expenseCategories, () => renderCostsPage(root, api, state, helpers))
    );
    ...
    root.querySelectorAll("[data-edit]").forEach((btn) => {
      ...
      if (expense) {
        btn.onclick = () =>
          openEditExpenseModal(api, helpers, expense, expenseCategories, () => renderCostsPage(root, api, state, helpers));
      }
    });
  ...
  function openMarkPaidModal(api, helpers, funds, expenseId, amount, onDone) {
    ...
    const pettyFunds = (funds || []).filter((f) => !/main\s*fund/i.test(f.fundName || ""));
    const defaultPetty = pettyFunds[0] || funds[0];
    const fundOpts = (pettyFunds.length ? pettyFunds : funds).map((f) =>
    ...
    openModal(`
      ...
      <form id="paid-form" class="form-grid modal-body-scroll">
        <label class="field"><span>Paid by</span><select name="paymentMethod" id="pay-method">
          ${paidByOptionsHtml("petty_cash")}
        </select></label>
        <label class="field" id="fund-field"><span>Petty cash fund</span><select name="pettyCashFundId">${fundOpts || `<option value="">No fund</option>`}</select></label>
    ...
    if (defaultPetty && fundSel) fundSel.value = defaultPetty.id;
    function updateHint() {
      if (methodSel.value !== "petty_cash" || !fundSel) {
        balanceHint.textContent =
          methodSel.value === "main_fund" ? "Paid from Main Fund (not deducted from petty cash)." : "";
        fundField?.classList.toggle("hidden", methodSel.value !== "petty_cash");
        return;
      }
      fundField?.classList.remove("hidden");
      const fund = funds.find((f) => f.id === fundSel.value);
      if (fund) {
        const after = fund.balance - amount;
        balanceHint.textContent = `Petty cash balance after: ${fmt(after)} EGP${after < 0 ? " (insufficient funds)" : ""}`;
      }
    }
    methodSel.onchange = updateHint;
    fundSel?.addEventListener("change", updateHint);
    updateHint();
    document.getElementById("paid-form").onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const method = String(fd.get("paymentMethod") || "petty_cash");
      const body = {
        status: "paid",
        paymentMethod: method,
        paidBy: method === "main_fund" ? "Main Fund" : "Petty cash",
      };
      if (method === "petty_cash") {
        body.pettyCashFundId = fd.get("pettyCashFundId") || defaultPetty?.id || null;
        if (!body.pettyCashFundId) return alert("Select a petty cash fund");
      } else {
        body.pettyCashFundId = null;
      }
      try {
  ...
  function categoryOptions(categories, selected = "other") {
    return (categories || ["other"]).map((c) => {
      const sel = c === selected ? "selected" : "";
      return `<option value="${c}" ${sel}>${c.replace(/_/g, " ")}</option>`;
    }).join("");
  }

  function openEditExpenseModal(api, helpers, expense, categories, onDone) {
    const { openModal, closeModal } = helpers;
    const currentPaidBy =
      expense.paymentMethod === "main_fund" || /main\s*fund/i.test(expense.paidBy || "")
        ? "main_fund"
        : "petty_cash";
    const statusOpts = ["paid", "pending", "on_hold", "pending_approval", "denied", "archived"]
      .map(
        (s) =>
          `<option value="${s}" ${expense.status === s ? "selected" : ""}>${s.replace(/_/g, " ")}</option>`
      )
      .join("");
    openModal(`
      ...
      <form id="edit-expense-form" class="form-grid modal-body-scroll">
        ...
        <label class="field"><span>Category</span><select name="category">${categoryOptions(categories, expense.category || "other")}</select></label>
        ...
        <label class="field"><span>Paid by</span><select name="paymentMethod">${paidByOptionsHtml(currentPaidBy)}</select></label>
        <p class="muted small" style="grid-column:1/-1;margin:0">Set status to <strong>pending</strong> if this cost is still waiting. Paid totals only include status = paid.</p>
    ...
    document.getElementById("edit-expense-form").onsubmit = async (e) => {
      ...
      const fd = new FormData(e.target);
      const method = String(fd.get("paymentMethod") || "petty_cash");
      const status = String(fd.get("status") || expense.status || "paid");
      const body = {
      ...
      if (method === "petty_cash" && expense.pettyCashFundId) {
        body.pettyCashFundId = expense.pettyCashFundId;
      }
      if (method === "main_fund") {
        body.pettyCashFundId = null;
      }
  ...
  function openExpenseModal(api, helpers, funds, categories, onDone) {
    const { openModal, closeModal } = helpers;
    openModal(`
      ...
      <form id="expense-form" class="form-grid modal-body-scroll">
        ...
        <label class="field"><span>Category</span><select name="category">${categoryOptions(categories)}</select></label>
