...
const PENDING_ATTENDANCE_KEY = "pendingAttendance";
...
function persistPendingAttendance() {
  try {
    ...
    localStorage.setItem(PENDING_ATTENDANCE_KEY, JSON.stringify(records));
  } catch {
    /* ignore storage errors */
  }
}

function restorePendingAttendance() {
  try {
    ...
    for (const r of records) {
      ...
      const emp = state.meta?.employees?.find((e) => e.id === r.employeeId);
      if (!emp) continue;
      const day = String(r.date || "").slice(0, 10);
      const hire = String(emp.employment_date || "").slice(0, 10);
      const depart = String(emp.depart_date || "").slice(0, 10);
      if (hire && day < hire) continue;
      if (depart && day > depart) continue;
      valid.push(r);
      state.pendingAttendance.set(`${r.employeeId}|${r.date}`, r);
    }
...
function clearPendingAttendance() {
  try {
    localStorage.removeItem(PENDING_ATTENDANCE_KEY);
  } catch {
    /* ignore */
  }
...
const state = {
  ...
  payrollSortKey: "",
  payrollSortDir: "asc",
  empFilter: { status: "", unit: "", nationality: "", workPermit: "", insuranceStatus: "", position: "", team: "", paymentMethod: "", fpStatus: "" },
...
function payrollDisplayNet(row) {
  if (!row) return 0;
  if (row.payrollKind === "dual") return Number(row.combinedNet ?? row.netSalary) || 0;
...
function employeeDepartedBeforeMonth(emp, month) {
  if (!isOutEmployeeStatus(emp?.status)) return false;
  const depart = String(emp?.depart_date || emp?.departDate || "").slice(0, 10);
...
function isAfterIsoDay(day, pivot) {
  const d = parseIsoDay(day);
  const p = parseIsoDay(pivot);
  return Boolean(d && p && d > p);
}

function filterPayrollHideOutRows(rows, month) {
  if (!state.hideOut) return rows;
  return rows.filter((r) => {
    if (employeeDepartedBeforeMonth(r, month) && !payrollRowWorkedInMonth(r)) return false;
    if (isOutEmployeeStatus(r.status) && !payrollRowWorkedInMonth(r)) return false;
    return true;
  });
}

function sumFilteredPayrollTotals(rows) {
  const totals = {
    totalBasic: 0,
    totalTransport: 0,
    totalBonuses: 0,
  ...
  for (const r of rows || []) {
    totals.totalBasic += Number(r.basicSalary) || 0;
    totals.totalTransport += Number(r.transportAllowance) || 0;
    totals.totalBonuses += Number(r.totalBonuses) || 0;
    totals.totalDeductions += Number(r.totalDeductions) || 0;
    totals.totalNet += payrollDisplayNet(r);
    totals.totalBonusTransfers += Number(r.bonusTransferPayroll) || 0;
  }
  return totals;
}

function applyPayrollRowFilters(rows, month) {
  let next = filterPayrollHideOutRows(rows, month);
  next = filterPayrollByZeroNet(next);
  if (getTabSearch("payroll")) {
    next = next.filter((r) => matchesEmployeeSearch(r, getTabSearch("payroll")));
  }
  ...
  if (state.payrollTeamFilter) {
    next = next.filter((r) => (r.team || "") === state.payrollTeamFilter);
  }
  if (state.payrollUnitFilter) {
    next = next.filter((r) => (r.unit || "") === state.payrollUnitFilter);
  }
  if (state.payrollStatusFilter) {
    next = next.filter((r) => (r.payrollStatus || "") === state.payrollStatusFilter);
  }
  return next;
}
  ...
  window.addEventListener("beforeunload", (e) => {
    if (state.pendingAttendance.size > 0) {
      ...
      if (navigator.sendBeacon) {
        navigator.sendBeacon("/api/attendance/batch", new Blob([payload], { type: "application/json" }));
      }
      try {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "/api/attendance/batch", false);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.send(payload);
      } catch {
        // ignore sync XHR errors
      }
      state.pendingAttendance.clear();
      try { localStorage.removeItem(PENDING_ATTENDANCE_KEY); } catch { /* ignore */ }
      e.preventDefault();
...
const PAGE_CRUMB = {
  ...
  loans: { title: "Loans" },
  "loan-approvals": { title: "Loan approvals" },
  salaries: { title: "Salaries" },
  reports: { title: "Reports" },
  analytics: { title: "Analytics" },
  sales: { title: "Sales log" },
  breaks: { title: "Breaks" },
  "team-dashboard": { title: "Team dashboards" },
  costs: { title: "Costs" },
  "meeting-requests": { title: "Meeting Requests" },
  requests: { title: "Requests" },
  payslip: { title: "My payslip" },
  "it-requests": { title: "IT Requests" },
  equipment: { title: "Equipment" },
  org: { title: "Organization" },
  settings: { title: "Settings" },
  users: { title: "Users" },
  "access-control": { title: "Access Control" },
  "sales-permissions": { title: "Sales permissions" },
  "sales-log-columns": { title: "Log columns" },
  rules: { title: "Rules" },
...
function updateTopCrumb(page) {
  const titleEl = document.getElementById("top-crumb-title");
  const subEl = document.getElementById("top-crumb-sub");
  if (!titleEl) return;
  const crumb = PAGE_CRUMB[page] || { title: page };
  titleEl.textContent = crumb.title;
  if (subEl) subEl.textContent = crumb.sub || "";
}
...
function paymentMethodFieldsHtml(emp = {}) {
  ...
  return `
    ...
    <div id="pay-insta-wrap" class="field ${pm === "instapay" ? "" : "hidden"}" style="grid-column:1/-1">
    ...
    <div id="pay-bank-wrap" class="${pm === "bank" ? "" : "hidden"}" style="grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr;gap:1rem">
...
function bindPaymentMethodFields() {
  ...
  const refresh = () => {
    const v = sel.value;
    document.getElementById("pay-insta-wrap")?.classList.toggle("hidden", v !== "instapay");
    document.getElementById("pay-cash-wrap")?.classList.toggle("hidden", v !== "cash");
    document.getElementById("pay-bank-wrap")?.classList.toggle("hidden", v !== "bank");
  };
...
async function api(path, options = {}, timeoutMs = 120000) {
  ...
  if (res.status === 401) {
    window.hrAuthDebug?.log("app.api.401", { path, sessionId: window.hrAuthDebug?.maskSessionId(sessionId) });
    clearSessionId();
    window.location.href = "/login";
...
const ICON_FALLBACKS = {
  menu: "☰", refresh: "↻", "refresh-cw": "↻", bell: "🔔", eye: "👁", x: "✕",
  lock: "🔒", check: "✓", trash: "🗑", "more-vertical": "⋯", search: "⌕",
  pencil: "✎", download: "⤓", chevron: "›", "chevron-down": "⌄", alert: "!",
  calendar: "📅", filter: "⚲", plus: "+", "arrow-left": "←", "arrow-right": "→",
  upload: "⤒", user: "👤", "file-text": "🗎", "log-out": "⏻",
  "layout-dashboard": "▦", users: "👥", "building-2": "🏢", monitor: "🖥",
  "calendar-check": "📅", coffee: "☕", inbox: "📥", "calendar-plus": "🗓",
  laptop: "💻", wallet: "👛", scale: "⚖", gift: "🎁", scissors: "✂",
  "hand-coins": "🪙", "check-check": "✔", "receipt-text": "🧾", "line-chart": "📈",
  "users-round": "👥", "pie-chart": "🥧", "bar-chart-3": "📊", "user-cog": "🛠",
  shield: "🛡", "key-round": "🔑", "columns-3": "▤", "scroll-text": "📜",
  history: "🕘", settings: "⚙", "arrow-up": "↑", "arrow-down": "↓",
};
function refreshIcons(root) {
  if (window.lucide && typeof window.lucide.createIcons === "function") {
    try { window.lucide.createIcons(root ? { nameAttr: "data-lucide", root } : undefined); }
    catch { try { window.lucide.createIcons(); } catch { /* noop */ } }
  }
  // Offline / CDN-missing fallback: show the unicode glyph for any icon Lucide did not replace.
  document.querySelectorAll("i[data-lucide]").forEach((el) => {
    if (el.querySelector("svg")) return;
    el.textContent = ICON_FALLBACKS[el.dataset.lucide] || "•";
  });
}
...
function getUserCompanyContext() {
  const unit = String(state.user?.unit || "").trim();
  const isHs2 = /^(HS2|HS-2|PT)/i.test(unit);
  return isHs2 ? "hs2" : "hangup";
}

function isRaymond() {
  return String(state.user?.username || "").trim().toLowerCase() === "raymond";
}

function getDepartDateConflicts(employees, month) {
  const conflicts = [];
  for (const emp of employees || []) {
    const depart = String(emp?.depart_date || "").slice(0, 10);
    if (!depart || !String(depart).startsWith(month)) continue;
...
function applyCompanyBranding() {
  ...
  const isHs2 = state.companyContext === "hs2";
  if (title) title.textContent = isHs2 ? "Hangup HS-2" : "Hangup Portal";
  document.body.classList.toggle("company-hs2", isHs2);
  if (!wrap) return;
  if (canManageHs2Company()) {
    ...
    wrap.querySelectorAll("[data-company]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        sessionStorage.setItem("companyContext", next);
        applyCompanyBranding();
        render();
      });
    });
  } else {
    wrap.innerHTML = isHs2
      ? `<span class="company-switcher-label">Viewing: <strong>HS-2</strong></span>`
      : `<span class="company-switcher-label">Viewing: <strong>Main Hangup</strong></span>`;
  }
}

function openConfirmDeleteModal({ title, message, onConfirm }) {
  openModal(
    `<div class="modal-header"><h2>${escapeHtml(title || "Confirm deletion")}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
function openConfirmModal({ title, message, confirmLabel = "Confirm", danger = false, onConfirm }) {
  openModal(
    `<div class="modal-header"><h2>${escapeHtml(title || "Confirm")}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
function showRegistrationCredentialsModal(res, note = "") {
  ...
  const usedAgentPassword = Boolean(res.usedAgentPassword);
  const loginActive = Boolean(res.loginActive);
  const statusLine = loginActive
    ? "Login is <strong>active</strong>. The agent can sign in with their User ID and the password they chose at registration."
    : "Login could not be activated — check Users page.";
  const passwordRow = usedAgentPassword
    ? `<p class="muted small">Password: the one the agent chose during registration (not shown here).</p>`
    : copyRow("Temp password", res.tempPassword);
  openModal(
    `<div class="modal-header"><h2>Registration approved</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
      <p class="muted" style="margin-top:0">${statusLine}</p>
      <div class="cred-box">
...
function openPromptModal({ title, message = "", defaultValue = "", inputType = "text", placeholder = "", confirmLabel = "OK", required = false, onSubmit }) {
  openModal(
    `<div class="modal-header"><h2>${escapeHtml(title || "Enter value")}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
function openDeferAmountModal({ title, message, defaultMonth, defaultAmount, maxAmount, onSubmit }) {
  openModal(
    `<div class="modal-header"><h2>${escapeHtml(title || "Defer payment")}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
function applyChangesButtonVisibility() {
  ...
  if (analyticsBtn) analyticsBtn.classList.toggle("hidden", !canViewReports());
  ...
  const interviewBtn = document.getElementById("nav-interview");
  if (interviewBtn) {
    interviewBtn.classList.toggle("hidden", state.user?.canViewInterviews !== true);
  }
  const trainingBtn = document.getElementById("nav-training");
  if (trainingBtn) {
    const role = state.user?.role;
    const canAccess = ["admin", "ceo", "hr", "op", "tl", "quality"].includes(role);
    trainingBtn.classList.toggle("hidden", !canAccess);
  }
  const announcementsBtn = document.getElementById("nav-announcements");
  if (announcementsBtn) {
    const hideAnn =
      state.user?.canViewAnnouncements === false && state.user?.canEditAnnouncements !== true;
    announcementsBtn.classList.toggle("hidden", hideAnn);
  }
  const coachingBtn = document.getElementById("nav-coaching");
  if (coachingBtn) {
    const role = String(state.user?.role || "").toLowerCase();
    const showCoaching =
      state.user?.canViewCoaching === true ||
      state.user?.canSubmitCoaching === true ||
      ["tl", "op", "quality", "hr", "rtm", "admin", "ceo"].includes(role) ||
      Boolean(state.user?.employeeId);
    coachingBtn.classList.toggle("hidden", state.user?.canViewCoaching === false || !showCoaching);
  }
...
function filterEmployeesList(employees) {
  ...
  if (state.empFilter.position) list = list.filter((e) => e.position === state.empFilter.position);
  if (state.empFilter.team) list = list.filter((e) => e.team === state.empFilter.team);
  if (state.empFilter.paymentMethod) list = list.filter((e) => (e.paymentMethod || e.payment_method) === state.empFilter.paymentMethod);
  if (state.empFilter.fpStatus) {
    if (state.empFilter.fpStatus === "has_fp") list = list.filter((e) => !!(e.fp_number || e.fpNumber));
    if (state.empFilter.fpStatus === "no_fp") list = list.filter((e) => !(e.fp_number || e.fpNumber));
  }
...
function employeeListRowHtml(e) {
  ...
  const fpBadge = (!e.fp_number && !e.fpNumber && !stub) ? '<span class="badge badge-warn" style="margin-left:.35rem" title="No FP number assigned">No FP</span>' : "";
  return `<tr class="${rowClass}${stub ? " row-stub" : ""}" data-emp="${e.id}">
    <td><div class="emp-cell">${avatarHtml(e)}<strong>${name}</strong>${fpBadge}${stub ? '<span class="stub-badge">No user assigned</span>' : ""}</div></td>
...
function attendanceEmployeeRowHtml(emp, ctx) {
  ...
  return `<tr data-emp-row="${emp.id}">
    ...
    <td class="text-center att-summary-col">${s.workingDays || 0}</td>
    <td class="text-center att-summary-col">${s.lateness || 0}</td>
    <td class="text-center att-summary-col">${s.latenessDeductions || 0}</td>
    ${days.map((d) => {
      const rec = recordMap.get(`${emp.id}|${d}`);
      const st = rec?.status || "";
      const depart = parseIsoDay(emp.depart_date);
      const locked = depart && isAfterIsoDay(d, depart);
      const dayCls =
      ...
      const holidayNote = holidayName
        ...
        : "";
      const lockNote = locked ? `<div class="att-lock-note" title="After depart date"><i data-lucide="lock"></i></div>` : "";
...
/* Shared floating popover/menu — portaled to <body> so it is never clipped by
   scroll containers (attendance grid, sales table). Closes on outside-click,
   Esc, scroll, resize. align: "left" | "right" | "center". */
let __activePopover = null;
function closeActivePopover() {
  if (__activePopover) {
    __activePopover.el.remove();
    document.removeEventListener("click", __activePopover.onDoc, true);
    document.removeEventListener("keydown", __activePopover.onKey, true);
    window.removeEventListener("resize", __activePopover.onResize, true);
    __activePopover = null;
  }
}
function openPopover(anchor, { html, align = "left", variant = "popover", className = "", onClose } = {}) {
  closeActivePopover();
  const el = document.createElement("div");
  el.className = `popover ${variant === "menu" ? "menu" : ""}${variant === "att" ? " att-popover" : ""}${className ? " " + className : ""}`;
  el.innerHTML = html;
  ...
  let left;
  if (align === "right") left = rect.right - pw;
  ...
  const onKey = (e) => { if (e.key === "Escape") { closeActivePopover(); onClose?.(); } };
  const onResize = () => { closeActivePopover(); onResize?.(); };
  setTimeout(() => {
...
/* Custom select component — progressive enhancement over a native <select>.
   Keeps the native <select> in the DOM (so FormData / change handlers keep
   working) and renders a styled trigger + popover menu of options. */
function enhanceSelect(selectEl) {
  if (!selectEl || selectEl.__selectEnhanced || selectEl.classList.contains("transport-ov-select")) return;
  selectEl.__selectEnhanced = true;
  ...
  trigger.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (trigger.__selectOpen) { closeActivePopover(); return; }
    const opts = Array.from(selectEl.options);
    const menuHtml = opts
      .map((o, i) => `<button type="button" class="select-option${o.selected ? " selected" : ""}" role="option" aria-selected="${o.selected}" data-i="${i}">${escapeHtml(o.textContent)}</button>`)
      .join("");
    const pop = openPopover(trigger, { html: menuHtml, align: "left", variant: "popover", className: "select-menu", onClose: () => { trigger.__selectOpen = false; trigger.setAttribute("aria-expanded", "false"); } });
    pop.setAttribute("role", "listbox");
    trigger.__selectOpen = true;
    trigger.setAttribute("aria-expanded", "true");
    pop.addEventListener("keydown", (ev) => {
      const items = Array.from(pop.querySelectorAll(".select-option"));
    ...
    pop.querySelectorAll(".select-option").forEach((mi) => {
    ...
    pop.querySelector(".select-option")?.focus();
  });
}

function enhanceSelectsWithin(root) {
  if (!root) return;
  root.querySelectorAll("select:not(.transport-ov-select)").forEach((s) => enhanceSelect(s));
}
...
function bindAttendanceGridEvents(root, ctx) {
  ...
  function onTransportOverrideChange(e) {
    ...
    const statusSel = cell?.querySelector(".status-chip");
  ...
  function openStatusPopover(chip) {
    ...
    const html = statuses.map((x) => {
      const sel = x === current ? " selected" : "";
      return `<button type="button" class="menu-item${sel}" data-value="${escapeHtml(x)}">
        <span class="status-pill ${statusClass(x)}">${escapeHtml(x === "Day-OFF" && isWeekend(date) ? "OFF★" : x)}</span>
      </button>`;
    }).join("");
    const pop = openPopover(chip, {
      html: `<div class="att-popover-inner">${html}</div>`,
      align: "center",
      variant: "att",
    });
    pop.querySelectorAll(".menu-item").forEach((item) => {
      item.addEventListener("click", () => {
        ...
        chip.className = `status-chip status-select ${statusClass(value)}`;
        const label = value || "—";
        chip.innerHTML = `<span>${escapeHtml(value === "Day-OFF" && isWeekend(date) ? "OFF★" : label)}</span>`;
        const cell = chip.closest(".att-cell");
        const ovSel = cell?.querySelector(".transport-ov-select");
        if (ovSel) {
          if (isTransportOverrideStatus(value)) ovSel.classList.remove("hidden");
          else { ovSel.classList.add("hidden"); ovSel.value = ""; }
        } else if (isTransportOverrideStatus(value) && cell && canEdit) {
          cell.insertAdjacentHTML("beforeend", transportOverrideHtml(emp, date, value, "", true));
        ...
        queueAttendanceSave(emp, date, value, ov);
        closeActivePopover();
      });
    });
  }

  root.querySelectorAll(".status-chip").forEach((sel) => {
    if (sel.tagName === "BUTTON") {
      sel.addEventListener("click", (e) => { e.preventDefault(); openStatusPopover(sel); });
      return;
    }
...
function updatePayrollTable(root) {
  ...
  let rows = applyPayrollRowFilters(view.rows || [], state.month);
  if (state.payrollSortKey) {
    const key = state.payrollSortKey;
    const dir = state.payrollSortDir === "asc" ? 1 : -1;
    rows = [...rows].sort((a, b) => {
      const av = key === "salesCount" ? (a.salesCount || 0) : key === "commissionAmount" ? (a.commissionAmount || 0) : key === "totalWorkingDays" ? (a.totalWorkingDays || 0) : key === "basicSalary" ? (a.basicSalary || 0) : key === "transportAllowance" ? (a.transportAllowance || 0) : key === "loanDeductionTotal" ? (a.loanDeductionTotal || 0) : key === "netSalary" ? (a.netSalary || 0) : 0;
      const bv = key === "salesCount" ? (b.salesCount || 0) : key === "commissionAmount" ? (b.commissionAmount || 0) : key === "totalWorkingDays" ? (b.totalWorkingDays || 0) : key === "basicSalary" ? (b.basicSalary || 0) : key === "transportAllowance" ? (b.transportAllowance || 0) : key === "loanDeductionTotal" ? (b.loanDeductionTotal || 0) : key === "netSalary" ? (b.netSalary || 0) : 0;
  ...
  const tab = state.payrollTab || "agent";
  const totals = sumFilteredPayrollTotals(rows);
  ...
  const statTransport = root.querySelector("#payroll-stat-transport");
  ...
  if (statTransport) statTransport.textContent = fmt(totals.totalTransport || 0);
...
function attendanceStatusCellHtml(empId, d, displaySt, rec, canEdit, locked) {
  ...
  const statuses = window.__attendanceStatuses || [];
  const label = displaySt || "—";
  const chip = `<button type="button" class="status-chip ${statusClass(displaySt)}" data-emp="${empId}" data-date="${d}" data-value="${escapeHtml(displaySt)}" title="Set status">
    <span>${escapeHtml(displaySt === "Day-OFF" && isWeekend(d) ? "OFF★" : label)}</span>
  </button>`;
  return `${chip}
...
function queueAttendanceSave(employeeId, date, status, transportOverride) {
  ...
  const root = document.getElementById("app");
  const emp =
    root?.__attendanceCtx?.data?.employees?.find((e) => e.id === employeeId) ||
    state.meta?.employees?.find((e) => e.id === employeeId);
  const hire = parseIsoDay(emp?.employment_date);
  const depart = parseIsoDay(emp?.depart_date);
  const day = parseIsoDay(date);
  if (hire && day && day < hire) {
    showSaveIndicator("Cannot edit attendance before employment date.", "error");
    return;
  }
  if (depart && day && isAfterIsoDay(day, depart)) {
    showSaveIndicator("Days after depart date are locked as OUT.", "error");
    return;
  }
  ...
  // Flush immediately — no debounce. A 50ms delay means a quick F5 can
  // unload the page before the save ever starts, causing the revert-to-blank bug.
  state.saveTimer = setTimeout(flushAttendanceSaves, 0);
}
...
async function flushAttendanceSaves(useBeacon = false) {
  ...
  try {
    ...
    if (records.length > 0 && batchRes?.saved && batchRes.saved.length === 0) {
      const accessDenied = (batchRes?.skipped || []).some((s) => s.reason === "access_denied");
      const locked = (batchRes?.skipped || []).some((s) => s.reason && s.reason.includes("locked"));
      const employmentBlocked = (batchRes?.skipped || []).some(
        (s) => s.reason && (s.reason.includes("employment") || s.reason.includes("depart"))
      );
      const errMsg = accessDenied
        ? "Save failed — no permission to edit these employees"
        : locked
          ? "Save failed — payroll month is locked"
          : employmentBlocked
            ? batchRes.skipped.find((s) => s.reason)?.reason || "Save failed — outside employment period"
            : "Save failed — records were rejected by the server";
      console.warn("[attendance-debug] all records dropped by server", { skipped: batchRes?.skipped });
      showSaveIndicator(errMsg, "error");
      return;
    }
    ...
    const savedKeys = new Set((batchRes?.saved || []).map((r) => `${r.employeeId}|${r.date}`));
    const skippedKeys = new Set((batchRes?.skipped || []).map((r) => `${r.employeeId}|${r.date}`));
    for (const r of records) {
      const key = `${r.employeeId}|${r.date}`;
      if (savedKeys.has(key) || skippedKeys.has(key)) {
        state.pendingAttendance.delete(key);
      }
    }

    if ((batchRes?.skipped || []).length && (batchRes?.saved || []).length) {
      const firstSkip = batchRes.skipped[0];
      showSaveIndicator(firstSkip?.reason || "Some attendance rows were not saved", "error");
      persistPendingAttendance();
      return;
    }
    ...
    if ((batchRes?.skipped || []).length && !(batchRes?.saved || []).length) {
      const firstSkip = batchRes.skipped[0];
      showSaveIndicator(firstSkip?.reason || "Save failed — records were rejected", "error");
...
function employeeFormFields(emp = {}) {
  ...
  const appIdDisplay = emp.archived_app_id && String(emp.id || "").startsWith("DEL-")
    ? emp.archived_app_id
    : emp.id;
  // Filter units by company context
  const currentCompany = emp.company || state.companyContext || "hangup";
  const filteredUnits = filterUnitsByCompany(state.meta.units, currentCompany);
  // Company dropdown for Raymond only
  const companyDropdown = isRaymond()
    ? `<label class="field"><span>Company</span><select name="company" id="emp-company-select">
        <option value="hangup" ${currentCompany !== "hs2" ? "selected" : ""}>Main Hangup</option>
        <option value="hs2" ${currentCompany === "hs2" ? "selected" : ""}>HS-2</option>
      </select></label>`
    : "";
  return `<form id="emp-form" class="field-grid">
    <div class="emp-tab-panel" data-tab="identity">
      <label class="field"><span>App ID (shown in HR screens)</span>
        ${canManagePayrollEvents() && emp.status !== "Deleted" && !isUnassignedIdStub(emp)
          ...
          : `<input name="app_id_display" value="${escapeHtml(appIdDisplay || "")}" readonly /></label>`}
      </label>
      ${dbIdNote}
      ${historyNote}
      ${companyDropdown}
      ...
      <label class="field"><span>Unit</span><select name="unit" id="emp-unit-select">${filteredUnits.map((u) =>
    ...
    <div class="emp-tab-panel" data-tab="contact" hidden>
    ...
    <div class="emp-tab-panel" data-tab="payroll" hidden>
...
function openEmployeeModal(emp, options = {}) {
  ...
  openModal(`
    <div class="modal-header"><h2>Edit employee</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body" id="emp-modal-body">
      ...
      <div class="emp-tabs" role="tablist">
        <button type="button" class="emp-tab active" data-tab="identity" role="tab">Identity</button>
        <button type="button" class="emp-tab" data-tab="contact" role="tab">Contact &amp; Dates</button>
        <button type="button" class="emp-tab" data-tab="payroll" role="tab">Payroll</button>
        <button type="button" class="emp-tab" data-tab="lifecycle" role="tab">Lifecycle</button>
      </div>
      ${canChangeAppId ? `` : ""}
      <div class="emp-tab-panel" data-tab="lifecycle" hidden>
  ...
  const empModalBody = document.getElementById("emp-modal-body");
  empModalBody?.querySelectorAll(".emp-tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      const name = tab.dataset.tab;
      empModalBody.querySelectorAll(".emp-tab").forEach((t) => t.classList.toggle("active", t === tab));
      empModalBody.querySelectorAll(".emp-tab-panel").forEach((p) => { p.hidden = p.dataset.tab !== name; });
    });
  });
  enhanceSelectsWithin(document.getElementById("emp-form"));
  ...
  // Company dropdown handler for Raymond - updates unit options
  const companySelect = document.getElementById("emp-company-select");
  if (companySelect) {
    companySelect.addEventListener("change", () => {
      const newCompany = companySelect.value;
      const unitSelect = document.getElementById("emp-unit-select");
      if (unitSelect) {
        const newUnits = filterUnitsByCompany(state.meta.units, newCompany);
        unitSelect.innerHTML = newUnits.map((u) =>
  ...
  document.getElementById("save-emp").onclick = async () => {
    ...
    delete body.company; // Company is inferred from unit, not stored on employee
    ...
    if (markingOut) {
      openModal(`
        <div class="modal-header"><h2>Mark depart</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function openPromoteEmployeeModal(emp) {
  ...
  openModal(
    `<div class="modal-header"><h2>Reposition ${escapeHtml(emp.american_name || emp.id)}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
async function openPayslipModal(employeeId, options = {}) {
    ...
    ? `<div class="alert alert-info" style="margin-bottom:1rem" id="dual-payslip-banner">
        ...
        <div class="btn-row payslip-tabs" style="margin-top:.5rem">
          <div class="segmented" role="tablist">
            <button type="button" class="btn btn-sm ${initialTab === "training" ? "active" : ""}" data-payslip-tab="training" role="tab">Training</button>
            <button type="button" class="btn btn-sm ${initialTab === "agent" ? "active" : ""}" data-payslip-tab="agent" role="tab">Agent</button>
            <button type="button" class="btn btn-sm ${initialTab === "combined" ? "active" : ""}" data-payslip-tab="combined" role="tab">Combined</button>
          </div>
        </div>
      </div>`
    : "";
  ...
  openModal(
    ...
    <div class="modal-body payslip" id="payslip-modal-body">
      ...
      <div class="payslip-header profile-hero">
      ...
      <div class="grid-2" style="margin-top:1rem">
        <div class="card">
          <h4>Extra Payroll</h4>
          <div id="extra-payroll-list">${(data.extraPayroll || []).map((e) => `<div class="payslip-row" style="display:flex;justify-content:space-between;align-items:center"><span>${escapeHtml(e.label)}: ${e.workingDays}d × ${fmt(e.dailyRate)} = <strong>${fmt(e.netAmount)}</strong></span><span style="display:flex;gap:.35rem"><button class="btn btn-sm" data-extra-pdf="${e.id}">PDF</button><button class="btn btn-sm btn-danger" data-del-extra='${JSON.stringify({ id: e.id })}'>Delete</button></span></div>`).join("") || '<p class="muted">None</p>'}</div>
          <form id="extra-payroll-form" class="field-grid" style="margin-top:.75rem">
            <label class="field"><span>Label</span><input name="label" placeholder="e.g. Extra, Training, Part-time" required /></label>
            <label class="field"><span>Working days</span><input name="workingDays" type="number" step="0.5" min="0" value="0" /></label>
            <label class="field"><span>Day rate (EGP)</span><input name="dailyRate" type="number" step="0.01" min="0" value="0" /></label>
            <label class="field"><span>Net amount (EGP)</span><input name="netAmount" type="number" step="0.01" min="0" value="0" /></label>
          </form>
          <button class="btn btn-primary btn-sm" id="add-extra-payroll-btn" style="margin-top:.5rem">Add extra payroll</button>
        </div>
      </div>
        ...
        <form id="adj-form" class="field-grid">
          ...
          <label class="field"><span>Position salary override <span class="muted" style="font-size:.75rem">(replaces position rate for this month only)</span></span>
            <div style="display:flex;gap:.4rem;align-items:center">
              ...
              <button type="button" class="btn btn-sm btn-icon" id="clear-salary-override-btn" title="Remove override"><i data-lucide="x"></i></button>
            </div>
          </label>
          <label class="field"><span>Net salary override <span class="muted" style="font-size:.75rem">(replaces calculated net salary for this month only)</span></span>
            <div style="display:flex;gap:.4rem;align-items:center">
              ...
              <button type="button" class="btn btn-sm btn-icon" id="clear-net-salary-override-btn" title="Remove override"><i data-lucide="x"></i></button>
            </div>
          </label>
  ...
  enhanceSelect(document.querySelector('[name="payrollStatus"]'));
  ...
  const extraPayrollForm = document.getElementById("extra-payroll-form");
  if (extraPayrollForm) {
    const calcExtraNet = () => {
      const days = Number(extraPayrollForm.querySelector('[name="workingDays"]')?.value || 0);
      const rate = Number(extraPayrollForm.querySelector('[name="dailyRate"]')?.value || 0);
      const net = days * rate;
      const netInput = extraPayrollForm.querySelector('[name="netAmount"]');
      if (netInput && net > 0) netInput.value = net.toFixed(2);
    };
    extraPayrollForm.querySelector('[name="workingDays"]')?.addEventListener("input", calcExtraNet);
    extraPayrollForm.querySelector('[name="dailyRate"]')?.addEventListener("input", calcExtraNet);
  }
  document.getElementById("add-extra-payroll-btn")?.addEventListener("click", async () => {
    const body = Object.fromEntries(new FormData(document.getElementById("extra-payroll-form")));
    try {
      await api(`/payroll-extra/${employeeId}/${state.month}`, {
        method: "POST",
        body: JSON.stringify({
          label: body.label || "Extra",
  ...
  document.querySelectorAll("[data-del-extra]").forEach((btn) => {
    btn.onclick = () => {
      const payload = JSON.parse(btn.dataset.delExtra);
      openConfirmDeleteModal({
        title: "Delete extra payroll",
        message: `Delete this extra payroll entry?`,
        onConfirm: async () => {
          await api(`/payroll-extra/${payload.id}`, { method: "DELETE" });
          closeModal();
          openPayslipModal(employeeId);
        },
      });
  ...
  document.querySelectorAll("[data-extra-pdf]").forEach((btn) => {
    btn.onclick = () => {
      const id = btn.dataset.extraPdf;
      downloadFile(`/payroll-extra/${id}/pdf`, `extra-payroll-${id}.pdf`).catch((e) => alert(e.message));
    };
  });
...
async function renderEmployees(root) {
  ...
  root.innerHTML = `
    ...
    ${showFilters ? `<div class="toolbar">
      ...
      <select id="filter-position"><option value="">All positions</option>${(data.positions || []).map((p) =>
        `<option value="${escapeHtml(p)}" ${state.empFilter.position === p ? "selected" : ""}>${escapeHtml(p)}</option>`
      ).join("")}</select>
      <select id="filter-team"><option value="">All teams</option>${(data.teams || []).map((t) =>
        `<option value="${escapeHtml(t)}" ${state.empFilter.team === t ? "selected" : ""}>${escapeHtml(t)}</option>`
      ).join("")}</select>
      <select id="filter-payment"><option value="">All payment methods</option>${(data.paymentMethods || []).map((m) =>
        `<option value="${escapeHtml(m)}" ${state.empFilter.paymentMethod === m ? "selected" : ""}>${escapeHtml(m)}</option>`
      ).join("")}</select>
      <select id="filter-fp"><option value="">All FP statuses</option>
        <option value="has_fp" ${state.empFilter.fpStatus === "has_fp" ? "selected" : ""}>Has FP</option>
        <option value="no_fp" ${state.empFilter.fpStatus === "no_fp" ? "selected" : ""}>No FP</option>
      </select>
  ...
  if (showFilters) {
    ...
    root.querySelector("#filter-position")?.addEventListener("change", (e) => {
      state.empFilter.position = e.target.value;
      updateEmployeesTable(root);
    });
    root.querySelector("#filter-team")?.addEventListener("change", (e) => {
      state.empFilter.team = e.target.value;
      updateEmployeesTable(root);
    });
    root.querySelector("#filter-payment")?.addEventListener("change", (e) => {
      state.empFilter.paymentMethod = e.target.value;
      updateEmployeesTable(root);
    });
    root.querySelector("#filter-fp")?.addEventListener("change", (e) => {
      state.empFilter.fpStatus = e.target.value;
      updateEmployeesTable(root);
    });
...
async function renderAttendance(root) {
  ...
  const transportHint = canViewTransportControls()
    ...
    : "";
  const primaryToolbar = `${pageSearchInputHtml("attendance", "Search name, Arabic name, or ID…")}
    <select id="unit-filter" class="att-filter" title="Filter by unit"><option value="">All units</option>${(data.units || []).map((u) =>
      `<option value="${u}" ${state.unit === u ? "selected" : ""}>${u}</option>`
    ).join("")}</select>
    <select id="team-filter" class="att-filter" title="Filter by team"><option value="">All teams</option>${(data.teams || []).map((t) =>
      `<option value="${t}" ${state.team === t ? "selected" : ""}>${t}</option>`
    ).join("")}</select>`;
  const editToolbar = attEditable
    ? `${primaryToolbar}
    <button type="button" class="btn" id="bulk-actions-toggle"><i data-lucide="sliders-horizontal"></i> Bulk actions</button>
    <div id="bulk-actions-panel" class="bulk-actions-panel" hidden>
      <div class="bulk-actions-group">
        <span class="bulk-actions-label">Working days</span>
        <label class="wd-stepper"><input type="number" id="wd-input" min="1" max="31" value="${data.workingDays || 22}" /> <button class="btn btn-sm" id="save-wd">Save</button></label>
      </div>
      <div class="bulk-actions-group">
      ...
      <div class="bulk-actions-group">
        <span class="bulk-actions-label">Reset</span>
        <button class="btn" id="bulk-reset-agent" title="Remove all attendance for selected agent this month">Reset month</button>
        <button class="btn" id="bulk-reset-unit" title="Remove all attendance for all employees in current unit this month">Reset unit month</button>
        <button class="btn" id="bulk-reset-company" title="Remove all attendance for all employees this month">Reset company month</button>
      </div>
      <div class="bulk-actions-group">
  ...
  root.innerHTML = `
    ...
    <div class="table-wrap attendance-grid"><table>
      <thead><tr>
        ...
        <th class="text-center att-summary-col">Work</th><th class="text-center att-summary-col">Late</th><th class="text-center att-summary-col">Ded.</th>
        ${data.days.map((d) => {
  ...
  root.querySelector("#bulk-agent-month")?.addEventListener("click", async () => {
    ...
    const emp = (state.meta?.employees || []).find((e) => e.id === employeeId);
    const month = state.month;
    const isRaymondUser = isRaymond();
    let confirmMsg = `Mark all weekdays in ${monthLabel(month)} as Attended for ${emp?.american_name || emp?.arabic_name || employeeId}?`;
    let force = false;

    if (!isRaymondUser && emp?.depart_date) {
      const depart = String(emp.depart_date || "").slice(0, 10);
      if (String(depart).startsWith(month)) {
        const [year, mo] = month.split("-").map(Number);
        const daysInMonth = new Date(year, mo, 0).getDate();
        const departDayNum = parseInt(depart.slice(8, 10), 10);
        const affectedDays = daysInMonth - departDayNum;
        if (affectedDays > 0) {
          confirmMsg = `⚠ Warning: ${emp?.american_name || emp?.arabic_name || employeeId} has an Out Date on ${depart}.\nDays after ${depart} will be overwritten.\n\nContinue?`;
        }
      }
    }
    ...
    try {
      await api("/attendance/bulk-agent-month", {
        method: "PATCH",
        body: JSON.stringify({ month, employeeId, status: "Attended", force: isRaymondUser || force }),
      });
      render();
    } catch (e) {
      alert(e.message);
  ...
  root.querySelector("#bulk-reset-agent")?.addEventListener("click", async () => {
    const employeeId = root.querySelector("#bulk-agent-select")?.value;
    if (!employeeId) {
      alert("Select an agent first.");
      return;
    ...
    const emp = (state.meta?.employees || []).find((e) => e.id === employeeId);
    const month = state.month;
    if (!confirm(`Remove all attendance records for ${emp?.american_name || emp?.arabic_name || employeeId} in ${monthLabel(month)}?`)) return;
    try {
      const res = await api("/attendance/bulk-reset-month", {
        method: "PATCH",
        body: JSON.stringify({ month, employeeId }),
      });
      showSaveIndicator(`Reset ${res.count} records`, "saved");
      render();
    } catch (e) {
      alert(e.message);
    }
  });
  root.querySelector("#bulk-reset-unit")?.addEventListener("click", async () => {
    const month = state.month;
    if (!confirm(`Remove all attendance records for all employees in unit ${state.unit || "current"} in ${monthLabel(month)}?`)) return;
    try {
      const res = await api("/attendance/bulk-reset", {
        method: "PATCH",
        body: JSON.stringify({ month, unit: state.unit, scope: "unit" }),
      });
      showSaveIndicator(`Reset ${res.count} records`, "saved");
      render();
    } catch (e) {
  ...
  root.querySelector("#bulk-reset-company")?.addEventListener("click", async () => {
    const month = state.month;
    if (!confirm(`Remove ALL attendance records for all employees in ${monthLabel(month)}?`)) return;
    try {
      const res = await api("/attendance/bulk-reset", {
        method: "PATCH",
        body: JSON.stringify({ month, scope: "company" }),
      });
      showSaveIndicator(`Reset ${res.count} records`, "saved");
      render();
    } catch (e) {
  ...
  root.querySelector("#bulk-actions-toggle")?.addEventListener("click", () => {
    const panel = root.querySelector("#bulk-actions-panel");
...
function openFpImportModal() {
  openModal(`
    <div class="modal-header"><h2>Import fingerprint attendance — ${monthLabel(state.month)}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function openFpRulesModal() {
  ...
  openModal(`
    <div class="modal-header"><h2>FP rules — ${monthLabel(state.month)}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function renderPayroll(root) {
  ...
  const view = activePayrollView(data);
  let payrollRows = applyPayrollRowFilters(view.rows || [], state.month);
  const tab = state.payrollTab || "agent";
  const totals = sumFilteredPayrollTotals(payrollRows);
  ...
  root.innerHTML = `
    ...
    <label class="field field-inline"><span>Team</span>
      <select id="payroll-team-filter" style="min-width:9rem">
        <option value="">All teams</option>
        ${(data.teams || []).map((t) => `<option value="${escapeHtml(t)}" ${state.payrollTeamFilter === t ? "selected" : ""}>${escapeHtml(t)}</option>`).join("")}
      </select>
    </label>
    <label class="field field-inline"><span>Unit</span>
      <select id="payroll-unit-filter" style="min-width:9rem">
        <option value="">All units</option>
        ${(data.units || []).map((u) => `<option value="${escapeHtml(u)}" ${state.payrollUnitFilter === u ? "selected" : ""}>${escapeHtml(u)}</option>`).join("")}
      </select>
    </label>
    <label class="field field-inline"><span>Status</span>
      <select id="payroll-status-filter" style="min-width:9rem">
        <option value="">All statuses</option>
        ${(data.payrollStatuses || []).map((s) => `<option value="${escapeHtml(s)}" ${state.payrollStatusFilter === s ? "selected" : ""}>${escapeHtml(s)}</option>`).join("")}
      </select>
    </label>
    ${hideOutToggle()}
    ...
    <div class="grid-4">
      ...
      <div class="card card-stat"><strong id="payroll-stat-transport">${fmt(totals.totalTransport || 0)}</strong><span class="muted">Transport</span></div>
    ...
    <div class="table-wrap"><table>
      <thead><tr><th>Employee</th><th>Status</th><th class="sortable" data-sort="salesCount">Sales</th><th class="sortable" data-sort="commissionAmount">Commission</th><th>${thDue}</th><th class="sortable" data-sort="totalWorkingDays">${thDays}</th><th class="text-right sortable" data-sort="basicSalary">Basic</th>
        <th class="text-right sortable" data-sort="transportAllowance">Transport</th><th class="text-right sortable" data-sort="loanDeductionTotal">Loan</th><th class="text-right sortable" data-sort="netSalary">Net</th><th></th></tr></thead>
  ...
  root.querySelector("#payroll-team-filter")?.addEventListener("change", (e) => {
    state.payrollTeamFilter = e.target.value;
    updatePayrollTable(root);
  });
  root.querySelector("#payroll-unit-filter")?.addEventListener("change", (e) => {
    state.payrollUnitFilter = e.target.value;
    updatePayrollTable(root);
  });
  root.querySelector("#payroll-status-filter")?.addEventListener("change", (e) => {
    state.payrollStatusFilter = e.target.value;
    updatePayrollTable(root);
  });
  root.querySelectorAll(".sortable").forEach((th) => {
    th.addEventListener("click", () => {
      const key = th.dataset.sort;
      if (!key) return;
      if (state.payrollSortKey === key) {
        state.payrollSortDir = state.payrollSortDir === "asc" ? "desc" : "asc";
      } else {
        state.payrollSortKey = key;
        state.payrollSortDir = "asc";
      }
      updatePayrollTable(root);
    });
  });
...
async function openEmployeeLoansModal(employeeId) {
  ...
  openModal(
    `<div class="modal-header"><h2>Loans — ${employeeId}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
function openLoanEditModal(employeeId, loan) {
  ...
  openModal(
    `<div class="modal-header"><h2>Edit loan</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
async function openLoansManagerModal() {
  ...
  openModal(
    `<div class="modal-header"><h2>Active loans (${active.length})</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
      <div class="table-wrap"><table>
...
async function openBonusEditModal(bonus, employees, bonusTypes) {
  ...
  const cleanReason = String(bonus.reason || "")
    ...
    .trim();
  openModal(
    `<div class="modal-header"><h2>Edit bonus</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
async function openDeductionEditModal(deduction, employees, deductionTypes) {
  const isTlBonus = deduction.type === "Bonus from TL / OP";
  const recipientId = deduction.bonusRecipientId || parseTlTargetFromReason(deduction.reason);
  const recipient = recipientId ? employees.find((e) => e.id === recipientId) : null;
  const recipientLabel = recipient
    ? `${recipient.id} — ${escapeHtml(recipient.american_name || recipient.arabic_name || recipient.id)}`
    : recipientId
      ? escapeHtml(recipientId)
      : "—";
  const tlBonusField = isTlBonus
    ? `<label class="field" style="grid-column:1/-1"><span>Bonus given to</span><input value="${recipientLabel}" readonly disabled /></label>`
    : "";
  openModal(
    `<div class="modal-header"><h2>Edit deduction</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
      <form id="deduction-edit-form" class="field-grid">
        <label class="field"><span>Employee</span><select name="employeeId" required ${isTlBonus ? "disabled" : ""}>${employeeSelectOptions(employees, deduction.employeeId)}</select></label>
        <label class="field"><span>Date</span><input name="date" type="date" value="${deduction.date}" required ${isTlBonus ? "readonly" : ""} /></label>
        <label class="field"><span>Type</span><select name="type" disabled>${deductionTypes.map((t) => `<option value="${t}" ${deduction.type === t ? "selected" : ""}>${t}</option>`).join("")}</select></label>
        <label class="field"><span>Amount (EGP)</span><input name="amount" type="number" min="0" step="0.01" value="${deduction.amount}" required ${isTlBonus ? "readonly" : ""} /></label>
        ${tlBonusField}
        <label class="field" style="grid-column:1/-1"><span>Reason</span><input name="reason" value="${escapeHtml(deduction.reason || "")}" ${isTlBonus ? "readonly" : ""} /></label>
      </form>
      <div class="btn-row" style="margin-top:1rem">
        <button class="btn" data-close>Cancel</button>
        <button class="btn btn-primary" id="save-ded-edit-btn" ${isTlBonus ? "disabled" : ""}>Save</button>
      </div>
    </div>`,
    true
  );
...
async function renderBonuses(root) {
  ...
  if (canSubmitBonusRequest() || canApproveBonusRequest()) {
    const bonusQs = state.companyContext === "hs2" ? "&company=hs2" : "";
    reqs.push(api(`/bonus-requests?month=${state.month}&status=pending${bonusQs}`).catch(() => ({ requests: [] })));
  }
  ...
  root.querySelector("#submit-bonus-req")?.addEventListener("click", () => {
    ...
    openModal(`
      <div class="modal-header"><h2>Request bonus for agent</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
  ...
  root.querySelectorAll("[data-approve-req]").forEach((btn) => {
    btn.onclick = async () => {
      const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
      await api(`/bonus-requests/${btn.dataset.approveReq}${qs}`, { method: "PATCH", body: JSON.stringify({ action: "approve" }) });
      render();
    };
  });
    ...
    btn.onclick = () => {
      openPromptModal({
        ...
        onSubmit: async (denyReason) => {
          const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
          await api(`/bonus-requests/${btn.dataset.denyReq}${qs}`, {
            method: "PATCH",
...
async function renderLoanApprovalsPage(root) {
  ...
  const [data, empData] = await Promise.all([
    api(`/loan-requests?status=pending${state.companyContext === "hs2" ? "&company=hs2" : ""}`),
    api(`/employees${employeesQuery()}`).catch(() => ({ employees: [] })),
  ]);
  ...
  root.querySelectorAll("[data-loan-approve]").forEach((btn) => {
    btn.onclick = async () => {
      ...
      try {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/loan-requests/${btn.dataset.loanApprove}/approve${qs}`, { method: "POST" });
    ...
    btn.onclick = () => {
      openPromptModal({
        ...
        onSubmit: async (reason) => {
          try {
            const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
            await api(`/loan-requests/${btn.dataset.loanDeny}/deny${qs}`, {
              method: "POST",
  ...
  document.getElementById("add-loan-submit-btn").onclick = async () => {
    ...
    try {
      ...
      if (autoApprove && created.request?.id) {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/loan-requests/${created.request.id}/approve${qs}`, { method: "POST" });
...
async function openEmployeeWarningsModal(employeeId) {
  ...
  openModal(
    `<div class="modal-header"><h2>HR notes — ${escapeHtml(employeeId)}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
  ...
  document.querySelectorAll("[data-edit-warn]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (!w) return;
      openModal(
        `<div class="modal-header"><h2>Edit note</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function openEmployeeQualityNotesModal(employeeId) {
  ...
  openModal(
    `<div class="modal-header"><h2>Quality notes — ${escapeHtml(employeeId)}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body modal-body-scroll">
  ...
  document.querySelectorAll("[data-edit-qnote]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!n) return;
      openModal(
        `<div class="modal-header"><h2>Edit quality note</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function renderSalaries(root) {
  ...
  root.querySelector("#add-rate-btn").onclick = () => {
    openModal(`
      <div class="modal-header"><h2>Add position</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
...
async function openEmployeeDocsModal(employeeId) {
  ...
  openModal(
    `<div class="modal-header"><h2>Documents — ${employeeId}</h2><button class="btn btn-sm" data-close><i data-lucide="x"></i></button></div>
    <div class="modal-body">
...
async function renderSettings(root) {
  ...
  if (canManagingUnits) {
    ...
    if (managersData) {
      ...
      const currentCompany = state.companyContext || "hangup";
      const isHs2Unit = (name) => /^(HS2|HS-2|PT)/i.test(String(name || "").trim());
      const orderedNames = Object.keys(unitRules)
        .filter((n) => units.some((u) => u.unit === n))
        .filter((n) => {
          const unitIsHs2 = isHs2Unit(n);
          return currentCompany === "hs2" ? unitIsHs2 : !unitIsHs2;
        });
      units.forEach((u) => {
        const unitIsHs2 = isHs2Unit(u.unit);
        const matchesCompany = currentCompany === "hs2" ? unitIsHs2 : !unitIsHs2;
        if (matchesCompany && !orderedNames.includes(u.unit)) orderedNames.push(u.unit);
      });
  ...
  root.querySelectorAll("[data-approve-reg]").forEach((btn) => {
    btn.addEventListener("click", () => {
      openConfirmModal({
        ...
        message: "Approve this registration? Creates employee + active login.",
...
async function renderDashboard(root) {
  ...
  const salesStats = salesDash
    ? `<div class="card" style="margin-top:1rem"><h3>Sales this month</h3>
        <p><strong>${salesDash.totals?.passed ?? 0}</strong> passed · <strong>${salesDash.totals?.pending ?? 0}</strong> pending · <strong>${salesDash.totals?.callback ?? 0}</strong> callback · <strong>${salesDash.totals?.denied ?? 0}</strong> denied
...
async function renderRulesPage(root) {
  ...
  let activeCompany = state.rulesActiveCompany || state.companyContext || "hangup";
  ...
  if (activeCompany === "hs2" && !isHs2ByUnit && !state.user?.canManageHs2Company) {
  ...
  root.innerHTML = `
    ...
    <style>
      ...
      .rules-table tr:nth-child(even) { background:var(--surface-2); }
      ...
      .rules-edit-textarea { width:100%; font-family:inherit; font-size:0.9rem; padding:0.5rem; border:1px solid var(--border); border-radius:6px; background:var(--surface-2); color:var(--text); }
...
async function navigate(page) {
  ...
  if (window.InterviewModule && typeof window.InterviewModule.stopSync === "function") {
    window.InterviewModule.stopSync();
  }
  if (window.TrainingModule && typeof window.TrainingModule.stopSync === "function") {
    window.TrainingModule.stopSync();
  }
...
const PAGE_TITLES = {
  dashboard: "Dashboard", employees: "Employees", attendance: "Attendance", payroll: "Payroll",
  bonuses: "Bonuses", deductions: "Deductions", loans: "Loans", salaries: "Salaries",
  reports: "Reports", sales: "Sales Log", breaks: "Breaks", costs: "Costs",
  "team-dashboard": "Team Dashboards", "loan-approvals": "Loan Approvals",
  "sales-permissions": "Sales Permissions", "sales-log-columns": "Log Columns",
  rules: "Rules", changes: "Changes", users: "Users", "access-control": "Access Control",
  "it-requests": "IT Requests", "meeting-requests": "Meeting Requests", requests: "Requests",
  equipment: "Equipment", org: "Organization", settings: "Settings",
  payslip: "My Payslip", "my-payslip": "My Payslip",
  announcements: "Announcements", coaching: "Coaching",
};
function updateTopBar(page) {
  const titleEl = document.getElementById("page-crumb-title");
  const subEl = document.getElementById("page-crumb-sub");
  if (titleEl) titleEl.textContent = PAGE_TITLES[page] || "Hangup Portal";
  if (subEl) subEl.textContent = state.user?.companyContext ? `Working · ${state.user.companyContext}` : "Hangup Portal";
  const gs = document.getElementById("global-search");
  if (gs && state.tabSearch.hasOwnProperty(page)) gs.value = state.tabSearch[page] || "";
}
...
async function render() {
  ...
  const labels = {
    ...
    rules: "Loading rules…",
    interview: "Loading interviews…",
    announcements: "Loading announcements…",
    coaching: "Loading coaching…",
  };
  if (!appReady) {
  ...
  try {
    ...
    else if (page === "analytics" && window.AnalyticsModule) {
      await window.AnalyticsModule.renderAnalyticsPage(root, api, state, { escapeHtml, openModal, closeModal, employeeSelectOptions });
    }
    ...
    } else if (page === "interview" && window.InterviewModule) {
      window.InterviewModule.init(root);
    }     else if (page === "training" && window.TrainingModule) {
      window.TrainingModule.init(root);
    } else if (page === "announcements" && window.AnnouncementsCoachingModule) {
      await window.AnnouncementsCoachingModule.renderAnnouncementsPage(root);
    } else if (page === "coaching" && window.AnnouncementsCoachingModule) {
      await window.AnnouncementsCoachingModule.renderCoachingPage(root);
    } else {
      renderErrorCard(
        root,
...
// ── Live sync (SSE ← Supabase Realtime) ────────────────────────────────────
// Opens a Server-Sent Events stream that the server populates from Supabase
// Realtime on attendance_events / payroll_adjustments changes. When another
// user edits a row, we patch our in-memory view immediately so every screen
// stays in sync (shared "Google Sheet" behaviour). Falls back silently to the
// existing polling path if SSE is unavailable.
let __liveEs = null;
function connectLiveSync() {
  if (__liveEs || typeof EventSource === "undefined") return;
  try {
    __liveEs = new EventSource("/api/live/stream");
  } catch {
    __liveEs = null;
    return;
  }
  __liveEs.addEventListener("live", (ev) => {
    let msg;
    try { msg = JSON.parse(ev.data); } catch { return; }
    if (!msg || !msg.type) return;
    const root = document.getElementById("app");
    if (msg.type === "attendance_update" && msg.change) {
      applyLiveAttendance(msg.change, root);
    } else if (msg.type === "payroll_update" && msg.change) {
      applyLivePayroll(msg.change, root);
    }
  });
  __liveEs.onerror = () => {
    // EventSource auto-reconnects; nothing to do. Avoid throwing.
  };
}

function applyLiveAttendance(change, root) {
  if (!change || !change.employeeId || !change.date) return;
  // Only apply if the attendance page is the active view and month matches.
  const ctx = root?.__attendanceCtx;
  if (state.page !== "attendance" || !ctx || !String(change.date).startsWith(state.month)) return;
  const key = `${change.employeeId}|${change.date}`;
  const existing = ctx.recordMap?.get(key) || ctx.data?.records?.find((r) => `${r.employeeId}|${r.date}` === key);
  const incomingStatus = String(change.status || "").trim();
  ...
  if (!incomingStatus && existingStatus) {
    ...
    const incomingMs = parseMs(change.updatedAt);
    const existingMs = parseMs(existing?.updatedAt);
    if (existingMs != null && (incomingMs == null || incomingMs <= existingMs)) return;
    if (existingMs == null) return;
  }
  const rec = { employeeId: change.employeeId, date: change.date, status: change.status || "", transportOverride: change.transportOverride || "", updatedAt: change.updatedAt || "" };
  if (ctx.recordMap) ctx.recordMap.set(key, { ...(ctx.recordMap.get(key) || {}), ...rec });
  if (ctx.data?.records) {
    const i = ctx.data.records.findIndex((r) => `${r.employeeId}|${r.date}` === key);
    if (i >= 0) ctx.data.records[i] = { ...ctx.data.records[i], ...rec };
    else ctx.data.records.push(rec);
  }
  updateAttendanceTable(root);
}

function applyLivePayroll(change, root) {
  if (!change || !change.employeeId || !change.yearMonth) return;
  if (state.page !== "payroll" || !root?.__payrollData) return;
  if (change.yearMonth !== state.month) return;
  const idx = root.__payrollData.payroll.findIndex((r) => r.employeeId === change.employeeId);
  if (idx < 0) return;
  const row = root.__payrollData.payroll[idx];
  if (change.monthlySalaryOverride !== undefined && change.monthlySalaryOverride !== null) row.monthlySalaryOverride = change.monthlySalaryOverride;
  if (change.netSalaryOverride !== undefined && change.netSalaryOverride !== null) row.netSalaryOverride = change.netSalaryOverride;
  if (change.payrollStatus) row.payrollStatus = change.payrollStatus;
  if (row.adj) {
    if (change.monthlySalaryOverride !== undefined && change.monthlySalaryOverride !== null) row.adj.monthlySalaryOverride = change.monthlySalaryOverride;
    if (change.netSalaryOverride !== undefined && change.netSalaryOverride !== null) row.adj.netSalaryOverride = change.netSalaryOverride;
    if (change.payrollStatus) row.adj.payrollStatus = change.payrollStatus;
  }
  updatePayrollTable(root);
}
...
async function boot() {
  ...
  window.hrAuthDebug?.log("app.boot.start", { sessionId: window.hrAuthDebug?.maskSessionId(sid) });
  if (!sid) {
    window.hrAuthDebug?.log("app.boot.redirect_login", { reason: "no_session_in_storage" });
    window.location.href = "/login";
    return;
  }
  const root = document.getElementById("app");
  ...
  restorePendingAttendance();
  let syncOk = false;
  let cacheWarm = false;
  try {
    const warm = await api("/sync/status", {}, 8000);
    cacheWarm = !!warm.warm;
    window.hrAuthDebug?.log("app.boot.sync_status", { warm: cacheWarm });
  } catch (e) {
    window.hrAuthDebug?.logError("app.boot.sync_status", e);
    /* proceed to sync */
  }
  try {
    if (cacheWarm) {
    ...
    window.hrAuthDebug?.logError("app.boot.sync", e);
    try {
  ...
  try {
    ...
    connectLiveSync();
    window.hrAuthDebug?.log("app.boot.complete");
  } catch (e) {
    renderErrorCard(
      root,
