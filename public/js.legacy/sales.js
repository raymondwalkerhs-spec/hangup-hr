...
window.SalesModule = (function () {
  ...
  function visibleSalesUnits() {
    // HS-2 is never shown in the unit toggle — it is scoped via the company switcher.
  ...
  function renderSaleListCell(colKey, sale, empById, escapeHtml, fmt) {
    ...
    switch (colKey) {
      ...
      case "priceTier": {
        const clientName = sale.client || fd.client || "";
        const deviceName = sale.device || fd.deviceType || "";
        const tierLabel = sale.priceTierLabel || fd.priceTierLabel || fd.priceTier || "";
        const parts = [clientName, deviceName, tierLabel].filter(Boolean);
  ...
  /** Keep day / week / month pickers aligned so the same sales rows (and actions) stay visible. */
  function syncSalesPeriodDates(state, newPeriod, oldPeriod) {
    const pick = state.salesPickDate || todayIso();
    const weekMon = state.salesWeekDate || mondayOf(pick);
    const month = state.month || pick.slice(0, 7);
    if (newPeriod === "month") {
      if (oldPeriod === "day") state.month = pick.slice(0, 7);
      else if (oldPeriod === "week") state.month = weekMon.slice(0, 7);
    } else if (newPeriod === "week") {
      if (oldPeriod === "day") state.salesWeekDate = mondayOf(pick);
      else if (oldPeriod === "month") state.salesWeekDate = mondayOf(`${month}-01`);
    } else if (newPeriod === "day") {
      if (oldPeriod === "week") state.salesPickDate = weekMon;
      else if (oldPeriod === "month") state.salesPickDate = `${month}-01`;
    }
  }
  ...
  function employeeSelectOptions(employees, escapeHtml, selectedId = "", filter = "all") {
    ...
    if (filter === "dialing") {
      ...
      list = list.filter((e) => {
        const role = String(e.role || "").toLowerCase();
        return role === "quality" || role === "rtm" || role === "admin" || role === "ceo";
      });
  ...
  async function renderSalesPage(root, api, state, helpers) {
    ...
    const thead = listColumns.map((c) => `<th>${escapeHtml(c.label || c.columnKey)}</th>`).join("") + '<th class="sales-actions-col">Actions</th>';
    const tbody = sales.length
      ? sales
          .map((s) => {
            ...
            if (canViewSale()) primaryActions.push(`<button type="button" class="btn-icon" data-sale-action="view" data-sale-id="${s.id}" title="View sale" aria-label="View sale"><i data-lucide="eye"></i></button>`);
            if (canFullEditSale()) primaryActions.push(`<button type="button" class="btn-icon" data-sale-action="edit" data-sale-id="${s.id}" title="Edit sale" aria-label="Edit sale"><i data-lucide="pencil"></i></button>`);
            if (canOpenQualityTicketForSale(s)) primaryActions.push(`<button type="button" class="btn-icon" data-sale-action="quality" data-sale-id="${s.id}" title="${canFullEditSale() ? 'Quality ticket' : 'Open ticket'}" aria-label="Quality ticket"><i data-lucide="${canFullEditSale() ? 'ticket' : 'message-square-plus'}"></i></button>`);
            if (canApprove() && s.status === "pending") {
              primaryActions.push(`<button type="button" class="btn-icon btn-icon-success" data-sale-action="approve" data-sale-id="${s.id}" title="Approve" aria-label="Approve"><i data-lucide="check"></i></button>`);
              primaryActions.push(`<button type="button" class="btn-icon btn-icon-danger" data-sale-action="deny" data-sale-id="${s.id}" title="Deny" aria-label="Deny"><i data-lucide="x"></i></button>`);
            }
            const kebabItems = [];
            if (canApprove()) kebabItems.push({ action: "callback", label: "Callback", icon: "phone-call" });
            if (canExportSalesList()) kebabItems.push({ action: "export", label: "Export", icon: "download" });
            if (canDeleteSales()) kebabItems.push({ action: "delete", label: "Delete", icon: "trash-2", danger: true });
            const kebab = kebabItems.length ? `<button type="button" class="btn-icon" data-sales-kebab data-sale-id="${s.id}" title="More actions" aria-label="More actions" aria-haspopup="true"><i data-lucide="more-vertical"></i></button>` : "";
            return `<tr>${cells}<td class="sales-actions-col"><div class="row-actions">${primaryActions.join("")}${kebab}</div></td></tr>`;
          })
          .join("")
      : (() => {
          const activeFilters = [];
          if (state.salesAgentFilter) activeFilters.push(`Agent: ${escapeHtml(state.salesAgentFilter)}`);
          if (state.salesCloserFilter) activeFilters.push(`Closer: ${escapeHtml(state.salesCloserFilter)}`);
          if (state.salesClientFilter) activeFilters.push(`Client: ${escapeHtml(state.salesClientFilter)}`);
          if (state.salesClientFeedbackFilter) activeFilters.push(`Client status: ${escapeHtml(state.salesClientFeedbackFilter)}`);
          if (state.salesVerifierFeedbackFilter) activeFilters.push(`Reviewer status: ${escapeHtml(state.salesVerifierFeedbackFilter)}`);
          if (state.salesHiddenUnits?.length) activeFilters.push(`Hidden units: ${state.salesHiddenUnits.map(escapeHtml).join(", ")}`);
          const hasAdvancedFilter = Boolean((state.salesAdvancedFilter || loadAdvancedFilter())?.rules?.length);
          if (hasAdvancedFilter) activeFilters.push("Advanced filter active");
          const filterHint = activeFilters.length
            ? `<div class="muted" style="margin-top:.5rem">Active filters: ${activeFilters.join(" · ")}</div>
               <button class="btn btn-sm" id="sales-clear-filters" style="margin-top:.75rem">Clear filters</button>`
            : "";
          return `<tr><td colspan="${colCount}" class="muted">No sales in this ${periodLabel.toLowerCase()} period${filterHint}</td></tr>`;
        })();

    const filtersActive = Boolean(
      state.salesAgentFilter || state.salesCloserFilter || state.salesClientFilter ||
      state.salesClientFeedbackFilter || state.salesVerifierFeedbackFilter ||
      (state.salesAdvancedFilter || loadAdvancedFilter())?.rules?.length
    );
    const salesFiltersPanelHtml = `
      <div class="sales-filters-bar">
        <button type="button" class="btn btn-sm" id="sales-filters-toggle" aria-expanded="false"><i data-lucide="sliders-horizontal"></i> Filters${filtersActive ? '<span class="filter-dot">●</span>' : ""}</button>
        <div class="sales-filters-pop card" id="sales-filters-pop" hidden>
          <div class="sales-filters-pop-inner">
            ${salesPersonFilters(employees, salesClients, escapeHtml, state)}
            ${feedbackFilterSelects(state, escapeHtml)}
    ...
    root.innerHTML = `
      ...
      ${salesFiltersPanelHtml}
      ${statGridHtml}
      <div class="table-wrap card sales-log-table"><table>
    ...
    root.querySelector("#sales-period")?.addEventListener("change", (e) => {
      ...
      const oldPeriod = state.salesPeriod || "month";
      state.salesPeriod = newPeriod;
      syncSalesPeriodDates(state, newPeriod, oldPeriod);
      // When switching to weekly, snap to Monday of the aligned week.
      if (newPeriod === "week") {
        state.salesWeekDate = mondayOf(state.salesWeekDate || state.salesPickDate || todayIso());
      }
      // Clear ALL active filters on period switch. Any filter set for a narrow
      // daily view (agent, closer, client, feedback, advanced, hidden units)
      // would otherwise silently return zero rows on week/month, hiding all
      // action buttons.
      state.salesClientFeedbackFilter = "";
      state.salesVerifierFeedbackFilter = "";
      state.salesAgentFilter = "";
      state.salesCloserFilter = "";
      state.salesClientFilter = "";
      state.salesAdvancedFilter = null;
      state.salesHiddenUnits = [];
      saveAdvancedFilter(null);
      rerender();
    });
    ...
    root.querySelector("#sales-clear-filters")?.addEventListener("click", () => {
      state.salesClientFeedbackFilter = "";
      state.salesVerifierFeedbackFilter = "";
      state.salesAgentFilter = "";
      state.salesCloserFilter = "";
    ...
    function replacePeriodWithSegmented() {
      const sel = root.querySelector("#sales-period");
      if (!sel) return;
      const wrap = document.createElement("div");
      wrap.className = "segmented";
      wrap.setAttribute("role", "tablist");
      wrap.setAttribute("aria-label", "Sales period");
      [["day", "Daily"], ["week", "Weekly"], ["month", "Monthly"]].forEach(([val, label]) => {
        const b = document.createElement("button");
        b.type = "button";
        b.textContent = label;
        b.dataset.period = val;
        if (val === (state.salesPeriod || "month")) b.classList.add("active");
        b.addEventListener("click", () => switchPeriod(val));
        wrap.appendChild(b);
      });
    ...
    function openCallbackModal(id) {
      ...
      document.getElementById("sale-callback-form").onsubmit = async (e) => {
        ...
        await api(`/sales/${id}`, {
          method: "PATCH",
          body: JSON.stringify({ action: "callback", feedback, callbackVisibleToAgent: fd.get("visible") === "on" }),
        });
        closeModal();
        rerender();
      };
    ...
    function runSaleAction(action, id) {
      const sale = sales.find((s) => s.id === id);
      if (!sale) return;
      switch (action) {
        case "view": openViewSaleModal(api, employees, helpers, sale); break;
        case "edit": openSaleModal(api, employees, helpers, sale, rerender); break;
        case "quality": openQualityTicketModal(api, employees, helpers, sale, rerender); break;
        case "approve":
          api(`/sales/${id}`, { method: "PATCH", body: JSON.stringify({ action: "approve" }) }).then(rerender);
          break;
        case "deny":
          openPromptModal({
            title: "Deny sale",
            ...
            onSubmit: async (feedback) => {
              await api(`/sales/${id}`, { method: "PATCH", body: JSON.stringify({ action: "deny", feedback }) });
              rerender();
            },
          });
          break;
        case "callback": openCallbackModal(id); break;
        case "export": runSalesExport({ saleId: id }); break;
        case "delete": deleteSaleFromList(sale, api, helpers, rerender); break;
      }
    }

    root.querySelectorAll("[data-sale-action]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        runSaleAction(btn.dataset.saleAction, btn.dataset.saleId);
      });
    });
    root.querySelectorAll("[data-sales-kebab]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (!sale) return;
        const items = [];
        if (canOpenQualityTicketForSale(sale)) items.push({ action: "quality", label: canFullEditSale() ? "Quality ticket" : "Open ticket", icon: canFullEditSale() ? "ticket" : "message-square-plus" });
        if (canApprove()) items.push({ action: "callback", label: "Callback", icon: "phone-call" });
        if (canExportSalesList()) items.push({ action: "export", label: "Export", icon: "download" });
        if (canDeleteSales()) items.push({ action: "delete", label: "Delete", icon: "trash-2", danger: true });
        if (!items.length) return;
        const html = items
          .map((it) => `<button type="button" class="menu-item${it.danger ? " danger" : ""}" data-sale-action="${it.action}" data-sale-id="${id}"><i data-lucide="${it.icon}"></i>${escapeHtml(it.label)}</button>`)
          .join("");
        const pop = openPopover(btn, { html, align: "right", variant: "menu" });
        pop.querySelectorAll("[data-sale-action]").forEach((mi) => {
          mi.addEventListener("click", () => {
            runSaleAction(mi.dataset.saleAction, mi.dataset.saleId);
            closeActivePopover();
          });
        });
        refreshIcons(pop);
    ...
    root.querySelector("#sales-filters-toggle")?.addEventListener("click", (e) => {
      const pop = root.querySelector("#sales-filters-pop");
      if (!pop) return;
    ...
    replacePeriodWithSegmented();
    refreshIcons(root);
    enhanceSelectsWithin(document.getElementById("sales-filters-pop"));
  }
