...
window.AccessControlModule = (function () {
  ...
  function rbacQuery() {
    return typeof state !== "undefined" && state.companyContext === "hs2" ? "?company=hs2" : "";
  }
  ...
  async function loadData(api) {
    const qs = rbacQuery();
    const [cat, ov] = await Promise.all([api("/rbac/catalog"), api(`/rbac/overrides${qs}`)]);
    catalog = cat;
  ...
  async function renderAccessControlPage(root, api, helpers) {
    ...
    root.querySelector("#rbac-save")?.addEventListener("click", async () => {
      ...
      try {
        await api(`/rbac/overrides${rbacQuery()}`, { method: "PUT", body: JSON.stringify({ entries }) });
        pending.clear();
        await loadData(api);
        renderMatrix(root);
        updateSaveState(root);
    ...
    root.querySelector("#rbac-reset-role")?.addEventListener("click", () => {
      openConfirmModal({
        ...
        danger: true,
        onConfirm: async () => {
          await api(`/rbac/reset${rbacQuery()}`, { method: "POST", body: JSON.stringify({ role: selectedRole }) });
          pending.clear();
          await loadData(api);
          renderMatrix(root);
          updateSaveState(root);
