...
window.RequestsModule = (function () {
  ...
  async function renderRequestsPage(root, api, state, helpers) {
    ...
    const leaveQs = state.companyContext === "hs2" ? "?company=hs2" : "";
    const [data, empData] = await Promise.all([
      api(`/hrms/leave${leaveQs}`),
    ...
    const reqFrequency = state.requestFrequency || "daily";
    ...
    const activeEmployees = isHrRole()
      ? employees.filter((e) => {
    ...
    const filterTeam = state.requestFilter?.team || "";
    const filterUnit = state.requestFilter?.unit || "";
    const filterType = state.requestFilter?.leaveType || "";
    const filterStatus = state.requestFilter?.status || "";

    if (filterTeam) {
      const teamIds = new Set(employees.filter((e) => e.team === filterTeam).map((e) => e.id));
      requests = requests.filter((r) => teamIds.has(r.employeeId));
    }
    if (filterUnit) {
      requests = requests.filter((r) => (empById(r.employeeId).unit || "") === filterUnit);
    }
    if (filterType) {
      requests = requests.filter((r) => (r.requestKind || r.leaveType || "") === filterType);
    }
    if (filterStatus) {
      requests = requests.filter((r) => (r.status || "") === filterStatus);
    }

    const grouped = groupRequestsByFrequency(requests, reqFrequency);

    function groupRequestsByFrequency(reqs, freq) {
      ...
      return [...map.entries()].sort((a, b) => b[0].localeCompare(a[0])).map(([label, items]) => ({ label, items }));
    }
    ...
    root.innerHTML = `
      ...
      ${state.user?.canViewRequestFilters ? `<div class="toolbar">
        <select id="req-filter-team"><option value="">All teams</option>${(data.teams || []).map((t) => `<option value="${escapeHtml(t)}" ${filterTeam === t ? "selected" : ""}>${escapeHtml(t)}</option>`).join("")}</select>
        <select id="req-filter-unit"><option value="">All units</option>${(data.units || []).map((u) => `<option value="${escapeHtml(u)}" ${filterUnit === u ? "selected" : ""}>${escapeHtml(u)}</option>`).join("")}</select>
        <select id="req-filter-type"><option value="">All types</option>${(data.leaveTypes || []).map((t) => `<option value="${escapeHtml(t)}" ${filterType === t ? "selected" : ""}>${escapeHtml(t)}</option>`).join("")}</select>
        <select id="req-filter-status"><option value="">All statuses</option>${(data.statuses || []).map((s) => `<option value="${escapeHtml(s)}" ${filterStatus === s ? "selected" : ""}>${escapeHtml(s)}</option>`).join("")}</select>
        <select id="req-frequency">
          <option value="daily" ${reqFrequency === "daily" ? "selected" : ""}>Daily</option>
          <option value="weekly" ${reqFrequency === "weekly" ? "selected" : ""}>Weekly</option>
          <option value="monthly" ${reqFrequency === "monthly" ? "selected" : ""}>Monthly</option>
        </select>
      </div>` : ""}
      ${grouped.map((group) => `
        ${group.label !== "All requests" ? `<h3 style="margin:1rem 0 .5rem">${escapeHtml(group.label)}</h3>` : ""}
        <div class="table-wrap card" style="margin-bottom:1rem"><table>
          <thead><tr>
            <th>Employee</th><th>Dates</th><th>Type</th><th>Status</th><th>Notes</th><th></th>
          </tr></thead>
          <tbody>${group.items.length ? group.items.map((r) => {
            const isMedical = ["medical", "exam", "same_day"].includes(r.requestKind || r.leaveType);
            const submitterEmp = r.requestedBy && r.requestedBy !== r.employeeId ? empById(r.requestedBy) : null;
            return `<tr>
            <td>${empName(r.employeeId)}${submitterEmp ? `<br><span class="badge badge-status" title="Submitted by">${escapeHtml(submitterEmp.american_name || submitterEmp.id)}</span>` : ""}<br><span class="muted">${escapeHtml(r.employeeId)}</span></td>
    ...
    root.querySelector("#new-request-btn").onclick = () =>
      openRequestModal({ api, employees: activeEmployees, selfId, openModal, closeModal, employeeSelectOptions, escapeHtml, onDone: refresh });

    root.querySelectorAll("[data-approve]").forEach((b) => {
      b.onclick = async () => {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/leave/${b.dataset.approve}${qs}`, { method: "PUT", body: JSON.stringify({ status: "approved" }) });
        refresh();
      };
    });
    root.querySelectorAll("[data-reject]").forEach((b) => {
      b.onclick = async () => {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/leave/${b.dataset.reject}${qs}`, { method: "PUT", body: JSON.stringify({ status: "rejected" }) });
        refresh();
      };
    });
    root.querySelectorAll("[data-delete]").forEach((b) => {
      b.onclick = async () => {
        ...
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/leave/${b.dataset.delete}${qs}`, { method: "DELETE" });
        refresh();
      };
    });
    root.querySelectorAll("[data-edit]").forEach((b) => {
      b.onclick = () => {
        ...
        if (r) openRequestModal({ api, employees: activeEmployees, selfId, request: r, openModal, closeModal, employeeSelectOptions, escapeHtml, onDone: refresh });
      };
    });
    ...
    if (state.user?.canViewRequestFilters) {
      const bindReqFilter = (id, key) => {
        const el = root.querySelector(id);
        if (!el) return;
        el.value = state.requestFilter[key] || "";
        el.addEventListener("change", () => {
          state.requestFilter[key] = el.value;
          renderRequestsPage(root, api, state, helpers);
        });
      };
      bindReqFilter("#req-filter-team", "team");
      bindReqFilter("#req-filter-unit", "unit");
      bindReqFilter("#req-filter-type", "leaveType");
      bindReqFilter("#req-filter-status", "status");
      root.querySelector("#req-frequency")?.addEventListener("change", (e) => {
        state.requestFrequency = e.target.value;
        renderRequestsPage(root, api, state, helpers);
      });
    }
  }
  ...
  function openRequestModal({ api, employees, selfId, request, openModal, closeModal, employeeSelectOptions, escapeHtml, onDone }) {
    ...
    const teamList  = hrRole
      ? [...new Set(agentList.map((e) => e.team).filter(Boolean))].sort()
      : [];
    const defaultEmpId = request?.employeeId || selfId || (agentList[0]?.id || "");
    const defaultTeam  = request?.employeeId
      ? (agentList.find((e) => e.id === request.employeeId)?.team || "")
      : "";
    ...
    // HR/admin/ceo can always choose annual; everyone else when target agent has 180+ days
    ...
    openModal(`
      ...
      <form id="request-form" class="modal-body field-grid modal-body-scroll">
        ${hrRole && teamList.length > 0 ? `
        <label class="field"><span>Team</span>
          <select name="teamFilter" id="req-team-filter">
            <option value="">All teams</option>
            ${teamList.map((t) => `<option value="${escapeHtml(t)}" ${defaultTeam === t ? "selected" : ""}>${escapeHtml(t)}</option>`).join("")}
          </select>
        </label>` : ""}
    ...
    if (hrRole && teamList.length > 0) {
      const teamFilterEl = document.getElementById("req-team-filter");
      const empSelectEl  = document.getElementById("req-emp");
      function refreshEmployees(team) {
        const filtered = team ? agentList.filter((e) => e.team === team) : [];
        empSelectEl.innerHTML = filtered.length
          ? employeeSelectOptions(filtered, defaultEmpId)
          : `<option value="">No agents in this team</option>`;
      }
      if (defaultTeam) {
        refreshEmployees(defaultTeam);
      } else {
        empSelectEl.innerHTML = `<option value="">Select a team first</option>`;
      }
      teamFilterEl?.addEventListener("change", () => {
        refreshEmployees(teamFilterEl.value || "");
      });
    }
    ...
    document.getElementById("submit-request").onclick = async () => {
      ...
      try {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        if (isEdit) {
          await api(`/hrms/leave/${request.id}${qs}`, { method: "PUT",  body: JSON.stringify(body) });
        } else {
          await api(`/hrms/leave${qs}`, { method: "POST", body: JSON.stringify(body) });
        }
        closeModal();
        onDone();
      } catch (e) {
  ...
  function openLeaveDocsModal({ leaveId, employeeId, requestKind, api, openModal, closeModal, escapeHtml }) {
    ...
    async function loadDocs() {
      ...
      try {
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        const data = await api(`/hrms/leave/${leaveId}/documents${qs}`);
    ...
    document.getElementById("leave-doc-upload-btn").onclick = async () => {
      ...
      try {
        ...
        const docQs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/leave/${leaveId}/documents${docQs}`, {
          method: "POST",
          body: JSON.stringify({
            fileName: file.name,
            contentBase64,
