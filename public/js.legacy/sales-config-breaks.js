...
window.HRSalesConfigBreaks = (function () {
  ...
  function visibleBreakUnits(state) {
    ...
    if (ctx === "hs2") return ["HS2"];
    return ["HS1", "HS3", "HS4", "HS5", "HS-Back-End", "Management"];
  }
  ...
  async function loadCatalog(api, force, company) {
    const effectiveCompany = company || settingsCompany || "hangup";
    const rev = catalogCache.revision || 0;
    const data = await api(`/sales-config/catalog?company=${encodeURIComponent(effectiveCompany)}${force ? "&activeOnly=false" : ""}`);
    if (!force && data.revision === rev && catalogCache.clients.length && catalogCache.company === effectiveCompany) return catalogCache;
    catalogCache = { clients: data.clients || [], revision: data.revision || 0, company: effectiveCompany };
    return catalogCache;
  }
  ...
  function onSettingsRevision(api, revision, state) {
    ...
    if (revision !== prev) {
      ...
      catalogCache = { clients: [], revision: 0, company: "" };
      breaksCache.revision = 0;
      loadCatalog(api, true, settingsCompany).catch(() => {});
  ...
  async function enhanceSaleModal(api, helpers, sale, employees, formRoot, canEditFn, openSaleAttachment, modalOpts = {}) {
    ...
    const catalogCompany = state.companyContext === "hs2" ? "hs2" : "hangup";
    const catalog = await loadCatalog(api, false, catalogCompany).catch(() => ({ clients: [] }));
  ...
  async function enhanceSettings(root, api, state, helpers) {
    ...
    settingsCompany = state.companyContext === "hs2" ? "hs2" : "hangup";
    const wrap = document.createElement("div");
    wrap.className = "grid-2";
    wrap.style.gridColumn = "1 / -1";
    const companyToggleHtml = state.user?.canManageHs2Company ? `
      <div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.75rem">
        <span class="muted">Company:</span>
        <button type="button" class="btn btn-sm ${settingsCompany === "hangup" ? "btn-primary" : ""}" id="sc-company-hangup">Main Hangup</button>
        <button type="button" class="btn btn-sm ${settingsCompany === "hs2" ? "btn-primary" : ""}" id="sc-company-hs2">HS-2</button>
      </div>` : "";
    wrap.innerHTML = `
    ...
    // Wire company toggle buttons
    wrap.querySelector("#sc-company-hangup")?.addEventListener("click", () => {
      settingsCompany = "hangup";
      catalogCache = { clients: [], revision: 0, company: "" };
      enhanceSettings(root, api, state, helpers);
    });
    wrap.querySelector("#sc-company-hs2")?.addEventListener("click", () => {
      settingsCompany = "hs2";
      catalogCache = { clients: [], revision: 0, company: "" };
      enhanceSettings(root, api, state, helpers);
    });
    ...
    async function refreshClients() {
      const data = await api(`/sales-config/clients?company=${encodeURIComponent(settingsCompany)}`);
    ...
    function openClientModal(client) {
      ...
      document.getElementById("sc-client-form").onsubmit = async (e) => {
        e.preventDefault();
        const fd = new FormData(e.target);
        const body = { name: fd.get("name"), status: fd.get("status"), statusMessage: fd.get("statusMessage"), company: settingsCompany };
