...
window.HRMSFeatures = (function () {
  ...
  async function initNotificationsBell(api, state) {
    ...
    function attachBell(mount) {
      ...
      btn.innerHTML = '<i data-lucide="bell"></i> <span class="notif-count hidden hr-notif-count">0</span>';
  ...
  async function renderOrgPage(root, api, helpers = {}) {
    ...
    const [structure, empData, teamsRes, mgrRes] = await Promise.all([
      ...
      api("/hrms/teams").catch(() => ({ teams: [], orgUnits: [] })),
      api("/org/managers").catch(() => ({ managers: [] })),
    ]);
    const employees = empData.employees || [];
    const orgUnits = teamsRes.orgUnits || structure.orgUnits || [];
    ...
    const companyCtx = typeof require === "function" ? require("../lib/company-context") : { isHs2Unit: (u) => /^(HS2|HS-2|PT)/i.test(String(u || "").trim()) };
    const unitSections = (structure.units || [])
      .filter((section) => {
        if (typeof state !== "undefined" && state.user?.canManageHs2Company) return true;
        if (section.unit === (state?.user?.unit || "")) return true;
      ...
      return `<section class="card org-unit-block org-hierarchy-unit" style="margin-bottom:1rem" data-unit="${esc(unit)}">
        <div class="flex-between" style="margin-bottom:.75rem;align-items:flex-start;gap:1rem;flex-wrap:wrap">
          <div><h2 style="margin:0">${esc(unit)}${typeof state !== "undefined" && state.companyContext === "hs2" && companyCtx?.isHs2Unit?.(unit) ? ' <span class="badge">HS2 Company</span>' : ""}</h2>${opHeader}</div>
    ...
    if (canViewPin || canApproveReg) {
      ...
      if (canViewPin) {
        try {
          ...
          const activePin = isHs2 ? (pinRes.hs2 || {}) : (pinRes.hangup || {});
          const companyLabel = isHs2 ? "HS-2" : "Main Hangup";
          pinBlock = `<div class="card card-flat" style="padding:.75rem 1rem;margin-bottom:.75rem;background:var(--surface-2)">
            <strong>Today's agent registration PIN${isHs2 ? "" : "s"}</strong>
            <span class="muted" style="margin-left:.5rem">(${esc(activePin.date || "today")}) — share with new ${esc(companyLabel)} agents only</span>
            <div style="display:flex;gap:2rem;margin-top:.5rem;flex-wrap:wrap">
              <div><strong>${esc(companyLabel)}:</strong>
                <span style="font-size:1.35rem;letter-spacing:.2em;margin-left:.5rem">${esc(activePin.pin || "—")}</span>
              </div>
            </div>
          </div>`;
        } catch (_) {
        ...
        try {
          ...
          pendingBlock = pending.length
            ? `<section class="card" style="margin-bottom:1rem"><h3>Pending agent registrations (${pending.length})</h3>
              <div class="table-wrap"><table><thead><tr><th>Company</th><th>Full name</th><th>American name</th><th>Nationality</th><th>ID / Passport</th><th>Unit</th><th>Phone</th><th>Submitted</th><th></th></tr></thead>
              <tbody>${pending.map((p) => `<tr>
                <td>${esc(p.company === "hs2" ? "HS-2" : "Main Hangup")}</td>
      ...
      btn.onclick = () => {
        ...
        openModal(
          ...
          <form id="approve-reg-form" class="modal-body field-grid">
            <p class="muted" style="grid-column:1/-1">Creates employee + active login. Optionally assign a team now.</p>
  ...
  async function renderEquipmentPage(root, api, helpers) {
    ...
    if (!inventory) {
      ...
      const data = await api(`/hrms/equipment/${encodeURIComponent(selfId)}${state.companyContext === "hs2" ? "?company=hs2" : ""}`);
    ...
    const equipQs = state.companyContext === "hs2" ? "?company=hs2" : "";
    const [data, empData] = await Promise.all([
      api(`/hrms/equipment${equipQs}`),
      api(`/employees${typeof employeesQuery === "function" ? employeesQuery() : ""}`).catch(() => ({ employees: [] })),
    ]);
    ...
    const equipFilterUnit = state.equipFilter?.unit || "";
    const equipFilterTeam = state.equipFilter?.team || "";
    const equipFilterPosition = state.equipFilter?.position || "";
    const equipFilterDevice = state.equipFilter?.deviceType || "";
    if (equipFilterUnit || equipFilterTeam || equipFilterPosition || equipFilterDevice) {
      activeRows = activeRows.filter(({ assignment: a, equipment: e }) => {
        const emp = empById(a.employeeId);
        if (equipFilterUnit && (emp?.unit || e.unit || "") !== equipFilterUnit) return false;
        if (equipFilterTeam && emp?.team !== equipFilterTeam) return false;
        if (equipFilterPosition && emp?.position !== equipFilterPosition) return false;
        if (equipFilterDevice && e.itemType !== equipFilterDevice) return false;
        return true;
      });
    }
    ...
    root.innerHTML = `
      ...
      <div class="toolbar equip-toolbar">
        ...
        <select id="equip-filter-unit"><option value="">All units</option>${(data.units || []).map((u) => `<option value="${escapeHtml(u)}" ${equipFilterUnit === u ? "selected" : ""}>${escapeHtml(u)}</option>`).join("")}</select>
        <select id="equip-filter-team"><option value="">All teams</option>${(data.teams || []).map((t) => `<option value="${escapeHtml(t)}" ${equipFilterTeam === t ? "selected" : ""}>${escapeHtml(t)}</option>`).join("")}</select>
        <select id="equip-filter-position"><option value="">All positions</option>${(data.positions || []).map((p) => `<option value="${escapeHtml(p)}" ${equipFilterPosition === p ? "selected" : ""}>${escapeHtml(p)}</option>`).join("")}</select>
        <select id="equip-filter-device"><option value="">All devices</option>${(data.deviceTypes || []).map((d) => `<option value="${escapeHtml(d)}" ${equipFilterDevice === d ? "selected" : ""}>${escapeHtml(d)}</option>`).join("")}</select>
    ...
    state.equipFilter = state.equipFilter || {};
    const bindEquipFilter = (id, key) => {
      const el = root.querySelector(id);
      if (!el) return;
      el.value = state.equipFilter[key] || "";
      el.addEventListener("change", () => {
        state.equipFilter[key] = el.value;
        renderEquipmentPage(root, api, helpers);
      });
    };
    bindEquipFilter("#equip-filter-unit", "unit");
    bindEquipFilter("#equip-filter-team", "team");
    bindEquipFilter("#equip-filter-position", "position");
    bindEquipFilter("#equip-filter-device", "deviceType");
    ...
    root.querySelector("#add-equipment-btn").onclick = () => {
      ...
      document.getElementById("save-equip").onclick = async () => {
        ...
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/equipment${qs}`, { method: "POST", body: JSON.stringify(body) });
        closeModal();
        renderEquipmentPage(root, api, helpers);
      };
    };
      ...
      b.onclick = () => {
        ...
        document.getElementById("save-edit-equip").onclick = async () => {
          ...
          try {
            ...
            const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
            await api(`/hrms/equipment/${item.id}${qs}`, {
              method: "PATCH",
              body: JSON.stringify({
    ...
    root.querySelectorAll("[data-return]").forEach((b) => {
      b.onclick = async () => {
        ...
        const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
        await api(`/hrms/equipment/return/${b.dataset.return}${qs}`, { method: "POST" });
        renderEquipmentPage(root, api, helpers);
      };
    });
  }
  ...
  async function enhanceDashboard(root, api) {
    try {
      const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
      const { expiring } = await api(`/documents/expiring${qs}`);
      if (expiring?.length) {
    ...
    try {
      const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
      const { alerts } = await api(`/hrms/alerts/employment${qs}&days=60`);
      if (alerts?.length) {
        const card = document.createElement("div");
        card.className = "card";
  ...
  async function enhancePayroll(root, api, state, helpers) {
    ...
    if (header && !document.getElementById("payroll-lock-btn")) {
      ...
      compareBtn.onclick = async () => {
        const companyQs = state.companyContext === "hs2" ? "&company=hs2" : "";
      const report = await api(`/hrms/reports/payroll-compare?month=${state.month}${companyQs}`);
        alert(
    ...
    if (canManageCompanies) {
      ...
      async function loadCompaniesList() {
        ...
        try {
          ...
          // Per-company access control — removed from settings; use Access Control in sidebar
        } catch (e) {
  ...
  async function enhanceReports(root, api, state, helpers) {
    ...
    turnoverBtn.onclick = async () => {
      const qs = state.companyContext === "hs2" ? "?company=hs2" : "";
      const r = await api(`/hrms/reports/turnover${qs}`);
    ...
    rankBtn.onclick = () => {
      const qs = state.companyContext === "hs2" ? "?company=hs2&" : "?";
      api(`/hrms/reports/attendance-rankings${qs}month=${state.month}`).then((r) => {
