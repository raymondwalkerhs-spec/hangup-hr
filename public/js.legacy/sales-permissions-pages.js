...
window.SalesPermissionsPages = (function () {
  ...
  function renderAttachmentMatrix(root, escapeHtml, api) {
    const roleTitle = root.querySelector("#sf-selected-role-title");
    ...
    for (const a of attachPerms) {
      ...
      rows += `
        <tr>
          ...
          <td>${unsaved ? '<span class="badge">unsaved</span>' : '<span class="badge badge-muted">saved</span>'} <button type="button" class="btn btn-sm btn-link" data-reset-attach="${escapeHtml(a.attachmentKey)}">Reset</button></td>
        </tr>`;
    }
    ...
    root.querySelectorAll("[data-attach-toggle]").forEach((input) => {
      input.addEventListener("change", () => {
        ...
        renderAttachmentMatrix(root, escapeHtml, api);
        updateSaveState(root);
      });
    });

    root.querySelectorAll("[data-reset-attach]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const attachKey = btn.dataset.resetAttach;
        if (!attachKey) return;
        if (!confirm(`Reset permissions for ${attachKey} to defaults?`)) return;
        btn.disabled = true;
        try {
          await api(`/sales/attachment-permissions/${encodeURIComponent(attachKey)}`, { method: "DELETE" });
          await loadFieldData(api);
          renderAttachmentMatrix(root, escapeHtml, api);
          updateSaveState(root);
        } catch (e) {
          alert(e.message || "Could not reset attachment permissions");
        } finally {
          btn.disabled = false;
        }
      });
  ...
  function renderActivePanel(root, escapeHtml, api) {
    ...
    if (activeTab === "actions") {
      ...
      if (activeTab === "attachments") renderAttachmentMatrix(root, escapeHtml, api);
  ...
  async function renderSalesFieldPermissionsPage(root, api, helpers) {
    ...
    renderActivePanel(root, escapeHtml, api);
