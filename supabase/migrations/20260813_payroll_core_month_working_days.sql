-- Fix calculate_payroll_core daily/basic/transport denominators.
-- BUG: daily_rate was monthly_salary / attendance.working_days (e.g. 12000/10 = 1200).
-- Correct: daily_rate = monthly_salary / month_working_days (Salaries/config, weekday fallback).
-- Also align working_days units with app summarizeEmployeeMonth and fix transport divisor.
...
-- Optional 3rd arg: month working days from app config (workingDaysByMonth).
-- When NULL, fall back to Mon–Fri count in the month (matches app calendar fallback).
...
BEGIN
  ...
  SELECT
    ...
    -- App: (workingDays − unpaidHalf×0.5 − unpaidQuarter×0.25 − nsnc×2 − nsncHalf×1.5) × dailyRate
    CASE WHEN v_month_wd > 0 AND ed.monthly_salary > 0
      THEN round(
        (
  ...
  LEFT JOIN (
    SELECT
      v.employee_id,
      -- Match app summarizeEmployeeMonth.workingDays (no unpaid Day-OFF)
      COUNT(*) FILTER (
      ...
      COUNT(*) FILTER (WHERE v.status IN ('NSNC', 'Not Approved day off')) AS nsnc,
...
COMMENT ON FUNCTION calculate_payroll_core(text, text, integer) IS
  'Core payroll: daily_rate = monthly / month_working_days (p_working_days_in_month or weekday count). Attendance units match app summarizeEmployeeMonth.';
