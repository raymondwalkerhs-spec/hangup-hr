...
async function main() {
  ...
  const { data, error } = await sb.from('sale_attachments').select('id, sale_id, kind, file_name, created_at').limit(5);
  if (error) {
  ...
  // Count by date
  const { data: countData, error: countErr } = await sb.from('sale_attachments').select('created_at');
  if (countErr) { console.error('Count error:', countErr.message); return; }
  
  const before28 = (countData || []).filter(a => {
    const d = String(a.created_at || '').slice(0, 10);
    return d < '2026-06-28';
  });
  const onOrAfter28 = (countData || []).filter(a => {
    const d = String(a.created_at || '').slice(0, 10);
    return d >= '2026-06-28';
  });
  console.log(`\nTotal attachments: ${countData?.length || 0}`);
