# Build Hangup-Portal-Portable-Downloader.exe - small GUI downloader that
# pulls the latest *Portable*.exe asset from the GitHub release.
...
$srcFile = Join-Path $srcDir "PortableDownloader.cs"
New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
$workDir = Join-Path $OutputDir "portable-downloader-build"
New-Item -ItemType Directory -Force -Path $workDir | Out-Null
$workCs = Join-Path $workDir "PortableDownloader.cs"
...
$outExe = Join-Path $OutputDir "Hangup-Portal-Portable-Downloader.exe"

$pinLabel = if ($pinVersion) { $pinVersion } else { "latest" }
Write-Host "Compiling Portable downloader (repo: $repo, pin: $pinLabel)..." -ForegroundColor Cyan
