 ...
 * Only fills empty form_data fields (non-destructive).
 *
 * Usage:
 *   node scripts/backfill-sales-payment-from-csv.js --dry-run
 *   node scripts/backfill-sales-payment-from-csv.js
 */
...
function mapPaymentMethod(raw) {
  ...
  if (s.includes("bank")) return "Bank account";
  return "";
}
...
function parseBankFromNotes(notes) {
  ...
  const out = {};
  const routing = text.match(/routing\s*#?\s*:?\s*(\d{6,9})/i);
...
function rowToPayment(headers, row) {
  ...
  const paymentMethod = mapPaymentMethod(get("Payment method")) || mapPaymentMethod(get("Notes"));
  const out = {
    paymentMethod,
...
async function main() {
  ...
  for (const sale of sales || []) {
    ...
    for (const field of PAYMENT_FIELDS) {
      if (payment[field] && !fd[field]) {
        fd[field] = payment[field];
        changed = true;
      }
    }
    if (!changed) {
    ...
    updated++;
    log.push(`${sale.id} ${key} +${Object.keys(payment).filter((k) => fd[k] === payment[k]).join(",")}`);
    if (!DRY_RUN) {
  ...
  const summary = [
    `matched=${matched} updated=${updated} unchanged=${skipped} dryRun=${DRY_RUN}`,
    ...log,
  ].join("\n");
