# LEGACY — Google Sheets (DO NOT USE)

> **STRICT RULE — NEVER USE GOOGLE SHEETS IN THIS PROJECT**
>
> Production Hangup HR is **Supabase-only** (`DATA_BACKEND=supabase`). All HR data lives in
> Postgres (`supabase/migrations/`). The runtime app **must not** call `sheets.googleapis.com`
> or load Sheets code paths.
>
> This file is **historical reference only** — sheet IDs, tab layout, and one-time migration scripts.
> Legacy code lives under [`scripts/legacy/`](scripts/legacy/) and is **not** loaded at app startup.
>
> **Current schema:** `supabase/migrations/*.sql` · **Architecture:** [`README.md`](README.md)

---

# HR Data Sheet Schema (archived layout)

| Property | Value |
|----------|-------|
| **Spreadsheet title** | HR System Final |
| **Spreadsheet ID** | `17z8JrLV0_4fSXzsiZRpCZWFJk5FTit3IUkw0c3NOkvU` |
| **Access** | Service account (Editor) — `credentials/service-account.json` |
| **Code mapping** | `scripts/legacy/lib/sheets.js` (archived — not loaded at runtime) |

### Legacy sync flow

```
Google Sheet (tabs below)
  ...
  SQLite cache (.cache/hr-cache.db or portable HangupHR-data/hr-cache/)
        │
        ▼
  Express API → Desktop UI
```

- **First login:** full import into SQLite.
