...
function filterSales(list, filters = {}) {
  let rows = list;
  if (filters.from || filters.to) {
    rows = rows.filter((s) => saleInDateRange(s, filters.from, filters.to));
  }
...
async function createSale(payload, actor) {
  ...
  const row = {
    ...
    submission_date: payload.submissionDate || new Date().toISOString().slice(0, 10),
    effective_date: payload.effectiveDate || payload.submissionDate || new Date().toISOString().slice(0, 10),
...
async function reconcilePettyCashExpenseAmount({ expenseId, oldAmount, newAmount, vendorName }) {
  ...
  return recalculatePettyCashFundBalances(ledgerRow.fund_id);
}
