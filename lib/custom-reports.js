...
async function runReport(report, store, month, company = "hangup") {
  ...
  if (report.reportType === "employees") {
    let employees = applyEmployeeFilters(store.getEmployees(), filters);
  ...
  if (report.reportType === "payroll") {
    ...
    const extras = store.getPayrollExtras(ym, co);
    const payroll = buildPayroll(
      employees,
      summaries,
      ym,
      config,
      store.getPositionRates(ym).filter((r) =>
        co === "hs2" ? r.company === "hs2" : !r.company || r.company === "hangup"
      ),
      store.getBonusEvents(ym),
      store.getDeductionEvents(ym),
      store.getPayrollAdjustments(ym),
      new Map(),
      extras.commissionTiers,
      extras.loans,
      extras.loanPayments,
      [],
      [],
      new Map(),
      [],
      extras.loanMonthOverrides || [],
      extras.loanScheduleLines || []
    );
