...
async function notifyRegistrationSubmitted(request) {
  const company = String(request.company || "hangup").trim().toLowerCase() === "hs2" ? "hs2" : "hangup";
  const name = request.americanName || request.american_name || "Applicant";
  const unit = request.unit || "unit TBD";
  try {
    const dispatch = require("./notify-dispatch");
    await dispatch.dispatchNotification({
      actionKey: "registration_submitted",
      type: "registration_pending",
      title: "New agent registration",
      body: `${name} (${unit}) — review on Organization or Users`,
      entityType: "registration",
      entityId: String(request.id || ""),
      context: { company },
    });
  } catch (err) {
    // Fallback if routing table missing / rule disabled unexpectedly — still notify defaults
