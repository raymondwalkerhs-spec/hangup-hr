/**
 * Sales working-day rules — shift ends midnight Cairo; grace until 2:00 AM counts as previous day.
 */
...
/** Working day for payroll/dashboards (Cairo). 00:00–01:59 → previous calendar day. */
function computeWorkingDay(submissionDate) {
  const parts = parseSubmissionParts(submissionDate);
  if (parts.hour < 2) return addDays(parts.date, -1);
  return parts.date;
}

/** Current Cairo working day (grace until 02:00). */
