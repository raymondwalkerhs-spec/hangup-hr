/**
 * Sales working-day rules — shift ends midnight Cairo; grace until 3:00 AM counts as previous day.
 */
...
/** Working day for payroll/dashboards (Cairo). 00:00–02:59 → previous calendar day. */
function computeWorkingDay(submissionDate) {
  const parts = parseSubmissionParts(submissionDate);
  if (parts.hour < 3) return addDays(parts.date, -1);
  return parts.date;
}

/** Current Cairo working day (grace until 03:00). */
function currentWorkingDay(now = new Date()) {
  return computeWorkingDay(egyptDatetime.egyptNowFormatted(now));
}
...
/** Date-only + time columns for Postgres `date` + `text`/`time` storage. */
function storageDateParts(submissionDate) {
  const parts = parseSubmissionParts(submissionDate);
  return {
    submissionDate: parts.date,
    submissionTime: parts.time,
    workingDay: computeWorkingDay(`${parts.date} ${parts.time}`),
  };
}

/** Reconstruct Cairo-local submission timestamp for API/UI from date + time columns. */
function combineSubmissionDateTime(dateVal, timeVal) {
  const date = String(dateVal || "").slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return String(dateVal || "").trim();
  const timeRaw = String(timeVal || "").trim();
  if (!timeRaw) return date;
  const tm = timeRaw.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if (!tm) return `${date} ${timeRaw}`;
  const time = `${String(parseInt(tm[1], 10)).padStart(2, "0")}:${tm[2]}:${tm[3] || "00"}`;
  return `${date} ${time}`;
}
