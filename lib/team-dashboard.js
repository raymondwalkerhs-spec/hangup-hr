...
function saleTouchesDate(sale, date) {
  const wd = sale.workingDay || String(sale.submissionDate || "").slice(0, 10);
  const eff = sale.effectiveDate || "";
  const sub = String(sale.submissionDate || "").slice(0, 10);
  return wd === date || eff === date || sub === date;
}
...
function agentCountsForDay(sales, agentId, date) {
  ...
  for (const s of rows) {
    ...
    if (s.status === "pending" || s.status === "callback") {
      pending += 1;
      continue;
    }
  ...
  const totalSent = approved + postdated + dropped + pending;
  return { approved, postdated, dropped, pending, totalSent };
}
...
function buildDayDashboard({
  ...
  for (const team of sortedTeams) {
    ...
    if (weekend && !teamHasWeekendWork(attendanceRecords, date, teamAgentIds)) {
      ...
      const activeAgentsCount = allTeamAgents.length;
      teamSummaries.push({
        team,
        ...
        approved: 0,
        total: 0,
        conversion: "no sales yet",
        targetPercentage: teamDashboardRoster.teamTargetPercentage(0, activeAgentsCount),
        dayOffs: activeAgentsCount || 1,
      });
      continue;
    }
    ...
    const targetPercentage = teamDashboardRoster.teamTargetPercentage(teamApproved, activeAgentsCount);
...
function buildWeekDashboard({
  ...
  const days = dates.map((date) =>
    buildDayDashboard({ date, sales, employees, attendanceRecords, teamsMeta, appUsers })
  );
