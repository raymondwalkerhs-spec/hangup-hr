...
function buildDayDashboard({ date, sales, employees, attendanceRecords, teamsMeta = [] }) {
  ...
  for (const team of sortedTeams) {
    ...
    const activeAgents = allTeamAgents.filter((e) => !isAgentDayOff(attendanceRecords, e.id, date));
    const dayOffAgents = allTeamAgents.filter((e) => isAgentDayOff(attendanceRecords, e.id, date));
...
function dayHasAttendance(attendanceRecords, date) {
  return (attendanceRecords || []).some((r) => {
    if (r.date !== date) return false;
    const st = String(r.status || "").trim();
    if (!st || st === "Day-OFF") return false;
    return true;
  });
}

function buildWeekDashboard({ from, to, sales, employees, attendanceRecords, teamsMeta = [] }) {
  let dates = datesInRange(from, to);
  dates = dates.filter((date) => !isWeekendDate(date) || dayHasAttendance(attendanceRecords, date));
