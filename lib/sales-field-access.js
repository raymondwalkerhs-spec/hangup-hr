...
function redactSaleForRole(sale, userRole, perms, opts = {}) {
  ...
  if (opts.mergeQualityFormData && surface === "main") {
    const qualityOpts = { user: userRole, sale, surface: "quality" };
    for (const field of catalog.FIELDS) {
      if (field.section !== "quality" || catalog.isSystemHiddenField(field)) continue;
      if (!catalog.canViewFieldOnSurface(field, role, perms[field.key], "quality", qualityOpts)) continue;
      const val = sale.formData?.[field.key];
      if (val != null && val !== "") out.formData[field.key] = val;
    }
  }
  return out;
}
