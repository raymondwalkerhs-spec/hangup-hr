const { employeeDisplayName, getWorkingDaysForMonth, isPayrollEligibleForMonth } = require("./attendance");
...
function calcPayrollRow(
  ...
  const positionForSalary = options.positionOverride || resolved.position;
  const workingDaysInMonth = options.workingDaysOverride ?? getWorkingDaysForMonth(ym, config);
  const monthlySalary = options.monthlySalaryOverride ?? lookupSalary(positionForSalary, rates);
  ...
  if (options.includeCommission === false) salesCount = 0;
  ...
  if (noticePct > 0 && noticePct < 100 && adjustment?.noticePayScaledBasic != null) {
    basicSalaryAdjusted = Number(adjustment.noticePayScaledBasic);
  } else if (noticePct === 0 && adjustment?.noticePayScaledBasic === 0) {
  ...
  const netSalary = noPayroll ? 0 : basicSalaryAdjusted + totalBonuses - totalDeductions - bonusTransferPayroll;
