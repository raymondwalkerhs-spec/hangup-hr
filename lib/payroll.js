...
function calcPayrollRow(
  ...
  const loanDeductions = getEmployeeLoanDeductions(
    loans,
    emp.id,
    ym,
    loanPayments,
  ...
  for (const ld of loanDeductions) {
    const adjNote =
      ld.adjusted && ld.defaultInstallment != null
        ? ` (adjusted from ${ld.defaultInstallment})`
        : "";
    allDeductions.push({
      employeeId: emp.id,
      date: `${ym}-01`,
      amount: ld.amount,
      type: "Loan Repayment",
      reason: `Installment ${ld.installmentNumber}/${ld.installmentsTotal}${adjNote}${ld.notes ? ` — ${ld.notes}` : ""}`,
    });
  }
...
function buildEmployeePayrollHistory(
  ...
  return months.map((ym) => {
    ...
    return calcPayrollRow(
      ...
      {},
      [],
      data.loanMonthOverrides || [],
      data.loanScheduleLines || []
    );
  }).filter(Boolean);
}
