...
/** Slice for TL+closer split dashboards: "closed" | "team" | "all". */
function saleMatchesOpsSlice(sale, userRole, slice = "all") {
  if (slice === "all" || !slice) return saleMatchesOps(sale, userRole);
  const empId = userRole?.employeeId;
  if (slice === "closed") {
    return Boolean(empId && idsMatch(sale.closerId, empId));
  }
  if (slice === "team") {
    return salesScope.isTeamLeadOfSale(sale, userRole);
  }
  return saleMatchesOps(sale, userRole);
}

function shouldSplitTlClosedTeam(userRole) {
  // Company-wide roles always get the combined company sales cards — never TL split.
  if (isCompanyOpsRole(userRole)) return false;
  // Same gate as describeScope "team+closed" — TL (or org lead) who also closes sales.
  return isOpsCloser(userRole) && isOpsTl(userRole);
}

function filterSalesForDashboardOps(sales, userRole, slice = "all") {
  if (slice === "closed" || slice === "team") {
    return (sales || []).filter((s) => saleMatchesOpsSlice(s, userRole, slice));
  }
...
function buildOpsMonth({ month, sales, employees, attendanceRecords, userRole }) {
  ...
  if (shouldSplitTlClosedTeam(userRole)) {
    const closedSales = filterSalesForDashboardOps(sales, userRole, "closed");
    const teamSales = filterSalesForDashboardOps(sales, userRole, "team");
    payload.split = true;
    payload.closed = {
      scope: "closer",
      dailySales: dates.map((date) => ({
        date,
        sales: closedSales.filter((s) => saleDayKey(s) === date).length,
      })),
    };
    payload.team = {
      scope: "team",
      dailySales: dates.map((date) => ({
        date,
        sales: teamSales.filter((s) => saleDayKey(s) === date).length,
      })),
    };
  }
