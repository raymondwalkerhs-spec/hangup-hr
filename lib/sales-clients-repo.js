...
function mapClient(r) {
  return {
    ...
    saleProgram: r.sale_program || "mla",
    products: [],
  };
}
...
async function readSalesClientsCatalog(company, opts = {}) {
  let query = db().from("sales_clients").select("*").order("sort_order").order("name");
  if (company) {
    query = query.eq("company", company);
  }
  if (opts.saleProgram) {
    query = query.eq("sale_program", opts.saleProgram);
  }
  const { data: clients, error: e1 } = await query;
  if (e1) {
    // If company column doesn't exist yet, fall back to reading all
    if (/company.*column.*not.*exist|column.*company/i.test(e1.message)) {
      const { data: fallback, error: e1b } = await db().from("sales_clients").select("*").order("sort_order").order("name");
      if (e1b) throw new Error(e1b.message);
      return buildCatalogFromClients(fallback || []);
    }
    throw new Error(e1.message);
  }
  return buildCatalogFromClients(clients || []);
}

async function buildCatalogFromClients(clients) {
...
async function upsertClient(payload) {
  const row = {
    ...
    sale_program: String(payload.saleProgram || payload.sale_program || "mla").toLowerCase() === "rpm" ? "rpm" : "mla",
...
/** Seed sales_clients / products / prices from distinct values in the sales table (company-scoped). */
async function importFromExistingSales(company = "hangup") {
  const companyCtx = require("./company-context");
  const co = String(company || "hangup").toLowerCase();
  const { data: salesRaw, error } = await db().from("sales").select("client, device, price, form_data, unit");
  if (error) throw new Error(error.message);
  const sales = companyCtx.filterSalesByCompanyContext(
    (salesRaw || []).map((s) => ({ ...s, formData: s.form_data })),
    co
  );
  ...
  for (const sale of sales || []) {
    ...
    if (!clientId) {
      const { data: ins, error: e1 } = await db()
        .from("sales_clients")
        .insert({ name: clientName, sale_program: "mla" })
        .select()
        .single();
...
async function catalogHasActiveProducts(company, opts = {}) {
  const saleProgram = String(opts.saleProgram || "mla").toLowerCase();
  const catalog = await readSalesClientsCatalog(company, { saleProgram });
  if (saleProgram === "rpm") {
    return catalog.some((c) => c.status !== "disabled" && c.status !== "hold");
  }
  return catalog.some((c) => c.status !== "disabled" && (c.products || []).length > 0);
}

/** RPM uses client names only (no device/price tiers required). */
async function catalogHasActiveClients(company, opts = {}) {
  const saleProgram = String(opts.saleProgram || "mla").toLowerCase();
  const catalog = await readSalesClientsCatalog(company, { saleProgram });
  return catalog.some((c) => c.status !== "disabled" && c.status !== "hold");
}
...
async function validateAndResolveCatalogSale(body, company, opts = {}) {
  const saleProgram = opts.saleProgram || "mla";
  const catalog = await readSalesClientsCatalog(company, { saleProgram });
  ...
  const formData = {
    ...
    // Store tier label so quality surface can display "Price Tier: Standard / $X"
    priceTier: priceRow.label || "Standard",
    priceTierLabel: priceRow.label || "Standard",
  };
