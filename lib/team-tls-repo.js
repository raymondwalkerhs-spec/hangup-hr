...
async function readUnitCheckers(unit) {
  requireSupabase();
  const { data, error } = await db()
    .from("unit_checkers")
    .select("*")
...
async function readAllUnitCheckers() {
  requireSupabase();
  const { data, error } = await db().from("unit_checkers").select("*").order("created_at");
  if (error) throw new Error(error.message);
  const byUnit = {};
  for (const r of data || []) {
    if (!byUnit[r.unit]) byUnit[r.unit] = [];
...
async function addUnitChecker(unit, employeeId) {
  requireSupabase();
  const { error } = await db()
    .from("unit_checkers")
    .upsert({ unit, employee_id: employeeId }, { onConflict: "unit,employee_id", ignoreDuplicates: true });
...
async function removeUnitChecker(unit, employeeId) {
  requireSupabase();
  const { error } = await db()
    .from("unit_checkers")
    .delete()
...
function filterUnitCheckersByCompany(byUnit, company) {
  return filterUnitOpsByCompany(byUnit, company);
}
