 ...
 */
async function relocateTeamToUnit(teamId, newUnit, { reassignIds = true, username } = {}) {
  requireSupabase();
  const store = require("./data-store");
  ...
  await updateOrgTeam(teamId, { unit: newUnit, name: team.name }, username);
  ...
  for (const emp of onTeam) {
    ...
    if (reassignIds && rule && isDialingAgent(emp, { includeOut: true, activeOnly: false })) {
      ...
      if (needsNew) {
        const all = store.getEmployees({ hideOut: false });
        const nextId = idGen.suggestNextId(all, newUnit);
        if (store.getEmployeeById(nextId)) {
          throw new Error(`Suggested ID ${nextId} is already taken`);
        }
  ...
  return { team: { ...team, unit: newUnit }, changes };
}
