...
async function readItRequests(filters = {}) {
  ...
  if (filters.employeeId) {
    if (Array.isArray(filters.employeeId) && filters.employeeId.length) query = query.in('employee_id', filters.employeeId);
    else query = query.eq("employee_id", filters.employeeId);
  }
  // Fallback scope: when no employeeId is known, filter by the creator username
  if (!filters.employeeId && filters.createdBy) {
...
async function updateItRequest(id, patch) {
  ...
  if (patch.urgency !== undefined) row.urgency = patch.urgency || "normal";
...
async function deleteItRequest(id, actor) {
  requireSupabase();
  const existing = await readItRequestById(id);
...
async function readItUsers(options = {}) {
  ...
  // If the `is_it` column exists but no rows were returned, fall back to
  // the legacy role='it' lookup so older deployments without explicit IT
  // flags still resolve assignees.
  if (!users.length) {
    try {
      const { data: fallback, error: fbErr } = await db()
        ...
        .eq("role", "it")
        .eq("status", "active");
      if (!fbErr && fallback && fallback.length) {
        users = (fallback || []).map((u) => ({ username: u.username, employeeId: u.employee_id || "" }));
      }
    } catch (e) {
      // ignore and continue with empty users list
    }
  }
