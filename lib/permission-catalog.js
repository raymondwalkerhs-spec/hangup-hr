...
function defaultForRole(role, userRole) {
  ...
  return {
    ...
    viewSales: isChecker ? false : (ROLE_RANK[r] || 0) >= ROLE_RANK.agent,
    // RPM Sales log: agents/TL are restricted to "today only" unless allowed via Access Control.
    viewSalesThisMonth: ["op", "admin", "ceo", "hr", "rtm", "quality"].includes(r),
    // Period picker + team/agent/closer/client filters on Sales log (Admin can grant TL; can deny OP).
    viewSalesLogFilters: ["op", "admin", "ceo", "hr", "rtm", "quality"].includes(r),
    viewEquipment:
      !isChecker &&
    ...
    viewRpmWeeklyDashboard:
      ["admin", "rtm", "quality", "op", "tl"].includes(r) || Boolean((u.leadTeams || []).length),
    editRpmWeeklyTargets: ["admin", "rtm", "quality", "op"].includes(r),
    submitRpmChecks:
      ["admin", "ceo", "rtm", "quality", "tl", "op", "checker"].includes(r) ||
      Boolean((u.closerTeams || []).length),
    submitRpmQFeedback:
      ["admin", "ceo", "rtm", "quality", "tl", "op"].includes(r) ||
      Boolean((u.closerTeams || []).length),
    editRpmChecks: ["admin", "ceo", "rtm", "quality", "tl", "checker"].includes(r),
    editRpmQFeedback: ["admin", "ceo", "rtm", "quality", "tl"].includes(r),
    viewRpmChecks:
      ["admin", "ceo", "rtm", "quality", "tl", "op", "checker"].includes(r) ||
      Boolean((u.closerTeams || []).length) ||
      Boolean((u.leadTeams || []).length),
    viewRpmQFeedback:
      ["admin", "ceo", "rtm", "quality", "tl", "op"].includes(r) ||
      Boolean((u.closerTeams || []).length) ||
      Boolean((u.leadTeams || []).length),
    viewRpmQFeedbackAnalysis: ["op", "tl", "rtm", "quality", "hr", "admin", "ceo"].includes(r),
    viewRpmChecksDashboard:
      ["admin", "ceo", "rtm", "quality", "tl", "op", "checker"].includes(r) ||
      Boolean((u.closerTeams || []).length) ||
      Boolean((u.leadTeams || []).length),
    viewRpmCheckDuplicates: ["admin", "rtm"].includes(r),
    importRpmSaleFromCheck:
      ["admin", "ceo", "rtm", "quality", "tl", "op"].includes(r) ||
      Boolean((u.closerTeams || []).length),
    ...
    viewEmployeeDirectory: isChecker ? false : (ROLE_RANK[r] || 0) >= ROLE_RANK.agent,
    manageOrgStructure: roleIs(r, ORG_STRUCTURE_ROLES),
    deleteUnit: roleIs(r, ADMIN_ROLES),
    viewOrgFull:
    ...
    viewSale: isChecker ? false : (ROLE_RANK[r] || 0) >= ROLE_RANK.agent,
    ...
    viewAnalytics: roleIs(r, REPORTS_VIEW_ROLES),
    viewSalesRankings: roleIs(r, ["op", "admin", "ceo", "hr", "rtm", "quality"]),
    ...
    settingsThemeUnlocks: roleIs(r, MANAGE_ROLES),
    ...
    viewAnnouncements: (ROLE_RANK[r] || 0) >= ROLE_RANK.agent,
    editAnnouncements: roleIs(r, ADMIN_ROLES) || roleIs(r, ["hr", "rtm"]),
    viewCoaching:
      roleIs(r, ["tl", "op", "quality", "hr", "rtm", "admin", "ceo"]) ||
      Boolean(u.employeeId) ||
      Boolean((u.closerTeams || []).length) ||
      Boolean((u.leadTeams || []).length),
    submitCoaching:
      roleIs(r, ["tl", "op", "quality", "hr", "admin", "ceo"]) ||
      Boolean((u.closerTeams || []).length) ||
      Boolean((u.leadTeams || []).length),
    viewCoachingSecret: roleIs(r, ["hr", "quality", "admin", "ceo"]),
    ...
    viewInterviews: roleIs(r, ["hr", "admin", "ceo", "op", "tl", "quality"]),
    editInterview: roleIs(r, ["hr", "admin", "ceo", "quality"]),
    deleteInterview: roleIs(r, ["hr", "admin", "ceo"]),
    viewTraining: roleIs(r, ["hr", "admin", "ceo", "op", "tl", "quality"]),
    editTraining: roleIs(r, ["hr", "admin", "ceo", "op", "tl"]),
  };
}
...
const PERMISSIONS = [
  ...
  {
    key: "viewSalesThisMonth",
    label: "View sales this month",
    category: "Sales",
    description: "Allow agents/TL to view the current month in RPM sales logs (instead of today only)",
  },
  {
    key: "viewSalesLogFilters",
    label: "Sales log filters",
    category: "Sales",
    description:
      "Show period picker and team/agent/closer/client filters on Sales log. Default on for OP/Admin/Quality/RTM/HR/CEO; off for TL/agent (flip in Access Control)",
  },
  ...
  {
    key: "viewSalesRankings",
    label: "Sales Rankings report",
    category: "Reports",
    description:
      "Sales Rankings tab on Reports — top agents/closers/checks with period filter and hover breakdown. Default on for OP/Admin/Quality/RTM/HR/CEO; off for TL/agent",
  },
  { key: "viewAnalytics", label: "View analytics", category: "Pages", description: "Reporting & Analytics dashboard" },
  ...
  { key: "viewDashboardFull", label: "Dashboard full widgets", category: "Dashboard", description: "Headcount / active / units KPI row (legacy). Sales status and month curve use View sales log; company roles always see those widgets." },
  ...
  {
    key: "viewRpmWeeklyDashboard",
    label: "RPM weekly dashboard",
    category: "Dashboard",
    description:
      "Weekly closer/agent/client cards and team progress (Admin/RTM/Quality company-wide; OP unit; TL own team(s))",
  },
  {
    key: "editRpmWeeklyTargets",
    label: "Edit RPM weekly team targets",
    category: "Dashboard",
    description: "Set or clear per-team weekly RPM sales targets (Admin/RTM/Quality/OP — not TL)",
  },
  {
    key: "submitRpmChecks",
    label: "Submit RPM checks",
    category: "Sales",
    description: "Create Checks on /checks (Q/NQ/Age/Under Age/Duplicate)",
  },
  {
    key: "submitRpmQFeedback",
    label: "Submit Q Feedback",
    category: "Sales",
    description: "Disposition open Qs on /q-feedback (CallBack/Not Int/Retransfer/Dropped)",
  },
  {
    key: "editRpmChecks",
    label: "Edit RPM checks",
    category: "Sales",
    description: "Edit/delete existing check rows on Checks page",
  },
  {
    key: "editRpmQFeedback",
    label: "Edit Q Feedback",
    category: "Sales",
    description: "Edit Q Feedback dispositions (including terminal Dropped override)",
  },
  {
    key: "viewRpmChecks",
    label: "View Checks page",
    category: "Sales",
    description: "Open /checks list and period filters (without requiring submit)",
  },
  {
    key: "viewRpmQFeedback",
    label: "View Q Feedback page",
    category: "Sales",
    description: "Open /q-feedback queue and period filters (without requiring submit)",
  },
  {
    key: "viewRpmQFeedbackAnalysis",
    label: "Q Feedback Analysis tab",
    category: "Sales",
    description: "Closer Q→sale analysis and team active targets (OP / TL / RTM / Quality / HR)",
  },
  {
    key: "viewRpmChecksDashboard",
    label: "Checks dashboard widgets",
    category: "Dashboard",
    description: "Main Dashboard Checks / Q Feedback cards",
  },
  {
    key: "viewRpmCheckDuplicates",
    label: "View check duplicates review",
    category: "Sales",
    description: "Admin/RTM /check-duplicates review list",
  },
  {
    key: "importRpmSaleFromCheck",
    label: "Import sale from open Q",
    category: "Sales",
    description: "On Add RPM sale, import member/phone/DOB from same-day open Q checks",
  },
  { key: "manageHs2Company", label: "Manage HS-2 company", category: "Organization", description: "HS-2 sidebar switcher, org, payroll, and roster for HS-2. Hard-denied for OP, TL, and agent even if Access Control is flipped." },
  ...
  { key: "deleteUnit", label: "Delete unit", category: "Organization", description: "Permanently delete an organizational unit (Admin/CEO only)" },
  ...
  { key: "useNullAttendanceStatus", label: "Use null attendance status", category: "Payroll", description: "Allow selecting (--) status which clears attendance record" },
  ...
  { key: "viewOfficePo", label: "View Office PO", category: "Costs", description: "Open Office Purchase Order planner" },
  { key: "manageOfficePoItems", label: "Manage Office PO catalog", category: "Costs", description: "Create/edit Office PO items" },
  { key: "editOfficePoPurchases", label: "Edit Office PO purchases", category: "Costs", description: "Generate predictions, overrides, mark bought" },
  ...
  {
    key: "settingsThemeUnlocks",
    label: "Settings: premium theme unlock targets",
    category: "Settings",
    description: "Configure per-theme RPM sent/closed thresholds for premium themes (Admin/HR/CEO default)",
  },
  ...
  { key: "viewAnnouncements", label: "View announcements", category: "Pages", description: "Company announcements (current company only)" },
  { key: "editAnnouncements", label: "Edit announcements", category: "Pages", description: "Create, edit, and delete announcements (HR, RTM, Admin, CEO)" },
  { key: "viewCoaching", label: "View coaching", category: "People", description: "Coaching tickets (staff: scoped list; agents: own tickets)" },
  { key: "submitCoaching", label: "Submit coaching", category: "People", description: "Create coaching tickets for scoped agents (TL, OP, Closer, HR, Quality)" },
  { key: "viewCoachingSecret", label: "View coaching secret notes", category: "People", description: "See secret coaching notes (HR, Quality, Admin, CEO; coach/submitter can see notes on their own tickets)" },
  ...
  { key: "viewRequestFilters", label: "View request filters", category: "Requests", description: "Team/unit/type/status/frequency filters on requests page" },
  { key: "viewItRequestFilters", label: "View IT request filters", category: "IT", description: "Team/unit/status filters on IT requests page" },
  { key: "viewInterviews", label: "View interviews", category: "People", description: "Interviews page access" },
  { key: "editInterview", label: "Edit interviews", category: "People", description: "Create/update candidate interview records" },
  { key: "deleteInterview", label: "Delete interviews", category: "People", description: "Delete candidate interview records (HR/Admin/CEO)" },
  { key: "viewTraining", label: "View training", category: "People", description: "Training page access" },
  { key: "editTraining", label: "Edit training", category: "People", description: "Update training status, batches, and feedback" },
];
