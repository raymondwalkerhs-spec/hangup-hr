...
async function fetchVersionPolicyFromSupabase() {
  ...
  return {
    ...
    blockedMessage: current.notes
      ? `This app version is no longer supported. ${current.notes} Use Update now to install version ${current.version}.`
      : "",
    fieldBlockedMessage: forceUpdateMinVersion
      ? `This build is too old for HR and field staff. Use Update now to install version ${forceUpdateMinVersion} or newer. ${current.notes || ""}`.trim()
      : "",
    entries,
  };
}
