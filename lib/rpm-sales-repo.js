...
function filterSales(rows, filters = {}) {
  ...
  if (filters.day) out = out.filter((s) => String(s.workingDay || "") === String(filters.day));
...
async function readRpmSales(filters = {}) {
  ...
  if (filters.day) q = q.eq("working_day", filters.day);
  ...
  return sortRpmSales(rows, filters.sort);
}
