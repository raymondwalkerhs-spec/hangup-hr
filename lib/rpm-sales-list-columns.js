/**
 * Admin-configurable RPM sales log list columns.
 */
const { getSupabaseAdmin } = require("./supabase-client");
const { useSupabase } = require("./backend");
const rpmFieldAccess = require("./rpm-sales-field-access");
...
async function loadConfigMap() {
  ...
  const { data, error } = await db().from("rpm_sales_list_column_config").select("*");
  if (error) {
...
async function upsertColumn(columnKey, patch) {
  ...
  const { data, error } = await db().from("rpm_sales_list_column_config").upsert(row).select().single();
...
async function canViewColumn(columnKey, role, perms) {
  ...
  if (columnKey === "agent" || columnKey === "closer") {
    return ["tl", "op", "quality", "rtm", "admin", "ceo", "hr", "finance", "agent"].includes(
      String(role || "").toLowerCase()
    );
  }
  const field = catalog.getFieldDef(fieldKey) || { key: fieldKey, viewRoles: catalog.DEFAULT_VIEW || [] };
  return catalog.canViewFieldOnSurface(field, role, perms?.[fieldKey], "main");
}

async function getVisibleColumnsForUser(role) {
  const perms = await rpmFieldAccess.loadPermissionsMap();
...
async function upsertColumnsBatch(updates) {
  if (!useSupabase()) throw new Error("Requires Supabase");
  const results = [];
  for (const { columnKey, enabled } of updates || []) {
    results.push(await upsertColumn(columnKey, { enabled }));
  }
  invalidateCache();
  return results;
}
