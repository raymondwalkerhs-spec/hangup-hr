/**
 * RPM sales field + attachment access by surface (no verifier workflow).
 */
const catalog = require("./sales-rpm-field-catalog");
const { withQualityTicketAttachFloor } = require("./sales-quality-attachments");
...
function isTicketWorkflowSection(section) {
  return section === "quality" || section === "client" || section === "internal";
}
...
function rolesForSurface(dbPerm, field, surface) {
  if (isNotesField(field) && surface !== "edit") {
    return field.qualityViewRoles || field.mainViewRoles || field.viewRoles || DEFAULT_VIEW;
  }
  if (surface === "main") {
    const main = permRoles(dbPerm, "main_view_roles", "mainViewRoles") || permRoles(dbPerm, "view_roles", "viewRoles");
    if (main?.length) return main;
    if (field.mainViewRoles?.length) return field.mainViewRoles;
  ...
  if (surface === "quality") {
    ...
    if (field.qualityViewRoles?.length) return field.qualityViewRoles;
    if (isTicketWorkflowSection(field.section)) return field.viewRoles || QUALITY_ROLES;
    return [];
  }
  if (surface === "edit") {
...
function canEditClientFeedback(user, dbPerm = null) {
  ...
  const editRoles = rolesFromPerm(dbPerm, "edit_roles", "editRoles", field.editRoles || ["admin", "quality", "rtm"]);
...
function canEditFieldOnSurface(field, role, dbPerm = null, surface = "main", opts = {}) {
  ...
  const catalogEdit =
    field.section === "internal"
      ? field.editRoles || ["admin", "rtm"]
      : isTicketWorkflowSection(field.section)
        ? field.editRoles || QUALITY_ROLES
        : field.editRoles || DEFAULT_EDIT;
  const editRoles = rolesFromPerm(dbPerm, "edit_roles", "editRoles", catalogEdit);
  let canEdit = roleInList(role, editRoles);
  if (
    !canEdit &&
    surface === "quality" &&
    (field.section === "quality" || field.section === "client") &&
    roleInList(role, field.editRoles || QUALITY_ROLES)
  ) {
    canEdit = true;
  }
  if (field.key === "clientFeedback" && opts.user) {
...
function sanitizeFormPayload(formData, role, permissionsMap = {}, opts = {}) {
  const { create = false, user = null, sale = null, qualityTicket = false } = opts;
  const surface = opts.surface || "main";
  ...
  for (const field of FIELDS) {
    ...
    const perm = permissionsMap[field.key];
    let canEdit = canEditFieldOnSurface(field, role, perm, surface, { user, sale });
    if (
      !canEdit &&
      (qualityTicket || surface === "quality") &&
      (field.section === "quality" || field.section === "client") &&
      roleInList(role, field.editRoles || QUALITY_ROLES)
    ) {
      canEdit = true;
...
function listFieldsForSubmit(role, opts = {}) {
  return FIELDS.filter((f) => {
    ...
    if (isTicketWorkflowSection(f.section)) return false;
    if (SUBMIT_EXCLUDED_KEYS.has(f.key)) return false;
    return true;
  }).map((f) => ({ ...f, canView: true, canEdit: true }));
}
...
function listAttachmentKindsForSubmit(role) {
  return ATTACHMENT_KINDS.map((kindDef) => {
    ...
    const canEdit = roleInList(role, editRoles);
    const canView = canEdit || roleInList(role, viewRoles);
    return { key: kindDef.key, label: kindDef.label, viewRoles, editRoles, canView, canEdit };
  }).filter((k) => k.canEdit);
}
...
function resolveAttachmentKind(kindDef, role, attachPerm = null) {
  const viewRoles = withQualityTicketAttachFloor(
    kindDef.key,
    attachPerm?.viewRoles || attachPerm?.view_roles,
    kindDef.viewRoles || []
  );
  const editRoles = withQualityTicketAttachFloor(
    kindDef.key,
    attachPerm?.editRoles || attachPerm?.edit_roles,
    kindDef.editRoles || []
  );
  return {
    key: kindDef.key,
...
function canViewAttachmentKind(kind, role, attachPermMap = {}) {
  ...
  const viewRoles = withQualityTicketAttachFloor(
    kind,
    perm?.viewRoles || perm?.view_roles,
    def.viewRoles || []
  );
...
function canEditAttachmentKind(kind, role, attachPermMap = {}) {
  ...
  const editRoles = withQualityTicketAttachFloor(
    kind,
    perm?.editRoles || perm?.edit_roles,
    def.editRoles || []
  );
...
function getDefaultFieldPermissions(field) {
  ...
  const mainViewRoles = field.mainViewRoles?.length ? field.mainViewRoles : viewRoles;
  const qualityViewRoles =
    field.qualityViewRoles?.length
      ? field.qualityViewRoles
      : isTicketWorkflowSection(field.section)
        ? field.viewRoles || QUALITY_ROLES
        : [];
  return { viewRoles, editRoles, mainViewRoles, qualityViewRoles };
}
