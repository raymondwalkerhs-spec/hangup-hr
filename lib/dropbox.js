...
/** Test write + folder list scopes (call after regenerating token in App Console). */
async function verifyAccess() {
  ...
  try {
    await contentUpload(probePath, probe, "probe.txt");
    await apiRequest("/2/files/delete_v2", { body: { path: probePath.startsWith("/") ? probePath : `/${probePath}` } });
    return { ok: true, folder, message: "files.content.write OK" };
  } catch (err) {
    const hint = isScopeError(err)
      ? "In Dropbox App Console: enable files.content.write + sharing.write, click Generate access token, paste the NEW token into DROPBOX_ACCESS_TOKEN in .env (old tokens do not pick up new scopes)."
      : "";
...
async function createSharedLink(dropboxPath) {
  try {
    const shared = await apiRequest("/2/sharing/create_shared_link_with_settings", {
      body: { path: dropboxPath, settings: { requested_visibility: "team" } },
    });
    return shared.url || "";
  } catch {
    return "";
