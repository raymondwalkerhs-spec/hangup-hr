...
const PORTAL_SALE_ID_FIELD = process.env.AIRTABLE_PORTAL_SALE_ID_FIELD || "";
...
/** Attachment kinds to skip when not present on the target table (comma-separated env override) */
const skipEnv = process.env.AIRTABLE_SKIP_ATTACHMENT_KINDS;
const SKIP_ATTACHMENT_KINDS = new Set(
  skipEnv === ""
    ? []
    : String(skipEnv || "raw_call,quality_record,confirmation")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean)
);
...
function buildSaleFieldsForAirtable(sale, employees) {
  ...
  if (sale.price != null && sale.price !== "") fields.Price = Number(sale.price);
