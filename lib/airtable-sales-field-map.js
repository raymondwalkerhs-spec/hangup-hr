...
function formatDateTimeForAirtable(dateVal, timeVal) {
  ...
  if (time && /^\d{1,2}:\d{2}/.test(time)) {
    return egyptDatetime.cairoLocalToUtcIso(date, time);
  }
  return egyptDatetime.cairoLocalToUtcIso(date, "12:00:00");
}

function formatInstantForAirtable(val) {
...
function resolveFormValue(formKey, raw, employees) {
  ...
  if (formKey === "dateOfBirth" || formKey === "billingDate" || formKey === "effectiveDate") {
...
function buildSaleFieldsForAirtable(sale, employees) {
  ...
  if (sale.price != null && sale.price !== "") {
    ...
    const numPrice = m ? parseFloat(m[1].replace(/,/g, "")) : undefined;
    fields.Price = numPrice;
    if (raw.includes("(") && raw.includes(")")) {
      fields["Price tier"] = raw;
    } else if (numPrice != null) {
      const dev = reverseMapDevice(device || "");
      const cli = client || "";
      fields["Price tier"] = `$${numPrice.toFixed(2)} (${dev}) ${cli}`.trim();
    }
  }
