...
const EMPLOYEE_ID_COLUMNS = [
  ...
  { table: "rpm_sales", columns: ["agent_id", "closer_id"] },
  { table: "agent_training_programs", columns: ["employee_id"] },
  { table: "extra_payroll_entries", columns: ["employee_id"] },
...
async function releaseEmployeeAppId(appId, username) {
  ...
  try {
    await db().from("team_tls").delete().eq("employee_id", appId);
  } catch {
    /* optional */
  }
  try {
    await db().from("unit_ops").delete().eq("employee_id", appId);
    await db().from("unit_checkers").delete().eq("employee_id", appId);
  } catch {
    /* optional */
  }
