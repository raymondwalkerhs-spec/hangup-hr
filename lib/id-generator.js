...
function validateAppIdForUnit(id, unit, backendPool, employees = [], options = {}) {
  ...
  if ((employees || []).some((e) => e.id === trimmed && !isDeletedEmployee(e))) {
...
function filterEmployees(employees, { hideOut = true, unit, team, q, excludePromoted = true, month = null, includeDeleted = false, includeOtherAgent = false, attendanceRecords = [] } = {}) {
  let list = [...employees];
  if (!includeOtherAgent) {
    const { isOtherAgentId } = require("./other-agent");
    list = list.filter((e) => !isOtherAgentId(e?.id));
  }
  if (!includeDeleted) {
    const { isDeletedEmployee } = require("./employee-identity");
