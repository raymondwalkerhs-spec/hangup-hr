...
async function listAppUsers() {
  const { data, error } = await db()
    .from("app_users")
    .select("id, username, email, role, status, employee_id, is_it, last_login_at, created_at, updated_at")
    .order("username");
...
async function updateAppUser(username, { password, role, status, email, isIt }, actor) {
  const name = normalizeUsername(username);
  const existing = await getAppUser(name);
