...
const APP_USER_COLUMNS =
  "id, username, email, email_claimed, google_email, google_linked_at, mfa_required, mfa_enrolled_at, auth_user_id, role, status, employee_id, is_it, last_login_at, created_at, updated_at, password_changed_at";
...
async function selectAppUsers(includeIsIt = true) {
  ...
  if (error) {
    if (includeIsIt && /is_it|column.*not.*exist|email_claimed|google_email|mfa_enrolled|auth_user_id/i.test(error.message)) {
...
function sanitizeUser(row) {
  ...
  return {
    ...
    email: row.email || "",
    emailClaimed: row.email_claimed || row.email || "",
    googleEmail: row.google_email || "",
    googleLinkedAt: row.google_linked_at || null,
