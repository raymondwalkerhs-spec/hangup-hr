...
const ASSIGNABLE_ROLES = ["ceo", "admin", "hr", "finance", "op", "tl", "quality", "rtm", "office_assistant", "agent"];
...
async function updateAppUser(username, { password, role, status, email }, actor) {
  ...
  if (status !== undefined) {
    ...
    if (patch.status !== existing.status) changes.push(`status: ${existing.status} → ${patch.status}`);
  }
