...
function mapEmployeeFromDb(r) {
  const { normalizeEmployeeDepart } = require("../depart-attendance");
  return normalizeEmployeeDepart(
    mapEmployeeRow({
    ID: r.id,
    ...
    training_passed: r.training_passed === true,
    ...
    wfh: r.wfh === true,
    sales_mla_enabled: r.sales_mla_enabled === true,
    sales_rpm_enabled: r.sales_rpm_enabled === true,
    can_submit_checks_q_feedback: r.can_submit_checks_q_feedback === true,
    checks_submit_scope: r.checks_submit_scope || "self",
    team_dashboard_extra_teams: r.team_dashboard_extra_teams || [],
    sales_agent_picker:
      r.sales_agent_picker === true ? true : r.sales_agent_picker === false ? false : null,
  })
  );
}
...
function employeeToDb(emp) {
  const row = {
    ...
    unit: emp.unit || null,
    team: emp.team || null,
    payment_method: normalizePaymentMethodValue(emp.payment_method) || null,
    ...
    identification: emp.identification || emp.national_id || null,
    national_id: emp.national_id || null,
    passport_number: emp.passport_number || null,
    training_passed: Boolean(emp.training_passed),
    ...
    wfh: Boolean(emp.wfh),
    sales_mla_enabled: Boolean(emp.sales_mla_enabled ?? emp.salesMlaEnabled),
    sales_rpm_enabled: Boolean(emp.sales_rpm_enabled ?? emp.salesRpmEnabled),
    can_submit_checks_q_feedback: Boolean(
      emp.can_submit_checks_q_feedback ?? emp.canSubmitChecksQFeedback
    ),
    checks_submit_scope:
      String(emp.checks_submit_scope ?? emp.checksSubmitScope ?? "self").toLowerCase() === "team"
        ? "team"
        : "self",
    team_dashboard_extra_teams: (() => {
      const { normalizeExtraTeams } = require("../entity-mappers");
      return normalizeExtraTeams(emp.team_dashboard_extra_teams ?? emp.teamDashboardExtraTeams);
    })(),
    updated_at: new Date().toISOString(),
  };
  if ("sales_agent_picker" in emp || "salesAgentPicker" in emp) {
    const v = emp.sales_agent_picker ?? emp.salesAgentPicker;
    row.sales_agent_picker = v === true ? true : v === false ? false : null;
  }
  ...
  return {
    ...
    monthlySalaryOverride:
      r.monthly_salary_override != null && r.monthly_salary_override !== ""
        ...
        : null,
    netSalaryOverride:
      r.net_salary_override != null && r.net_salary_override !== ""
        ? Number(r.net_salary_override)
        : null,
    trainingNetSalaryOverride:
      r.training_net_salary_override != null && r.training_net_salary_override !== ""
        ? Number(r.training_net_salary_override)
        : null,
    agentNetSalaryOverride:
      r.agent_net_salary_override != null && r.agent_net_salary_override !== ""
        ? Number(r.agent_net_salary_override)
        : null,
    trainingPayrollPaid: boolFromDb(r.training_payroll_paid),
    trainingPhase1PayException: boolFromDb(r.training_phase1_pay_exception),
    trainingPayrollAnchorMonthOverride: String(r.training_payroll_anchor_month || "").slice(0, 7) || "",
    ...
    salesCountMla: Number(r.sales_count_mla) || 0,
    salesCountRpm: Number(r.sales_count_rpm) || 0,
    noPayroll: boolFromDb(r.no_payroll),
    payslipVisibleToAgent: boolFromDb(r.payslip_visible_to_agent),
    fullTransportGrant: boolFromDb(r.full_transport_grant),
  };
}
...
function payrollAdjustmentToDb(record, updatedBy) {
  return {
    ...
    monthly_salary_override: record.monthlySalaryOverride != null ? Number(record.monthlySalaryOverride) : null,
    net_salary_override: record.netSalaryOverride != null ? Number(record.netSalaryOverride) : null,
    training_net_salary_override:
      record.trainingNetSalaryOverride != null ? Number(record.trainingNetSalaryOverride) : null,
    agent_net_salary_override:
      record.agentNetSalaryOverride != null ? Number(record.agentNetSalaryOverride) : null,
    training_payroll_paid: boolToDb(record.trainingPayrollPaid),
    training_phase1_pay_exception: boolToDb(record.trainingPhase1PayException),
    training_payroll_anchor_month: record.trainingPayrollAnchorMonthOverride
      ? String(record.trainingPayrollAnchorMonthOverride).slice(0, 7)
      : null,
    payment_method: normalizePaymentMethodValue(record.paymentMethod) || "",
    ...
    sales_count_mla: Number(record.salesCountMla ?? record.sales_count_mla) || 0,
    sales_count_rpm: Number(record.salesCountRpm ?? record.sales_count_rpm) || 0,
    no_payroll: boolToDb(record.noPayroll),
    payslip_visible_to_agent: boolToDb(record.payslipVisibleToAgent),
    full_transport_grant: boolToDb(record.fullTransportGrant),
    updated_by: updatedBy,
    updated_at: new Date().toISOString(),
  };
}
...
function mapLoanPaymentFromDb(r) {
  return {
    ...
    scheduleLineId: r.schedule_line_id || null,
    recordedBy: r.recorded_by || "",
    recordedAt: r.recorded_at || "",
  };
}

function mapLoanScheduleLineFromDb(r) {
  return {
    id: r.id,
    loanId: r.loan_id,
    seq: Number(r.seq) || 0,
    ...
    deferredFromLineId: r.deferred_from_line_id || null,
    deferredToLineId: r.deferred_to_line_id || null,
    locked: boolFromDb(r.locked),
    unit: r.unit || "",
    createdAt: r.created_at || "",
    updatedAt: r.updated_at || "",
  };
...
function loanScheduleLineToDb(line) {
  const row = {
    id: line.id,
    loan_id: line.loanId,
    seq: Number(line.seq) || 0,
    year_month: line.yearMonth,
    due_amount: Number(line.dueAmount) || 0,
    status: line.status || "scheduled",
    paid_amount: line.paidAmount != null ? Number(line.paidAmount) : null,
    paid_at: line.paidAt || null,
    skip_reason: line.skipReason || "",
    deferred_from_line_id: line.deferredFromLineId || null,
    deferred_to_line_id: line.deferredToLineId || null,
    locked: boolToDb(line.locked === true),
    unit: line.unit || null,
    updated_at: new Date().toISOString(),
  };
  if (line.createdAt) row.created_at = line.createdAt;
  return row;
}

function mapLoanMonthOverrideFromDb(r) {
  return {
    id: r.id,
    loanId: r.loan_id,
    employeeId: r.employee_id,
...
module.exports = {
  ...
  mapLoanMonthOverrideFromDb,
  mapLoanScheduleLineFromDb,
  loanScheduleLineToDb,
  mapSplitFromDb,
