...
function loadSqlite() {
  if (!Database) {
    try {
      ...
      throw new Error(
        `Local cache module failed to load (${err.message}). Reinstall Hangup Portal or run the installer build with "npm run rebuild:native".`
      );
    }
  }
  return Database;
...
function initSchema(database) {
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS loan_month_overrides (
      loan_id TEXT NOT NULL,
      year_month TEXT NOT NULL,
      data TEXT NOT NULL,
      PRIMARY KEY (loan_id, year_month)
    );
    CREATE INDEX IF NOT EXISTS idx_loan_month_overrides_month ON loan_month_overrides(year_month);
    ...
    CREATE INDEX IF NOT EXISTS idx_loan_schedule_lines_loan ON loan_schedule_lines(loan_id);
    CREATE INDEX IF NOT EXISTS idx_loan_schedule_lines_month ON loan_schedule_lines(year_month);
...
function upsertPositionRateMonthly(yearMonth, position, monthlySalary, company = "hangup") {
  const ym = String(yearMonth || "").slice(0, 7);
  const co = String(company || "hangup").toLowerCase() === "hs2" ? "hs2" : "hangup";
  const row = { position, monthlySalary: Number(monthlySalary) || 0, yearMonth: ym, company: co };
  getDb()
    .prepare(
...
function deletePositionRateMonthly(yearMonth, position, company = "hangup") {
  const ym = String(yearMonth || "").slice(0, 7);
  const co = String(company || "hangup").toLowerCase() === "hs2" ? "hs2" : "hangup";
  const rows = getDb()
    .prepare("SELECT position, data FROM position_rate_monthly WHERE year_month = ?")
    .all(ym);
  for (const r of rows) {
    if (r.position !== position) continue;
    let rowCo = "hangup";
    try {
      rowCo = JSON.parse(r.data).company || "hangup";
    } catch {
      /* ignore */
    }
    if (rowCo === co) {
...
function getConfigRaw() {
  ...
  return {
    ...
    themeUnlockThresholds: out.themeUnlockThresholds || null,
    authBackend: out.authBackend || null,
  };
}
...
function syncPayrollAdjustmentsPaymentMethodInCache(employeeId, paymentMethod) {
  const database = getDb();
  const rows = database
    .prepare("SELECT employee_id, year_month, data FROM payroll_adjustments WHERE employee_id = ?")
    .all(employeeId);
  let count = 0;
  for (const row of rows) {
    const rec = JSON.parse(row.data);
    if (rec.paymentMethod === paymentMethod) continue;
    rec.paymentMethod = paymentMethod;
    upsertPayrollAdjustment(rec);
    count++;
  }
  return count;
}
...
function updateEmployeeWarningInCache(warning) {
  getDb()
    .prepare("INSERT OR REPLACE INTO employee_warnings (id, employee_id, data) VALUES (?, ?, ?)")
    .run(warning.id, warning.employeeId, JSON.stringify(warning));
}

function deleteEmployeeWarningFromCache(id) {
  getDb().prepare("DELETE FROM employee_warnings WHERE id = ?").run(id);
}
...
function setCommissionTiers(tiers) {
  ...
  const tx = database.transaction((rows) => {
    ...
    for (const t of rows) {
      const co = t.company === "hs2" ? "hs2" : "hangup";
      const cacheKey = `${co}:${t.yearMonth}`;
      stmt.run(cacheKey, t.minSales, JSON.stringify({ ...t, company: co }));
    }
  });
  tx(tiers);
}

function getCommissionTiersForMonth(yearMonth, company = "hangup") {
  const co = company === "hs2" ? "hs2" : "hangup";
  const cacheKey = `${co}:${yearMonth}`;
  let rows = getDb()
    .prepare("SELECT data FROM commission_tiers WHERE year_month = ? ORDER BY min_sales")
    .all(cacheKey);
  if (!rows.length && co === "hangup") {
    rows = getDb()
      .prepare("SELECT data FROM commission_tiers WHERE year_month = ? ORDER BY min_sales")
      .all(yearMonth);
  }
...
function setLoanMonthOverrides(overrides) {
  const database = getDb();
  const tx = database.transaction((rows) => {
    database.prepare("DELETE FROM loan_month_overrides").run();
    const stmt = database.prepare(
      "INSERT INTO loan_month_overrides (loan_id, year_month, data) VALUES (?, ?, ?)"
    );
    for (const o of rows) stmt.run(o.loanId, o.yearMonth, JSON.stringify(o));
  });
  tx(overrides || []);
}

function getAllLoanMonthOverrides() {
  return getDb()
    .prepare("SELECT data FROM loan_month_overrides ORDER BY year_month DESC")
    .all()
    .map((r) => JSON.parse(r.data));
}

function getLoanMonthOverridesForLoan(loanId) {
  return getAllLoanMonthOverrides().filter((o) => o.loanId === loanId);
}

function upsertLoanMonthOverrideCache(override) {
  getDb()
    .prepare(
      "INSERT OR REPLACE INTO loan_month_overrides (loan_id, year_month, data) VALUES (?, ?, ?)"
    )
    .run(override.loanId, override.yearMonth, JSON.stringify(override));
}

function deleteLoanMonthOverrideCache(loanId, yearMonth) {
  getDb()
    .prepare("DELETE FROM loan_month_overrides WHERE loan_id = ? AND year_month = ?")
    .run(loanId, yearMonth);
}

function setLoanScheduleLines(lines) {
  const database = getDb();
  const tx = database.transaction((rows) => {
    database.prepare("DELETE FROM loan_schedule_lines").run();
    const stmt = database.prepare(
      "INSERT INTO loan_schedule_lines (id, loan_id, year_month, data) VALUES (?, ?, ?, ?)"
    );
    for (const l of rows || []) {
      stmt.run(l.id, l.loanId, l.yearMonth, JSON.stringify(l));
    }
  });
  tx(lines || []);
}

function getAllLoanScheduleLines() {
  return getDb()
    .prepare("SELECT data FROM loan_schedule_lines ORDER BY loan_id, year_month")
    .all()
    .map((r) => JSON.parse(r.data));
}

function getLoanScheduleLinesForLoan(loanId) {
  return getDb()
    .prepare("SELECT data FROM loan_schedule_lines WHERE loan_id = ? ORDER BY year_month")
    .all(loanId)
    .map((r) => JSON.parse(r.data));
}

function upsertLoanScheduleLineCache(line) {
  getDb()
    .prepare(
      "INSERT OR REPLACE INTO loan_schedule_lines (id, loan_id, year_month, data) VALUES (?, ?, ?, ?)"
    )
    .run(line.id, line.loanId, line.yearMonth, JSON.stringify(line));
}

function replaceLoanScheduleLinesForLoanCache(loanId, lines) {
  const database = getDb();
  const tx = database.transaction(() => {
    database.prepare("DELETE FROM loan_schedule_lines WHERE loan_id = ?").run(loanId);
    const stmt = database.prepare(
      "INSERT INTO loan_schedule_lines (id, loan_id, year_month, data) VALUES (?, ?, ?, ?)"
    );
...
function deleteLoanScheduleLinesForLoanCache(loanId) {
  getDb().prepare("DELETE FROM loan_schedule_lines WHERE loan_id = ?").run(loanId);
}
...
function removeSaleFromBusinessCache(saleId) {
  const items = getBusinessCache("sales");
  if (!items?.length) return;
  const next = items.filter((s) => s.id !== saleId);
  if (next.length !== items.length) setBusinessCache("sales", next);
}

function upsertSaleInBusinessCache(sale) {
  if (!sale?.id) return;
  const items = getBusinessCache("sales");
  if (!items) return;
  const idx = items.findIndex((s) => s.id === sale.id);
  const next = items.slice();
  if (idx >= 0) next[idx] = { ...next[idx], ...sale };
  else next.unshift(sale);
  setBusinessCache("sales", next);
}

function upsertBonusRequestInBusinessCache(request) {
  if (!request?.id) return;
  const items = getBusinessCache("bonus_requests");
  if (!items) return;
  const idx = items.findIndex((r) => r.id === request.id);
  const next = items.slice();
  if (idx >= 0) next[idx] = { ...next[idx], ...request };
  else next.unshift(request);
  setBusinessCache("bonus_requests", next);
}
...
module.exports = {
  ...
  syncPayrollAdjustmentsPaymentMethodInCache,
  setCommissionTypes,
  getCommissionTypes,
  ...
  updateEmployeeWarningInCache,
  deleteEmployeeWarningFromCache,
  setCommissionTiers,
  ...
  getAllLoanPayments,
  appendLoanPayment,
  setLoanMonthOverrides,
  getAllLoanMonthOverrides,
  getLoanMonthOverridesForLoan,
  upsertLoanMonthOverrideCache,
  deleteLoanMonthOverrideCache,
  setLoanScheduleLines,
  getAllLoanScheduleLines,
  getLoanScheduleLinesForLoan,
  upsertLoanScheduleLineCache,
  replaceLoanScheduleLinesForLoanCache,
  deleteLoanScheduleLinesForLoanCache,
  setPayrollSplits,
  getAllPayrollSplits,
  ...
  setBusinessCache,
  getBusinessCache,
  removeSaleFromBusinessCache,
  upsertSaleInBusinessCache,
  upsertBonusRequestInBusinessCache,
};
