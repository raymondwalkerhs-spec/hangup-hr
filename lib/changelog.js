...
async function logChange(entry) {
  try {
    ...
    console.warn("Supabase change_log write skipped:", err.message);
  }
}
...
async function readChangeLog(opts = {}) {
  const { limit = 100, entity, username, month, employeeIds, company } = opts;
  ...
  if (employeeIds || company) {
    list = filterChangeLogByCompany(list, employeeIds, company);
  }
  return list.map((r) => ({
...
/** Keep entries tied to employees in scope; config rows scoped by company key. */
function filterChangeLogByCompany(entries, employeeIds, company) {
  const ids = employeeIds instanceof Set ? employeeIds : new Set(employeeIds || []);
  const co = String(company || "hangup").toLowerCase();
  const employeeEntities = new Set([
    "employee",
    "attendance",
    "bonus",
    "deduction",
  ...
  return (entries || []).filter((row) => {
    const ent = String(row.entity || "").toLowerCase();
    if (employeeEntities.has(ent)) {
      const eid = String(row.entity_id || "").split("|")[0];
      return ids.has(eid);
    }
    if (ent === "config") {
      const field = String(row.field || "").toLowerCase();
      if (co === "hs2") return field.includes("hs2");
      return !field.includes("hs2");
    }
    return co === "hangup";
  });
}
