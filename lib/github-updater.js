...
const { validateAsarHeader, verifyExtractedPatch } = require("./update-integrity");
...
function resolveRedirectUrl(loc) {
  if (!loc) throw new Error("Redirect without location");
  if (!isAllowedDownloadUrl(loc)) {
    throw new Error(`Download blocked: untrusted host (${loc})`);
  }
  return loc;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, { headers }, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        const loc = resolveRedirectUrl(res.headers.location);
...
function githubApiHeaders() {
  const headers = {
    ...
    "User-Agent": "Hangup-Portal-Updater",
  };
...
function verifyExtractedIfHashed(extractDir, info) {
  const hashes = info?.fileHashes;
  if (!hashes || !Object.keys(hashes).length) return;
  const files = info?.files?.length ? info.files : Object.keys(hashes);
  verifyExtractedPatch(extractDir, { fileHashes: hashes, files });
}

function verifyExtractedUpdatePayload(extractDir, info) {
  try {
    const infoPath = path.join(extractDir, "update-info.json");
    if (fs.existsSync(infoPath)) {
      const patchInfo = JSON.parse(fs.readFileSync(infoPath, "utf8"));
      verifyExtractedIfHashed(extractDir, patchInfo);
      return;
    }
  } catch (err) {
    throw new Error(`Update manifest verification failed: ${err.message}`);
  }
  verifyExtractedIfHashed(extractDir, info);
}
...
function pickReleaseAsset(release, latestVersion, installKind) {
  ...
  if (installKind === "nsis") {
    candidates.push(
      {
        re: new RegExp(`hangup-portal.*setup.*${latestEsc}.*\\.exe$`, "i"),
        exclude: /uninstall/i,
        method: "nsis",
        updateType: "installer",
      },
      {
        re: new RegExp(`hangup-hr-beta.*setup.*${latestEsc}.*\\.exe$`, "i"),
        exclude: /uninstall/i,
        method: "nsis",
        updateType: "installer",
      },
      {
        re: new RegExp(`setup.*${latestEsc}.*\\.exe$`, "i"),
        exclude: /uninstall/i,
        method: "nsis",
        updateType: "installer",
      },
      {
        re: new RegExp(`hangup-hr.*${latestEsc}.*setup.*\\.exe$`, "i"),
        exclude: /uninstall/i,
        method: "nsis",
        updateType: "installer",
      }
  ...
  const zipPatterns = [
    new RegExp(`hangup-portal-${latestEsc}-${suffix}-full\\.zip$`, "i"),
    new RegExp(`hangup-hr-${latestEsc}-${suffix}-full\\.zip$`, "i"),
    new RegExp(`hangup-portal-${latestEsc}-${suffix}\\.zip$`, "i"),
    new RegExp(`hangup-hr-${latestEsc}-${suffix}\\.zip$`, "i"),
  ];
  for (const re of zipPatterns) {
    candidates.push({ re, exclude: /patch/i, method: installKind === "mac" ? "mac-swap" : "portable-swap", updateType: "full" });
  }

  if (installKind === "portable") {
    candidates.push({
      re: new RegExp(`hangup-portal.*portable.*${latestEsc}\\.exe$`, "i"),
      method: "portable-exe",
      updateType: "portable-exe",
    });
  }

  if (installKind === "mac") {
    const altSuffix = suffix === "mac-arm64" ? "mac-x64" : "mac-arm64";
    candidates.push({
      re: new RegExp(`hangup-portal-${latestEsc}-${altSuffix}-full\\.zip$`, "i"),
      exclude: /patch/i,
      method: "mac-swap",
      updateType: "full",
    });
    candidates.push(
      { re: new RegExp(`hangup.*portal.*${latestEsc}\\.dmg$`, "i"), method: "mac-dmg", updateType: "installer" }
    );
  }

  for (const c of candidates) {
    const hit = assets.find(
      (a) => c.re.test(a.name) && !(c.exclude && c.exclude.test(a.name))
    );
    if (hit) {
      return { asset: hit, updateType: c.updateType, method: c.method };
    }
  }
...
async function applyNsisInstallerUpdate(info) {
  ...
  const setupPath = path.join(tmpDir, info.assetName || "Hangup-Portal-Setup.exe");
  try {
...
function stageMacPayload(extractDir, version) {
  ...
  const appName = path.basename(getMacAppBundle() || "Hangup Portal.app");
...
async function applyMacFullAppUpdate(info) {
  ...
  try {
    ...
    verifyExtractedUpdatePayload(extractDir, info);
...
async function applyPortableExeUpdate(info) {
  const currentExe = process.execPath;
  const exeName = path.basename(currentExe);
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "hangup-hr-portable-"));
  const newExePath = path.join(tmpDir, info.assetName || exeName);
  try {
    await downloadReleaseAsset(info, newExePath);
    const stat = fs.statSync(newExePath);
    if (stat.size < 1024 * 1024) {
      throw new Error("Downloaded portable executable is too small - file may be corrupt");
    }

    const script = path.join(os.tmpdir(), "hangup-hr-portable-update-" + Date.now() + ".bat");
    const lines = [
      "@echo off",
      "ping 127.0.0.1 -n 4 >nul",
      "taskkill /F /IM \"" + exeName + "\" /T 2>nul",
    ...
    return { ok: true, method: "portable-exe", needsQuit: true, version: info.latest };
  } catch (err) {
    try {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    } catch {
...
async function applyMacDmgUpdate(info) {
  const bundle = getMacAppBundle();
  if (!bundle) throw new Error("Could not locate .app bundle for update");

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "hangup-hr-dmg-"));
  const dmgPath = path.join(tmpDir, info.assetName || "update.dmg");
  let mounted = false;
  let mountPoint = null;
  try {
    await downloadReleaseAsset(info, dmgPath);
    const mountOut = execFileSync("hdiutil", ["attach", "-nobrowse", "-noautoopen", dmgPath], { encoding: "utf8" });
    const mountLine = mountOut.split("\n").find(function (l) { return /\/Volumes\//.test(l); });
    if (!mountLine) throw new Error("Could not mount update DMG");
    mountPoint = mountLine.trim().split(/\s+/).pop();
    mounted = true;

    let appPath = null;
    for (const e of fs.readdirSync(mountPoint)) {
      if (e.endsWith(".app")) { appPath = path.join(mountPoint, e); break; }
    }
    if (!appPath) throw new Error("DMG does not contain an .app bundle");

    const stagedApp = path.join(os.tmpdir(), "hangup-hr-staged-" + info.latest + "-" + Date.now(), path.basename(bundle));
    fs.mkdirSync(path.dirname(stagedApp), { recursive: true });
    copyDirRecursive(appPath, stagedApp);
    validatePayloadAsar(stagedApp, "mac");
    ...
    if (mounted && mountPoint) {
      try { execFileSync("hdiutil", ["detach", mountPoint], { timeout: 10000 }); } catch {}
    }
...
async function applyGitHubUpdate(info) {
  ...
  if (method === "nsis") return applyNsisInstallerUpdate(info);
  if (method === "mac-swap") return applyMacFullAppUpdate(info);
  if (method === "mac-dmg") return applyMacDmgUpdate(info);
  if (method === "portable-exe") return applyPortableExeUpdate(info);
