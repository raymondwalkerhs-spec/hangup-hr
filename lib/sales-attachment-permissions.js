...
async function deleteAttachmentPermission(attachmentKey) {
  if (!useSupabase()) return { deleted: false };
  const { error } = await db().from("sales_attachment_permissions").delete().eq("attachment_key", attachmentKey);
  if (error) {
