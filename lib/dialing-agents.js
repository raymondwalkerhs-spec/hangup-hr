...
function salesAgentPickerOverride(emp) {
...
function isDialingAgent(emp, { includeOut = false, activeOnly = false, teamLeadIds = null } = {}) {
  if (!emp) return false;
  const override = salesAgentPickerOverride(emp);
  if (override === true) return true;
  if (override === false) return false;

  const id = String(emp.id || "").trim().toUpperCase();
  if (!id) return false;
  if (teamLeadIds instanceof Set && teamLeadIds.has(String(emp.id || "").trim())) return false;
  ...
  // Deleted is never dialable unless sales_agent_picker override is explicitly true (handled above).
  if (normalizeStatusKey(emp.status) === "deleted") return false;
