...
function calcScopedPayrollRow(ctx, emp, dateSet, { position, payrollKind, includeCommission = true }) {
  ...
  const summary = summarizeEmployeeMonth(
    ...
    records,
    ctx.config,
    ctx.actionPlans.filter((p) => p.employeeId === emp.id && p.status === "active")
  );
...
function enrichPayrollRow(emp, standardRow, ctx, program) {
  ...
  if (!hasDualPayrollInMonth(program, ctx.ym)) {
    ...
    return applySplitsToKind(trainingOnly, ctx, "training");
  }
  return buildDualPayrollRow(emp, ctx, program);
}

async function enrichPayrollRows(rows, employees, ctx, programsByEmployee) {
