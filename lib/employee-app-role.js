...
function isEligibleReviewerEmployee(emp) {
  const lookedUp = lookupLiveAppRole(emp);
  if (lookedUp) return ["quality", "rtm", "admin", "ceo"].includes(lookedUp);
  const stamped = String(emp?.role || "").trim().toLowerCase();
  if (["quality", "rtm", "admin", "ceo"].includes(stamped)) return true;
  const lead = String(emp?.lead_role || emp?.leadRole || "").trim().toUpperCase();
  if (lead === "QA" || lead === "RTM" || lead === "MG") return true;
