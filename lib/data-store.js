...
const { normalizeAttendanceRecord } = require("./attendance-validation");
const {
...
async function readAttendanceEventsForMonth(yearMonth) {
  // Always read from the local SQLite cache first. saveAttendanceBatch keeps
  // the cache in sync via cache.upsertAttendanceRecord immediately after every
  // Supabase upsert, so the cache is authoritative for the current state of
  // the data. Going directly to Supabase introduces a race: the GET /attendance
  // response can arrive before Supabase replication has propagated the just-
  // written rows, making attendance edits appear to revert on reload.
  const cached = cache.getAttendanceForMonth(yearMonth);
  if (cached && cached.length > 0) return cached;

  // Cache is empty for this month (e.g. first ever load before any sync).
  // Fall back to Supabase and warm the cache so subsequent reads are fast.
  if (backendMod.useSupabase()) {
    const rows = await backend.readAttendanceEvents?.(yearMonth);
    if (rows && rows.length > 0) {
      cache.setAttendanceForMonth(yearMonth, rows);
      return rows;
    }
  }
  return cached; // empty array — nothing written yet
}
...
async function createEmployee(emp, username, { enforcePrefix = true } = {}) {
...
async function saveAttendanceBatch(records, username) {
  return withStoreMutationLock(async () => {
    ...
    let existingRows = [];
    if (backendMod.useSupabase()) {
      existingRows = (await backend.readAttendanceEvents?.(ym)) || [];
    } else {
      existingRows = cache.getAttendanceForMonth(ym);
    }
