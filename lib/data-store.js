...
} = require("./employee-roster-freshness");
...
async function syncFromSheetInner() {
  ...
  let allLoanMonthOverrides = [];
  if (typeof backend.readAllLoanMonthOverrides === "function") {
    try {
      allLoanMonthOverrides = await backend.readAllLoanMonthOverrides();
    } catch (err) {
      console.warn("[data-store] loan_month_overrides not loaded:", err.message);
      allLoanMonthOverrides = [];
    }
  }

  let allLoanScheduleLines = [];
  if (typeof backend.readAllLoanScheduleLines === "function") {
    try {
      allLoanScheduleLines = await backend.readAllLoanScheduleLines();
    } catch (err) {
      console.warn("[data-store] loan_schedule_lines not loaded:", err.message);
      allLoanScheduleLines = [];
    }
  }
  ...
  cache.setLoanMonthOverrides(allLoanMonthOverrides);
  cache.setLoanScheduleLines(allLoanScheduleLines);
...
function getPayrollExtras(yearMonth, company = "hangup") {
  ...
  return {
    ...
    // All payments needed so remaining balance is correct when computing this month's deduction
    loanPayments: cache.getAllLoanPayments().filter((p) => scopedIds.has(p.employeeId)),
    loanMonthOverrides: cache
      .getAllLoanMonthOverrides()
      .filter((o) => scopedIds.has(o.employeeId)),
    loanScheduleLines: cache
      .getAllLoanScheduleLines()
      .filter((l) => {
        const loan = cache.getEmployeeLoans().find((x) => x.id === l.loanId);
        return loan && scopedIds.has(loan.employeeId);
      }),
  };
}
...
/** Lightweight employee roster refresh (avoids full syncFromSheet). */
async function refreshEmployeesFromSupabase() {
  if (!backendMod.useSupabase()) {
    return getEmployees({ hideOut: false, includeDeleted: true });
  }
  const employees = await backend.readEmployees();
  cache.setEmployees(employees);
  try {
    const { getSupabaseAdmin } = require("./supabase-client");
    const { data: appUsers } = await getSupabaseAdmin().from("app_users").select("username, employee_id, role");
...
const employeeRosterFreshness = createEmployeeRosterFreshness(refreshEmployeesFromSupabase);

function markEmployeesFresh() {
  employeeRosterFreshness.markFresh();
}

/**
 * Shared roster freshness for auth, pickers, attendance, and team-scoped routes.
 * Local writes stamp the TTL so this desktop stays hot; other desktops pick up
 * Supabase team/status changes within ~15s without a full sync.
 */
async function ensureEmployeesFresh(opts = {}) {
  try {
    const result = await employeeRosterFreshness.ensureFresh(opts);
    if (result && result.skipped) {
      return getEmployees({ hideOut: false, includeDeleted: true });
    }
    return result;
  } catch (err) {
    console.warn("ensureEmployeesFresh failed, using cache:", err.message);
    return getEmployees({ hideOut: false, includeDeleted: true });
  }
}
...
async function createEmployee(emp, username, { enforcePrefix = true, skipLoginSync = false } = {}) {
  ...
  if (!mapped.employment_date) {
    const { todayLocalIsoDate } = require("./date-iso");
    mapped.employment_date = todayLocalIsoDate();
  }
  if (!mapped.probation_end_date && mapped.employment_date) {
    const { addCalendarDays, parseIsoDate } = require("./date-iso");
    const hire = parseIsoDate(mapped.employment_date);
    mapped.probation_end_date = hire ? addCalendarDays(hire, 90) : null;
  }
  if (mapped.position) {
...
async function updateThemeUnlockThresholds(thresholds, username) {
  const themeUnlocks = require("./theme-unlocks");
  const normalized = themeUnlocks.validateThemeUnlockThresholdsPayload(thresholds);
  await saveConfigKey("themeUnlockThresholds", normalized, username);
  themeUnlocks.clearUnlockCache();
  return themeUnlocks.getThemeUnlockThresholds(getConfig());
}
...
async function persistLoanScheduleForLoan(loanId, lines) {
  const forLoan = (lines || []).filter((l) => l.loanId === loanId);
  if (typeof backend.replaceLoanScheduleLinesForLoan === "function") {
    const saved = await backend.replaceLoanScheduleLinesForLoan(loanId, forLoan);
    cache.replaceLoanScheduleLinesForLoanCache(loanId, saved);
    return saved;
  }
  cache.replaceLoanScheduleLinesForLoanCache(loanId, forLoan);
  return forLoan;
}
...
async function createEmployeeLoan(loan, username) {
  ...
  const emp = cache.getEmployees({ hideOut: false }).find((e) => e.id === loan.employeeId);
  const saved = await backend.appendEmployeeLoan(
    {
      ...
      paused: loan.paused === true,
      unit: emp?.unit || loan.unit || null,
    },
    username
  );
  ...
  try {
    const lines = generateScheduleLines(saved, {
      startYearMonth: saved.startYearMonth,
      remainingAmount: Number(saved.totalAmount) || 0,
      unit: emp?.unit || null,
    });
    await persistLoanScheduleForLoan(saved.id, lines);
  } catch (err) {
    console.warn("[data-store] schedule generate on create failed:", err.message);
  }

  await changelog.logEmployeeChange(username, "loan", { id: loan.employeeId }, null, saved.id);
  return enrichLoanWithBalance(saved);
}
...
async function updateEmployeeLoanRecord(loanId, updates, username) {
  ...
  if (updates.paused !== undefined) merged.paused = updates.paused === true;
  if (updates.disbursementDate !== undefined) {
    merged.disbursementDate = updates.disbursementDate || "";
  }
  ...
  if (paid === 0 && !hasPayments) {
    ...
    merged = {
      ...
      startYearMonth:
        updates.startYearMonth ||
        computeStartYearMonth(merged.createdYearMonth, updates.skipCurrentMonth === true),
    };
  } else {
    merged = {
      ...merged,
      ...
      startYearMonth: updates.startYearMonth || existing.startYearMonth,
    };
  }
  ...
  if (
    updates.startYearMonth &&
    updates.startYearMonth !== existing.startYearMonth &&
    typeof backend.replaceLoanScheduleLinesForLoan === "function"
  ) {
    try {
      await setLoanStartMonth(loanId, updates.startYearMonth, username, { skipLoanUpdate: true });
    } catch (err) {
      console.warn("[data-store] schedule start shift after loan update failed:", err.message);
    }
  }
...
async function removeEmployeeLoan(loanId, username) {
  ...
  cache.deleteLoanScheduleLinesForLoanCache(loanId);
...
async function recordLoanPayment(loanId, yearMonth, username) {
  ...
  const allPayments = cache.getAllLoanPayments();
  if (allPayments.some((p) => p.loanId === loanId && p.yearMonth === yearMonth)) return null;

  const overrides = cache.getAllLoanMonthOverrides();
  const scheduleLines = cache.getAllLoanScheduleLines();
  const pending = getEmployeeLoanDeductions(
    [loan],
    loan.employeeId,
    yearMonth,
    allPayments,
  ...
  const payment = await backend.appendLoanPayment(
    {
      ...
      scheduleLineId: deduction.scheduleLineId || null,
    },
    username
  );
  cache.appendLoanPayment(payment);

  if (deduction.scheduleLineId) {
    const line = scheduleLines.find((l) => l.id === deduction.scheduleLineId);
    if (line) {
      const paidLine = markScheduleLinePaid(line, deduction.amount, payment.recordedAt);
      if (typeof backend.upsertLoanScheduleLines === "function") {
        const [saved] = await backend.upsertLoanScheduleLines([paidLine]);
        cache.upsertLoanScheduleLineCache(saved || paidLine);
      } else {
        cache.upsertLoanScheduleLineCache(paidLine);
      }
    }
  }

  const afterPayments = [...allPayments, payment];
  const rem = remainingLoanAmount(loan, afterPayments);
  const newPaid = (loan.installmentsPaid || 0) + 1;
  const updated = {
    ...loan,
    installmentsPaid: newPaid,
    status: rem <= 0.005 ? "completed" : "active",
  };
...
async function upsertLoanPaymentOverride(loanId, { yearMonth, amount, note }, username) {
  const loan = cache.getEmployeeLoans().find((l) => l.id === loanId);
  if (!loan) throw new Error("Loan not found");
  if (loan.status !== "active") throw new Error("Loan is not active");
  const ym = String(yearMonth || "").trim();
  if (!/^\d{4}-\d{2}$/.test(ym)) throw new Error("yearMonth must be YYYY-MM");
  if (ym < loan.startYearMonth) {
    throw new Error(`Month must be on or after loan start (${loan.startYearMonth})`);
  }
  const amt = roundMoney(amount);
  if (!(amt > 0)) throw new Error("amount must be greater than 0");

  const allPayments = cache.getAllLoanPayments();
  if (allPayments.some((p) => p.loanId === loanId && p.yearMonth === ym)) {
    throw new Error("Payment already recorded for that month — cannot change override");
  }
  ...
  const emp = cache.getEmployees({ hideOut: false }).find((e) => e.id === loan.employeeId);
  const saved = await backend.upsertLoanMonthOverride(
    {
      loanId: loan.id,
      employeeId: loan.employeeId,
      yearMonth: ym,
  ...
  cache.upsertLoanMonthOverrideCache(saved);
  await changelog.logEmployeeChange(
    username,
    "loan_payment_override",
    { id: loan.employeeId },
    null,
    `${loanId}:${ym}:${amt}`
  );
...
async function removeLoanPaymentOverride(loanId, yearMonth, username) {
  const loan = cache.getEmployeeLoans().find((l) => l.id === loanId);
  if (!loan) throw new Error("Loan not found");
  const ym = String(yearMonth || "").trim();
  await backend.deleteLoanMonthOverride(loanId, ym);
  cache.deleteLoanMonthOverrideCache(loanId, ym);
  await changelog.logEmployeeChange(
    username,
    "loan_payment_override_clear",
    { id: loan.employeeId },
    null,
    `${loanId}:${ym}`
  );
...
function enrichLoanWithBalance(loan) {
  const payments = cache.getAllLoanPayments().filter((p) => p.loanId === loan.id);
  const overrides = cache.getLoanMonthOverridesForLoan(loan.id);
  const schedule = cache.getLoanScheduleLinesForLoan(loan.id);
  const remaining = remainingLoanAmount(loan, payments);
  return {
    ...loan,
    remainingBalance: remaining,
    paidTotal: roundMoney((Number(loan.totalAmount) || 0) - remaining),
    estimatedInstallmentsLeft: installmentsRemaining(loan, payments),
    paymentOverrides: overrides,
    scheduleLines: schedule
      .filter((l) => l.status !== "cancelled")
      .slice()
      .sort((a, b) => a.yearMonth.localeCompare(b.yearMonth) || a.seq - b.seq),
  };
}
...
function getLoanSchedule(loanId) {
  ...
  if (!loan) throw new Error("Loan not found");
  return {
    loan: enrichLoanWithBalance(loan),
    scheduleLines: enrichLoanWithBalance(loan).scheduleLines || [],
  };
}

async function skipLoanScheduleMonth(loanId, yearMonth, reason, username) {
  const loan = cache.getEmployeeLoans().find((l) => l.id === loanId);
  if (!loan) throw new Error("Loan not found");
  if (loan.status !== "active") throw new Error("Loan is not active");
  const ym = String(yearMonth || "").trim();
  ...
  const allPayments = cache.getAllLoanPayments();
  if (allPayments.some((p) => p.loanId === loanId && p.yearMonth === ym)) {
    throw new Error("Payment already recorded for that month — cannot skip");
  }

  let lines = cache.getLoanScheduleLinesForLoan(loanId);
  if (!lines.length) {
    lines = generateScheduleLines(loan, {
      startYearMonth: loan.startYearMonth,
      remainingAmount: remainingLoanAmount(loan, allPayments),
    });
  }

  const emp = cache.getEmployees({ hideOut: false }).find((e) => e.id === loan.employeeId);
  const result = skipScheduleMonth(lines, {
    loanId,
    yearMonth: ym,
    reason: reason || "",
    unit: emp?.unit || null,
  });
  const saved = await persistLoanScheduleForLoan(loanId, result.lines);
  await changelog.logEmployeeChange(
    username,
    "loan_schedule_skip",
    { id: loan.employeeId },
  ...
  return {
    loan: enrichLoanWithBalance(cache.getEmployeeLoans().find((l) => l.id === loanId)),
    scheduleLines: saved.filter((l) => l.status !== "cancelled"),
    skipped: result.skipped,
    deferred: result.deferred,
  };
}

async function setLoanStartMonth(loanId, startYearMonth, username, { skipLoanUpdate } = {}) {
  const loan = cache.getEmployeeLoans().find((l) => l.id === loanId);
  if (!loan) throw new Error("Loan not found");
  const ym = String(startYearMonth || "").trim();
  if (!/^\d{4}-\d{2}$/.test(ym)) throw new Error("startYearMonth must be YYYY-MM");

  let updatedLoan = loan;
  if (!skipLoanUpdate && loan.startYearMonth !== ym) {
    updatedLoan = await backend.updateEmployeeLoan({ ...loan, startYearMonth: ym });
    cache.upsertEmployeeLoan(updatedLoan);
  }

  const payments = cache.getAllLoanPayments().filter((p) => p.loanId === loanId);
  const existing = cache.getLoanScheduleLinesForLoan(loanId);
  const emp = cache.getEmployees({ hideOut: false }).find((e) => e.id === loan.employeeId);
  const result = shiftScheduleStartMonth(updatedLoan, existing, ym, payments, {
    unit: emp?.unit || null,
  });
  const saved = await persistLoanScheduleForLoan(loanId, result.lines);
  await changelog.logEmployeeChange(
...
async function regenerateLoanSchedule(loanId, username, opts = {}) {
  const loan = cache.getEmployeeLoans().find((l) => l.id === loanId);
  if (!loan) throw new Error("Loan not found");
  const payments = cache.getAllLoanPayments().filter((p) => p.loanId === loanId);
  const existing = cache.getLoanScheduleLinesForLoan(loanId);
  ...
  const result = regenerateSchedule(loan, existing, payments, {
    startYearMonth,
    unit: emp?.unit || null,
  });
  const saved = await persistLoanScheduleForLoan(loanId, result.lines);
...
async function backfillLoanSchedules({ loanId, force = false } = {}) {
  const loans = loanId
    ? cache.getEmployeeLoans().filter((l) => l.id === loanId)
    : cache.getEmployeeLoans();
  const results = [];
  for (const loan of loans) {
    const existing = cache.getLoanScheduleLinesForLoan(loan.id);
    if (existing.length && !force) {
      results.push({ loanId: loan.id, skipped: true, reason: "already has schedule" });
      continue;
    }
    const payments = cache.getAllLoanPayments().filter((p) => p.loanId === loan.id);
    const emp = cache.getEmployees({ hideOut: false }).find((e) => e.id === loan.employeeId);
    const lines = backfillScheduleFromLoan(loan, payments, { unit: emp?.unit || null });
    const saved = await persistLoanScheduleForLoan(loan.id, lines);
    results.push({ loanId: loan.id, skipped: false, count: saved.length });
  }
  return results;
}
...
module.exports = {
  ...
  refreshCache,
  refreshEmployeesFromSupabase,
  ensureEmployeesFresh,
  markEmployeesFresh,
  getEmployees,
  ...
  upsertLoanPaymentOverride,
  removeLoanPaymentOverride,
  enrichLoanWithBalance,
  getLoanSchedule,
  skipLoanScheduleMonth,
  setLoanStartMonth,
  regenerateLoanSchedule,
  backfillLoanSchedules,
  addEmployeeWarning,
