...
function isPayrollEligible(emp) {
  ...
  if (emp.payroll_exempt) return false;
  return PAYROLL_ACTIVE_STATUSES.has(emp.status);
}

function isPayrollEligibleForMonth(emp, yearMonth, records = []) {
  if (!emp?.id) return false;
  if (emp.payroll_exempt) return false;
  if (PAYROLL_ACTIVE_STATUSES.has(emp.status)) return true;
  if (isOutStatus(emp.status) && employeeWorkedInMonth(emp, records, yearMonth)) return true;
  return false;
}
...
/** Half/quarter salary deductions apply only to unpaid leave (not annual paid fractions). */
function countUnpaidFractionDeductions(records = []) {
  let unpaidHalfDays = 0;
  let unpaidQuarterOff = 0;
  for (const r of records) {
    if (r.status === "Half Day" && !r.paidLeave) unpaidHalfDays += 1;
    if (r.status === "Quarter Day-Off" && !r.paidLeave) unpaidQuarterOff += 1;
  }
  return { unpaidHalfDays, unpaidQuarterOff };
}
...
function summarizeEmployeeMonth(emp, records, config, actionPlans = []) {
  ...
  const wfh = countStatus(records, "WFH");
  const outDays = records.filter((r) => r.status === "OUT" || r.status === "OUT BUT STILL GET PAID").length;
  const { amount, detail, aipNotes = [] } = calcLatenessDeduction(records, config, actionPlans);

  const paused = countStatus(records, "paused");
  ...
  return {
    ...
    unit: emp.unit,
    email: emp.email,
    workingDays: attended + paidLeaveDays + halfDays + quarterOff + lateness + wfh,
    paidLeaveDays,
    daysOff: countStatus(records, "Day-OFF") + paused,
    halfDays,
    quarterOff,
    wfh,
    lateness,
