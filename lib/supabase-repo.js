...
async function replaceLoanScheduleLinesForLoan(loanId, lines) {
  const { error: delErr } = await db().from("loan_schedule_lines").delete().eq("loan_id", loanId);
  if (delErr) throwDb(delErr, "replaceLoanScheduleLinesForLoan(delete)");
  if (!lines?.length) return [];

  // Two-pass insert: self-FKs (deferred_from/to) require targets to exist first.
  ...
  const { error: insErr } = await db().from("loan_schedule_lines").insert(rows);
  if (insErr) throwDb(insErr, "replaceLoanScheduleLinesForLoan(insert)");

  const withLinks = lines.filter((l) => l.deferredFromLineId || l.deferredToLineId);
  if (withLinks.length) {
