...
async function readPositionRatesForMonth(yearMonth) {
  ...
  const { data, error } = await db()
    ...
    .order("position");
  if (error) {
    if (isMissingTableError(error)) return readPositionRates();
    throwDb(error, "readPositionRatesForMonth");
  }
  if (!data?.length) return readPositionRates();
  return data.map((r) => ({
    position: r.position,
    monthlySalary: Number(r.monthly_salary) || 0,
  }));
...
async function upsertPositionRateForMonth(yearMonth, position, monthlySalary) {
  const ym = String(yearMonth || "").slice(0, 7);
  const row = {
    year_month: ym,
    position,
  ...
  await upsertPositionRate(position, monthlySalary);
  return { position, monthlySalary: Number(monthlySalary) || 0, yearMonth: ym };
}

async function deletePositionRateForMonth(yearMonth, position) {
  const ym = String(yearMonth || "").slice(0, 7);
