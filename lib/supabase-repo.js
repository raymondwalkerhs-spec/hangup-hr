...
async function appendEmployeeLoan(loan, createdBy = "system") {
  ...
  const row = {
    ...
    paused: m.boolToDb(loan.paused === true),
...
async function appendLoanPayment(payment, recordedBy = "system") {
  const row = {
    ...
    schedule_line_id: payment.scheduleLineId || null,
  ...
  return {
    ...
    scheduleLineId: row.schedule_line_id,
    recordedBy: row.recorded_by,
    recordedAt: row.recorded_at,
  };
}

async function readAllLoanMonthOverrides() {
  const { data, error } = await db().from("loan_month_overrides").select("*");
  if (error) throwDb(error, "readAllLoanMonthOverrides");
  return (data || []).map(m.mapLoanMonthOverrideFromDb);
}

async function upsertLoanMonthOverride(override, createdBy = "system") {
  const loanId = override.loanId;
  const yearMonth = String(override.yearMonth || "").trim();
  const amount = Number(override.amount);
  if (!loanId || !/^\d{4}-\d{2}$/.test(yearMonth)) {
  ...
  const { data, error } = await db()
    .from("loan_month_overrides")
    .upsert(row, { onConflict: "loan_id,year_month" })
    .select("*")
    .single();
  if (error) throwDb(error, "upsertLoanMonthOverride");
  return m.mapLoanMonthOverrideFromDb(data);
}

async function deleteLoanMonthOverride(loanId, yearMonth) {
  const { error } = await db()
    .from("loan_month_overrides")
    .delete()
    .eq("loan_id", loanId)
    .eq("year_month", yearMonth);
  if (error) throwDb(error, "deleteLoanMonthOverride");
  return true;
}

async function readAllLoanScheduleLines() {
  const { data, error } = await db().from("loan_schedule_lines").select("*");
  if (error) throwDb(error, "readAllLoanScheduleLines");
  return (data || []).map(m.mapLoanScheduleLineFromDb);
}

async function readLoanScheduleLinesForLoan(loanId) {
  const { data, error } = await db()
    .from("loan_schedule_lines")
    .select("*")
    .eq("loan_id", loanId)
    .order("seq", { ascending: true });
  if (error) throwDb(error, "readLoanScheduleLinesForLoan");
  return (data || []).map(m.mapLoanScheduleLineFromDb);
}

async function replaceLoanScheduleLinesForLoan(loanId, lines) {
  // Upsert by id so loan_payments.schedule_line_id FKs are preserved (no delete-all).
  const incoming = lines || [];
  const keepIds = new Set(incoming.map((l) => l.id).filter(Boolean));

  const { data: existing, error: exErr } = await db()
    .from("loan_schedule_lines")
    .select("id")
    .eq("loan_id", loanId);
  if (exErr) throwDb(exErr, "replaceLoanScheduleLinesForLoan(existing)");

  const toDelete = (existing || []).map((r) => r.id).filter((id) => !keepIds.has(id));
  if (toDelete.length) {
    const { error: delErr } = await db().from("loan_schedule_lines").delete().in("id", toDelete);
    if (delErr) throwDb(delErr, "replaceLoanScheduleLinesForLoan(deleteOrphans)");
  }

  if (!incoming.length) return [];

  // Two-pass upsert: clear self-FKs first, then set deferred links.
  const baseRows = incoming.map((line) => {
    const row = m.loanScheduleLineToDb({ ...line, loanId });
    if (!row.created_at) row.created_at = new Date().toISOString();
    return {
      ...row,
      deferred_from_line_id: null,
      deferred_to_line_id: null,
    };
  });
  const { error: upsErr } = await db()
    .from("loan_schedule_lines")
    .upsert(baseRows, { onConflict: "id" });
  if (upsErr) throwDb(upsErr, "replaceLoanScheduleLinesForLoan(upsert)");

  const withLinks = incoming.filter((l) => l.deferredFromLineId || l.deferredToLineId);
  if (withLinks.length) {
    const linkRows = withLinks.map((line) => m.loanScheduleLineToDb({ ...line, loanId }));
    const { error: linkErr } = await db()
      .from("loan_schedule_lines")
      .upsert(linkRows, { onConflict: "id" });
    if (linkErr) throwDb(linkErr, "replaceLoanScheduleLinesForLoan(links)");
  }
  ...
  const { data, error } = await db()
    ...
    .select("*")
    .eq("loan_id", loanId);
  if (error) throwDb(error, "replaceLoanScheduleLinesForLoan(select)");
  return (data || []).map(m.mapLoanScheduleLineFromDb);
}

async function upsertLoanScheduleLines(lines) {
  if (!lines?.length) return [];
  const rows = lines.map((line) => {
    const row = m.loanScheduleLineToDb(line);
    if (!row.created_at) row.created_at = new Date().toISOString();
    return row;
  });
  const { data, error } = await db()
    ...
    .select("*");
  if (error) throwDb(error, "upsertLoanScheduleLines");
  return (data || []).map(m.mapLoanScheduleLineFromDb);
}
...
module.exports = {
  ...
  readAllLoanPayments,
  appendLoanPayment,
  readAllLoanMonthOverrides,
  upsertLoanMonthOverride,
  deleteLoanMonthOverride,
  readAllLoanScheduleLines,
  readLoanScheduleLinesForLoan,
  replaceLoanScheduleLinesForLoan,
  upsertLoanScheduleLines,
