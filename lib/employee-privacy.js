...
function canSeePaymentMethod(userRole) {
  if (!userRole) return false;
  const role = String(userRole.role || "").toLowerCase();
  // TL must not see payment type on employee records
  if (role === "tl") return false;
  return true;
}
...
function sanitizeEmployee(emp, userRole) {
  ...
  if (!canSeePaymentMethod(userRole)) {
    delete copy.payment_method;
    delete copy.paymentMethod;
    delete copy.payment_details_insta_wallet;
    delete copy.payment_details_bank;
