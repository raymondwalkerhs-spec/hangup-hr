const { normalizePaymentMethodValue, resolvePaymentMethod } = require("./hr-constants");
...
function lookupSalary(position, rates, yearMonth = null) {
  if (!position) return 0;
  const key = String(position).trim();
  const list = rates || [];
  const exact = list.find((r) => r.position === key);
  if (exact) return exact.monthlySalary;
  const loose = list.find((r) => r.position.toLowerCase() === key.toLowerCase());
...
function buildDefaultProfile(emp, yearMonth) {
  return {
    ...
    paymentMethod: resolvePaymentMethod(emp, null),
...
function resolveEmployeeForMonth(emp, profile, rates, yearMonth) {
  ...
  const position = String(profile?.position || emp.position || "").trim();
  
  // Determine the monthly salary with proper override handling
  const baseFromPosition = lookupSalary(position, rates);
  const hasMonthlyOverride = profile?.monthlySalaryOverride != null && 
                              profile?.monthlySalaryOverride !== "" && 
                              !Number.isNaN(Number(profile.monthlySalaryOverride));
  const baseSalary = hasMonthlyOverride 
    ? Number(profile.monthlySalaryOverride)
    : baseFromPosition;
  ...
  return {
    ...
    position,
    unit: emp.unit,
    payment_method: resolvePaymentMethod(emp, profile),
