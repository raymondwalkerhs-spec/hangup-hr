/**
 * DB-backed RPM sales attachment kind permissions.
 */
...
async function loadMap() {
  ...
  const { data, error } = await db().from("rpm_sales_attachment_permissions").select("*");
  if (error) {
...
async function upsert(attachmentKey, patch) {
  ...
  const { data, error } = await db()
    .from("rpm_sales_attachment_permissions")
