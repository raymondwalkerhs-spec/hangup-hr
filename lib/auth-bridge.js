...
/**
 * Self-service forgot password: username + Authenticator TOTP → new password.
 * No email. Admin Users → Reset password remains for lost-phone / no-MFA recovery.
 */
async function resetPasswordWithTotp(username, totpCode, newPassword) {
  ...
  if (!sync.synced && !sync.skipped) {
    authDebugError("resetPasswordWithTotp.syncPasswordToAuth", new Error(sync.error || "sync failed"), {
