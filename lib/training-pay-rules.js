...
function computeEligibleTrainingPayDates(program, attendance, ym) {
  ...
  if (outcome === "voluntary_leave") return new Set();
  if (outcome === "failed" && rejectedAt === 1) return new Set();
  if (outcome === "company_terminated" && rejectedAt != null && rejectedAt <= 2) return new Set();
  ...
  if (outcome === "failed" && rejectedAt === 2) {
    ...
    if (!ph2) return new Set();
    const ws = ph2.weekStart || ph2.week_start;
    const we = ph2.weekEnd || ph2.week_end;
    return new Set(ymDates.filter((d) => dateInRange(d, ws, we) && worked.has(d)));
  }
  ...
  if (outcome === "company_terminated" && rejectedAt != null && rejectedAt >= 3) {
    ...
    return new Set(ymDates.filter((d) => d >= from && d <= to && worked.has(d)));
  }
...
function trainingPayPreview(program, attendance, ym, traineeDailyRate) {
  ...
  const trainingBasic = Math.round(trainingDays.size * traineeDailyRate * 100) / 100;
  return {
