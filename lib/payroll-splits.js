const SPLIT_KINDS = ["payment", "correction"];
...
function validateSplit(split, payslipContext = null) {
  ...
  if (!SPLIT_KINDS.includes(split.splitKind || "payment")) {
    return "Invalid split kind";
  }
  if (split.splitKind === "payment" && !(amount > 0)) {
    return "Payment split amount must be greater than 0";
  }
