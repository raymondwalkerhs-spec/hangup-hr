...
async function createBonusRequest(payload, actor) {
  ...
  const mapped = mapBonusRequest(data);
  cache.upsertBonusRequestInBusinessCache(mapped);
  return mapped;
}
...
function mapSale(r) {
  ...
  return {
    ...
    price: r.price != null ? (isNaN(Number(r.price)) ? String(r.price) : Number(r.price)) : formData.price != null ? (isNaN(Number(formData.price)) ? String(formData.price) : Number(formData.price)) : null,
    ...
    // Price tier label — from DB column (backfilled) or form_data catalog reference label
    priceTierLabel: r.price_tier_label || formData.priceTierLabel || "",
    formData: {
      ...formData,
      // Inject priceTier into formData so the field catalog can render it on quality surface
      priceTier: r.price_tier_label || formData.priceTierLabel || formData.priceTier || "",
    },
  };
}

async function setSaleAirtableMeta(saleId, { recordId, syncedAt, error }) {
  requireSupabase();
  const row = {};
  ...
  const { error: dbErr } = await db().from("sales").update(row).eq("id", saleId);
...
function saleInDateRange(sale, from, to, opts = {}) {
  ...
  const sub = String(sale.submissionDate || "").slice(0, 10);
  const wd =
    String(sale.workingDay || "").slice(0, 10) ||
    workingDay.computeWorkingDay(sale.submissionDate) ||
    sub;
  if (basis === "workingDay") {
...
function saleSubmissionSortKey(sale) {
  const date = String(sale.submissionDate || sale.effectiveDate || "").slice(0, 10);
  const m = String(sale.submissionTime || "").match(/^(\d{1,2}):(\d{2})/);
...
function sortSalesBySubmissionTime(sales) {
  return (sales || []).slice().sort((a, b) => saleSubmissionSortKey(b) - saleSubmissionSortKey(a));
}
...
function filterSales(list, filters = {}) {
  ...
  if (filters.client) rows = rows.filter((s) => String(s.client || "") === String(filters.client));
  ...
  return sortSalesBySubmissionTime(rows);
}
...
async function createSale(payload, actor) {
  ...
  // Persist price tier label so it survives quality ticket surface
  if (payload.priceTierLabel) row.price_tier_label = payload.priceTierLabel;
  else if (payload.formData?.priceTierLabel) row.price_tier_label = payload.formData.priceTierLabel;
  else if (payload.formData?.priceTier) row.price_tier_label = payload.formData.priceTier;
  let result = await db().from("sales").insert(row).select().single();
  if (result.error && row.price_tier_label && /price_tier_label|column.*not.*exist/i.test(result.error.message)) {
    delete row.price_tier_label;
    result = await db().from("sales").insert(row).select().single();
  }
  if (result.error) throw new Error(result.error.message);
  const sale = mapSale(result.data);
  cache.upsertSaleInBusinessCache(sale);
  return sale;
}
...
async function updateSale(id, patch, actor) {
  ...
  if (patch.submissionDate !== undefined || patch.submissionTime !== undefined || patch.workingDay !== undefined) {
    if (patch.submissionDate !== undefined) {
      ...
      if (patch.workingDay !== undefined) row.working_day = patch.workingDay;
    }
  }
  ...
  let result = await db().from("sales").update(row).eq("id", id).select().single();
  if (result.error && row.price_tier_label && /price_tier_label|column.*not.*exist/i.test(result.error.message)) {
    delete row.price_tier_label;
    result = await db().from("sales").update(row).eq("id", id).select().single();
  }
  if (result.error) throw new Error(result.error.message);
  const sale = mapSale(result.data);
  cache.upsertSaleInBusinessCache(sale);
...
async function getSalesVisibilityGrant(id) {
  requireSupabase();
  const { data, error } = await db().from("sales_visibility_grants").select("*").eq("id", id).maybeSingle();
  if (error) throw new Error(error.message);
...
const PAYMENT_METHODS = ["petty_cash", "main_fund", "instapay", "cash", "wallet", "own_pocket"];
const COST_PAID_BY_OPTIONS = [
  { value: "petty_cash", label: "Petty cash" },
  { value: "main_fund", label: "Main Fund" },
];

function paidByLabel(paymentMethod) {
  const m = String(paymentMethod || "").toLowerCase();
  if (m === "petty_cash") return "Petty cash";
  if (m === "main_fund") return "Main Fund";
  if (!m) return "—";
...
function mapExpense(r) {
  ...
  return {
    ...
    paidBy: r.paid_by || "",
    paidByLabel: paidByLabel(r.payment_method || r.paid_by),
...
function filterExpenseRequests(list, filters = {}) {
  ...
  if (filters.company) rows = rows.filter((r) => (r.company || "hangup") === filters.company);
  return rows;
}
...
async function createExpenseRequest(payload, actor) {
  ...
  let employeeId = actor;
  try {
    employeeId = store.getAppUserEmployeeId(actor) || actor;
  } catch {
    employeeId = actor;
  }
  let emp = null;
  try {
    emp = store.getEmployeeById(employeeId);
  } catch {
    emp = null;
  }
  const company =
    payload.company || (emp ? companyContext.getCompanyForUnit(emp.unit) : "hangup");
  ...
  const row = {
    ...
    company,
    category,
    unit: payload.unit || emp?.unit || null,
    updated_at: new Date().toISOString(),
  };
  // Optional columns that exist in some environments only
  if (payload.employeeId || (employeeId && employeeId !== actor)) {
    row.employee_id = payload.employeeId || employeeId;
  }
  let { data, error } = await db().from("expense_requests").insert(row).select().single();
  if (error && /employee_id/i.test(error.message)) {
    delete row.employee_id;
    ({ data, error } = await db().from("expense_requests").insert(row).select().single());
  }
  if (error && /category/i.test(error.message)) {
    delete row.category;
    ({ data, error } = await db().from("expense_requests").insert(row).select().single());
  }
  if (error && /paid_at|paid_by/i.test(error.message)) {
    delete row.paid_at;
    delete row.paid_by;
    ({ data, error } = await db().from("expense_requests").insert(row).select().single());
  }
...
async function updateExpenseRequest(id, patch, actor) {
  ...
  // Handle createdAt separately — convert date string to full ISO timestamp
  if (patch.createdAt !== undefined && patch.createdAt) {
  ...
  if (patch.status === "paid") {
    ...
    else if (!row.paid_by) row.paid_by = actor;
    // Default paid-by source when marking paid without an explicit method
    if (patch.paymentMethod === undefined && row.payment_method === undefined) {
      row.payment_method = "petty_cash";
      if (!row.paid_by || row.paid_by === actor) row.paid_by = "Petty cash";
    }
  }
  if (patch.status && patch.status !== "paid" && patch.status !== "archived") {
    // Allow Edit to move a paid cost back to pending / on_hold
    if (patch.paidAt === undefined) row.paid_at = null;
  }
  // Normalize paid-by labels for the two cost funding sources
  if (row.payment_method === "petty_cash" && (patch.paidBy === undefined || !patch.paidBy)) {
    row.paid_by = "Petty cash";
  }
  if (row.payment_method === "main_fund") {
    row.paid_by = "Main Fund";
    row.petty_cash_fund_id = null;
  }
...
async function getPettyCashFunds(company) {
  requireSupabase();
  let q = db().from("petty_cash_funds").select("*").order("fund_name");
  if (company) q = q.eq("company", company);
  const { data, error } = await q;
  if (error) {
    if (isMissingTableError(error)) return [];
    if (isMissingColumnError(error)) return (data || []).map((r) => ({
      id: r.id,
...
async function getPettyCashLedger(fundId, limit = 100, company) {
  requireSupabase();
...
async function addPettyCashTransaction(
  { fundId, transactionType, amount, notes, linkedExpenseId, createdAt },
  actor
) {
  requireSupabase();
  ...
  const ledgerRow = {
    ...
    notes: notes || "",
    created_by: actor,
    company: fund.company || "hangup",
  };
  if (createdAt) {
    const d = new Date(createdAt);
    if (!Number.isNaN(d.getTime())) ledgerRow.created_at = d.toISOString();
  }
...
async function readMonthlyBills(opts = {}) {
  ...
  if (!opts.skipCache && cache.isCacheWarm()) {
    ...
    if (cached) {
      ...
      return company ? cached.filter((b) => (b.company || "hangup") === company) : cached;
    }
  }
  let q = db().from("monthly_bills").select("*").order("starred", { ascending: false });
  if (opts.company) q = q.eq("company", opts.company);
  const { data, error } = await q;
  if (error) {
    if (isMissingTableError(error)) return [];
    if (isMissingColumnError(error)) return (data || []).map(mapBill);
    throw new Error(error.message);
  }
...
async function upsertMonthlyBill(payload, actor) {
  ...
  const row = {
    ...
    created_by: actor,
    company: payload.company || "hangup",
    updated_at: new Date().toISOString(),
  };
  if (payload.id) {
...
async function readRpmSalesFieldPermissions() {
  requireSupabase();
  const { data, error } = await db().from("rpm_sales_field_permissions").select("*").order("display_order");
  if (error) {
    if (isMissingTableError(error)) return [];
    throw new Error(error.message);
  }
...
async function upsertRpmSalesFieldPermission(fieldKey, patch) {
  requireSupabase();
  const row = {
    field_key: fieldKey,
    label: patch.label || fieldKey,
  ...
  const { data, error } = await db()
    .from("rpm_sales_field_permissions")
    .upsert(row, { onConflict: "field_key" })
    .select()
    .single();
  if (error) throw new Error(error.message);
...
async function deleteSaleAttachment(id, actor) {
  requireSupabase();
  const { data, error } = await db().from("sales_attachments").select("*").eq("id", id).maybeSingle();
  if (error) throw new Error(error.message);
  if (!data) return null;
...
async function findRecentDuplicateSale({ phoneNumber, agentId, ignoreAgent, workingDay }) {
  requireSupabase();
  ...
  if (ignoreAgent) {
    ...
    const { data, error } = await db()
      .from("sales")
      .select("id, phone_number, agent_id, working_day, created_at")
      .eq("working_day", day)
      .order("created_at", { ascending: false })
      .limit(80);
    if (error) throw new Error(error.message);
    const match = (data || []).find((r) => normalizePhoneDigits(r.phone_number) === norm);
    return match ? mapSale(match) : null;
  }
...
async function deleteSaleCompletely(id) {
  requireSupabase();
  const sale = await getSale(id);
  if (!sale) throw new Error("Sale not found");
  const airtableSync = require("./airtable-sales-sync");
  airtableSync.cancelSaleSync(id);
  const saleStorage = require("./sale-attachment-storage");
  const attachments = await readSaleAttachments(id);
  for (const att of attachments) {
    ...
    await deleteSaleAttachment(att.id);
  }
  const airtable = require("./airtable-client");
  if (airtable.isConfigured()) {
    await airtableSync.deleteSaleFromAirtable(sale);
  }
  const { error } = await db().from("sales").delete().eq("id", id);
  if (error) throw new Error(error.message);
  cache.removeSaleFromBusinessCache(id);
  return { ok: true, id };
}
...
module.exports = {
  ...
  readRpmSalesFieldPermissions,
  upsertRpmSalesFieldPermission,
  readSaleAttachments,
  getSaleAttachment,
