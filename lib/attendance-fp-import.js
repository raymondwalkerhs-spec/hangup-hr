...
/** True when an existing day should not be replaced by FP import.
 *  Manual edits ALWAYS win. FP import may only touch completely blank days. */
function isProtectedAttendanceRecord(existing) {
  if (!existing) return false;
  const status = String(existing.status || "").trim();
  const leaveNote = String(existing.leaveNote || "").trim();
  const fpNotes = String(existing.fpNotes || "").trim();
  const transportOverride = String(existing.transportOverride || "").trim();
  const paidLeave = Boolean(existing.paidLeave);

  // Any manual or previously-imported data makes this record protected.
  // FP import is ONLY allowed on completely blank days.
  if (status) return true;
  if (leaveNote) return true;
  if (fpNotes) return true;
  if (transportOverride) return true;
  if (paidLeave) return true;
  return false;
}

function processImport({ buffer, employees, rules, month, existingRecords = [], overwritePolicy = "skip_manual" }) {
  const { isLockedDepartDay } = require("./depart-attendance");
  ...
  for (const g of grouped) {
    ...
    if (emp && isLockedDepartDay(emp, g.date, null)) {
      skipped++;
      preview.push({
        employeeId,
        fpNumber: g.fpNumber,
        date: g.date,
        status: "skipped",
        proposed: "locked_post_depart",
        existing: "OUT (depart lock)",
        checkIn: formatMinutes(g.checkIn),
        checkOut: formatMinutes(g.checkOut),
      });
      continue;
