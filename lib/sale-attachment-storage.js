/**
 * Sale attachments — Supabase Storage only (MLA and RPM use separate roots).
 */
...
function isSupabaseStoragePath(storagePath) {
  return programStorage.isSaleProgramStoragePath(storagePath);
}

function saleObjectPath(saleId, kind, fileName) {
  return programStorage.mlaSaleObjectPath(saleId, kind, fileName);
}

function rpmSaleObjectPath(saleId, kind, fileName) {
  return programStorage.rpmSaleObjectPath(saleId, kind, fileName);
}
...
async function uploadRpmSaleAttachmentBuffer({ saleId, kind, fileName, buffer }) {
  if (!isConfigured()) throw new Error("Supabase not configured");
  const objectPath = rpmSaleObjectPath(saleId, kind, fileName);
  const mimeType = guessMime(fileName);
  const { error } = await getSupabaseAdmin().storage.from(BUCKET).upload(objectPath, buffer, {
    upsert: true,
    contentType: mimeType,
