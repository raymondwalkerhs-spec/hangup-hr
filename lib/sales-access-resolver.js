...
function canEditVerifierFeedback(user, sale, dbPerm = null) {
  ...
  const editRoles = permRoles(dbPerm, "edit_roles", "editRoles") || field.editRoles || [];
...
function canEditClientFeedback(user, dbPerm = null) {
  ...
  if (!field) return false;
  const editRoles = permRoles(dbPerm, "edit_roles", "editRoles") || field.editRoles || ["admin", "ceo", "rtm"];
...
function canEditFieldOnSurface(field, role, dbPerm = null, surface = "main", opts = {}) {
  ...
  const editRoles = permRoles(dbPerm, "edit_roles", "editRoles") || field.editRoles || DEFAULT_EDIT;
