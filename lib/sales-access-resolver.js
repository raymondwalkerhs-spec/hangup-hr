...
const QUALITY_ALWAYS_HIDDEN = new Set([
  ...
  "unit",
  "team",
  // NOTE: "price" removed — priceTier (label) and price (number) now visible on quality surface
]);

/** Non-quality-section fields that should appear on the quality ticket surface. */
const QUALITY_EXTRA_FIELDS = new Set(["monthlyBillingDate", "billingDate", "price", "priceTier"]);
...
function rolesForSurface(dbPerm, field, surface) {
  ...
  if (surface === "quality") {
    ...
    if (QUALITY_EXTRA_FIELDS.has(field.key)) return QUALITY_ROLES;
    return [];
  }
  if (surface === "edit") {
...
function canEditFieldOnSurface(field, role, dbPerm = null, surface = "main", opts = {}) {
  ...
  const catalogEdit =
    field.section === "quality" ? field.editRoles || QUALITY_ROLES : field.editRoles || DEFAULT_EDIT;
  const editRoles = rolesFromPerm(dbPerm, "edit_roles", "editRoles", catalogEdit);
  let canEdit = roleInList(role, editRoles);
  if (
    !canEdit &&
    surface === "quality" &&
    (field.section === "quality" || QUALITY_EXTRA_FIELDS.has(field.key)) &&
