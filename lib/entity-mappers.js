...
function mapEmployeeRow(r) {
  return {
    ...
    payment_method: normalizePaymentMethodValue(r["Payment Method"] || r.payment_method) || null,
    ...
    national_id: r.national_id || r["National ID"] || r.identification || null,
    passport_number: r.passport_number || r["Passport Number"] || null,
    nationality: r.Nationality || r.nationality || null,
    training_passed: r.training_passed === true || r.training_passed === "true",
    ...
    wfh: r.wfh === true || r.wfh === "true",
    sales_mla_enabled:
      r.sales_mla_enabled === true ||
      r.sales_mla_enabled === "true" ||
      r.salesMlaEnabled === true,
    sales_rpm_enabled:
      r.sales_rpm_enabled === true ||
      r.sales_rpm_enabled === "true" ||
      r.salesRpmEnabled === true,
    can_submit_checks_q_feedback:
      r.can_submit_checks_q_feedback === true ||
      r.can_submit_checks_q_feedback === "true" ||
      r.canSubmitChecksQFeedback === true,
    checks_submit_scope:
      String(r.checks_submit_scope || r.checksSubmitScope || "self").toLowerCase() === "team"
        ? "team"
        : "self",
    team_dashboard_extra_teams: normalizeExtraTeams(
      r.team_dashboard_extra_teams ?? r.teamDashboardExtraTeams
    ),
    sales_agent_picker:
      r.sales_agent_picker === true || r.sales_agent_picker === "true" || r.salesAgentPicker === true
        ? true
        : r.sales_agent_picker === false || r.sales_agent_picker === "false" || r.salesAgentPicker === false
          ? false
          : null,
...
function normalizeExtraTeams(raw) {
