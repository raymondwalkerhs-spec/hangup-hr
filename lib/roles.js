...
function canGrantTransferBonus(userRole, recipient, giverEmp) {
  ...
  const bonusScope = require("./bonus-scope");
  const employees = store.getEmployees({ hideOut: false }) || [];
  return bonusScope.canGrantBonusTransfer(userRole, recipient, giverEmp, employees);
}

function isLeadershipEmployeeId(employeeId) {
  const employeeIds = require("./employee-ids");
  // Keep legacy HR/TL/CL/OP match for payslip-only checks, plus transfer-payer IDs.
  if (/^(TL|CL|OP|HR)/i.test(String(employeeId || "").trim())) return true;
  return employeeIds.isBonusTransferPayerId(employeeId);
}
...
function canViewOfficePo(userRole) {
  return perm("viewOfficePo", userRole, () =>
    ["hr", "admin", "ceo", "finance", "op", "rtm"].includes(normalizeRole(userRole?.role))
  );
}

function canManageOfficePoItems(userRole) {
  return perm("manageOfficePoItems", userRole, () => MANAGE_ROLES.includes(userRole?.role) || ADMIN_ROLES.includes(userRole?.role));
}

function canEditOfficePoPurchases(userRole) {
  return perm("editOfficePoPurchases", userRole, () =>
    ["hr", "admin", "ceo", "finance"].includes(normalizeRole(userRole?.role))
  );
}
...
module.exports = {
  ...
  canSubmitExpense,
  canViewOfficePo,
  canManageOfficePoItems,
  canEditOfficePoPurchases,
  canViewSales,
