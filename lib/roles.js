...
function enrichUserRole(userRole, employees, appUser = null) {
...
function canEditAttendance(userRole) {
  return perm("editAttendance", userRole, () => {
    if (MANAGE_ROLES.includes(userRole.role)) return true;
...
function canSubmitSales(userRole) {
  return perm("submitSales", userRole, () => {
    ...
    return ["agent", "tl", "op", "admin", "ceo", "hr", "rtm", "quality"].includes(role);
  });
}
...
const NOTES_WRITE_ROLES = ["hr", "admin", "ceo"];
...
function canViewOrgFull(userRole) {
  return perm("viewOrgFull", userRole, () =>
    ...
    ["tl", "op", "rtm", "quality"].includes(userRole?.role)
  );
}

function canViewOrgAgentScope(userRole) {
  return userRole?.role === "agent" || userRole?.role === "office_assistant";
}
...
function canExportSales(userRole) {
  ...
  return perm("exportSales", userRole, () => {
    if (userRole.role === "agent" || userRole.role === "office_assistant") return false;
    return hasAppAccess(userRole);
  });
}

function canViewEquipment(userRole) {
  return perm("viewEquipment", userRole, () => EQUIPMENT_VIEW_ROLES.includes(userRole?.role));
}
...
function canManageSalesFieldPermissions(userRole) {
  return perm("manageSalesFieldPermissions", userRole, () =>
    ["admin", "ceo", "rtm", "hr"].includes(userRole?.role)
  );
}
...
function canUseEmployeeFilters(userRole) {
  return perm("useEmployeeFilters", userRole, () => {
    if (userRole?.role === "agent" || userRole?.role === "office_assistant") return false;
    return true;
  });
}
