...
/**
 * Leave approval: named executives OR any user with hr/admin/ceo role.
 * Accepts either (username) or (username, userRole) to support both call sites.
 */
function canApproveLeave(username, userRole) {
  const uname = String(username || "").trim().toLowerCase();
  if (LEAVE_APPROVERS.includes(uname)) return true;
  // Also allow by role when userRole is provided
...
function canManageHs2Company(userRole) {
  return perm("manageHs2Company", userRole, () =>
    ["admin", "ceo", "hr"].includes(normalizeRole(userRole?.role))
  );
}
...
function canAccessRulesCompany(userRole, company) {
  ...
  if (normalizedCompany === "hs2" || normalizedCompany === "hs-2") {
    return canManageHs2Company(userRole) || canEditRules(userRole) || normalizeRole(userRole?.role) === "quality";
  }
  return true;
}
...
function canViewItRequests(userRole) {
  return perm("viewItRequests", userRole, () =>
    IT_REQUEST_ROLES.includes(normalizeRole(userRole?.role)) || Boolean(userRole?.employeeId)
  );
}
...
function canAssignItRequest(userRole) {
  return perm("assignItRequest", userRole, () => IT_REQUEST_ASSIGN_ROLES.includes(normalizeRole(userRole?.role)));
}

function canResolveItRequest(userRole) {
  return perm("resolveItRequest", userRole, () => IT_REQUEST_ROLES.includes(normalizeRole(userRole?.role)));
}
