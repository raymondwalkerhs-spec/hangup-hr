...
function filterEmployeesForTeamDashboard(employees, userRole) {
  const base = filterEmployeesForUser(employees, userRole);
  if (!hasCloserTeamAssignment(userRole)) return base;
  const ids = new Set(base.map((e) => e.id));
  const extra = [];
  for (const e of employees || []) {
    if (!ids.has(e.id) && isCloserTeamMember(userRole, e)) {
      ids.add(e.id);
      extra.push(e);
    }
  }
...
async function enrichUserRoleWithOrgTeams(userRole, employees, appUser = null) {
  const [orgTeams, unitOps] = await Promise.all([loadOrgTeamsForScope(), loadUnitOpsForScope()]);
  const enriched = enrichUserRole(userRole, employees, appUser, orgTeams);
  return attachOpUnits(enriched, unitOps);
}
