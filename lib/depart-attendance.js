...
function applyDepartAutoOut(emp, records, yearMonth) {
  const depart = departDateStr(emp);
  if (!depart) return records;
  const ym = String(yearMonth).slice(0, 7);
  if (!/^\d{4}-\d{2}$/.test(ym)) return records;
  // Paint every day after depart in the viewed month (any month on/after depart month).
  if (ym < depart.slice(0, 7)) return records;
...
function shouldShowInMonth(emp, yearMonth, records, { hideOut = true, hideAllOut = false, showLegacyEmployees = false } = {}) {
  const { shouldShowEmployeeInMonth } = require("./employee-visibility");
  return shouldShowEmployeeInMonth(emp, yearMonth, records, { hideOut, hideAllOut, showLegacyEmployees });
}
