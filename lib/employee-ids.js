...
function collectReservedAppIds(employees) {
  const reserved = new Set();
  const { isDeletedEmployee } = require("./employee-identity");
  for (const emp of employees || []) {
    if (!emp) continue;
    const deleted = isDeletedEmployee(emp);
    for (const id of [emp.id, emp.promoted_from_id, emp.promoted_to_id]) {
      if (id) reserved.add(String(id).trim().toUpperCase());
    }
    if (deleted) continue;
    if (emp.archived_app_id) reserved.add(String(emp.archived_app_id).trim().toUpperCase());
    for (const id of parseFormerIds(emp.former_ids)) {
      if (id) reserved.add(String(id).trim().toUpperCase());
    }
  }
  return reserved;
 ...
 * Includes dialing leads (TL/CL/OP), backend staff IDs (HR/RTM/IT/Q),
 * and legacy RTM-style IDs like O1 (Oliver).
 */
function isBonusTransferPayerId(id) {
  const s = String(id || "").trim().toUpperCase();
  if (!s) return false;
...
function suggestNextLeadId(employees, leadRole = "TL") {
  ...
  const { isDeletedEmployee } = require("./employee-identity");
  const reserved = collectReservedAppIds(employees);
  const nums = [];
  for (const e of employees || []) {
    if (isDeletedEmployee(e)) continue;
    for (const id of [e.id, e.archived_app_id].filter(Boolean)) {
  ...
  let next = max + 1;
  while (reserved.has(`${prefix}${String(next).padStart(2, "0")}`.toUpperCase())) next += 1;
  while (reserved.has(`${prefix}${next}`.toUpperCase())) next += 1;
...
module.exports = {
  ...
  LEAD_POSITIONS,
  collectReservedAppIds,
  parseFormerIds,
  mergeFormerIds,
  isLeadershipId,
  isBonusTransferPayerId,
