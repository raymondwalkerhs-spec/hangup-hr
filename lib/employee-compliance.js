...
function sanitizeEmployeeComplianceFields(emp) {
  ...
  const nationalId = String(out.national_id || "").trim();
  const passport = String(out.passport_number || "").trim();
  if (isEgyptianNationality(nationality)) {
    out.national_id = nationalId || null;
    out.passport_number = null;
    if (out.national_id) out.identification = out.national_id;
  } else if (nationality) {
    out.passport_number = passport || null;
