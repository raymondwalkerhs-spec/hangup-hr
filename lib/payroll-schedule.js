...
/** Training payout due when program completes — aligned with payroll anchor month. */
function trainingPaymentDueDate(program) {
  ...
  if (terminationEnd) return terminationEnd;
  ...
  if (ph4End) return ph4End;
  const anchor = resolveTrainingPayrollAnchorMonth(program, []);
  if (anchor) {
