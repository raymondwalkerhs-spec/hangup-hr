...
} = require("./tl-bonus-link");

function parseTlBonusTargetId(reason) {
  return parseTlBonusRecipientFromReason(reason) || null;
}

function parseTlBonusSourceId(reason) {
  return parseTlBonusSourceFromReason(reason) || null;
}
...
function buildDeductionDetailLines(payslip, deductionEvents, empMap, attendanceLines, config) {
  ...
  if (payslip.notApprovedHalfDayAmount > 0) {
    lines.push({
      type: "Not Approved Half Day",
      amount: payslip.notApprovedHalfDayAmount,
      label: `Not Approved Half Day — ${payslip.notApprovedHalfDayAmount} EGP (${payslip.notApprovedHalfDay || 0} × 1.5 salary days)`,
    });
  }
  if (payslip.notApprovedQuarterDayAmount > 0) {
    lines.push({
      type: "Not Approved Quarter Day",
      amount: payslip.notApprovedQuarterDayAmount,
      label: `Not Approved Quarter Day — ${payslip.notApprovedQuarterDayAmount} EGP (${payslip.notApprovedQuarterDay || 0} × 0.75 salary days)`,
    });
  }
